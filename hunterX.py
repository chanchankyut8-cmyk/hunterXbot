...
def build_help_embed(category: str, prefix: str, bot_thumb: str) -> discord.Embed:
    ...
    if category == "hunterx":
        ...
        desc = '\n'.join([
            ...
            f'`ticketreset`',
            f'`ticketlogs`',
            f'`jvc #channel` - Bot joins voice channel.',
            f'`dvc` - Bot leaves voice channel.',
            f'`deletemsglog`',
            f'`editmsglogs`',
            f'`modlogs`',
            f'`setwelcome`',
...
async def jvc(ctx, channel: discord.VoiceChannel):
    ...
    try:
        ...
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Joined VC", f"Joined {channel.mention}, muted and deafened.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", f"Failed to join {channel.mention}: {e}")
...
async def dvc(ctx):
    ...
    if not vc or not vc.is_connected():
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Not Connected", "I'm not connected to any voice channel.")
    try:
        channel_name = vc.channel.name
        await vc.disconnect(force=True)
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Disconnected", f"Left **{channel_name}**.")
    except Exception as e:
