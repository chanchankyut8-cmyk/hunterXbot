import discord
from discord.ext import commands
from discord.ui import Button, View, Select
import asyncio
from datetime import datetime, timedelta, timezone
import json
import os
import re
import aiohttp
import base64
import collections
import yt_dlp
import time
import resource
import random
import io
from typing import Optional, List

# Optional image support (ship/leave background). If Pillow isn't installed, the bot will still run.
try:
    from PIL import Image, ImageDraw, ImageFont
except Exception:  # pragma: no cover
    Image = ImageDraw = ImageFont = None

# ==================== MUSIC (H!P PLAYLIST) ====================
# Note: "h!p playlist <link>" is handled inside on_message (not a normal prefix command).
music_queues = collections.defaultdict(collections.deque)   # {guild_id: deque[{"url": str, "title": str|None}]}
music_locks = collections.defaultdict(asyncio.Lock)        # per guild lock
music_control_messages = {}  # {guild_id: {"message_id": int, "channel_id": int}}

MUSIC_CONTROL_EMOJIS = {
    "pause": "⏸️",
    "resume": "▶️",
    "skip": "⏭️",
    "stop": "⏹️",
}

YTDL_PLAYLIST_OPTS = {
    "quiet": True,
    "no_warnings": True,
    "extract_flat": True,
    "skip_download": True,
}

YTDL_STREAM_OPTS = {
    "quiet": True,
    "no_warnings": True,
    "format": "bestaudio/best",
    "noplaylist": True,
    "source_address": "0.0.0.0",
}

YTDL_SEARCH_OPTS = {
    **YTDL_STREAM_OPTS,
    "default_search": "ytsearch1",
}

FFMPEG_BEFORE_OPTS = "-reconnect 1 -reconnect_streamed 1 -reconnect_delay_max 5"
FFMPEG_OPTS = "-vn"

def _pick_entry_url(entry: dict, *, fallback_extractor: str = "") -> Optional[str]:
    if not entry:
        return None
    url = entry.get("webpage_url") or entry.get("url")
    if not url:
        return None
    if isinstance(url, str) and url.startswith("http"):
        return url
    # YouTube playlist entries often give an ID instead of full URL
    if "youtube" in (fallback_extractor or "").lower() and isinstance(url, str):
        return f"https://www.youtube.com/watch?v={url}"
    # Spotify flat entries often provide a spotify:track:<id> URL
    if "spotify" in (fallback_extractor or "").lower() and isinstance(url, str) and url.startswith("spotify:track:"):
        track_id = url.split(":")[-1].strip()
        if track_id:
            return f"https://open.spotify.com/track/{track_id}"
    return None

def _build_entry_search_query(entry: dict) -> Optional[str]:
    if not entry:
        return None
    title = (entry.get("track") or entry.get("title") or "").strip()
    artist = (entry.get("artist") or entry.get("uploader") or "").strip()
    if not artist:
        artists = entry.get("artists")
        if isinstance(artists, list):
            artist_names = []
            for artist_item in artists[:2]:
                if isinstance(artist_item, dict):
                    name = str(artist_item.get("name") or "").strip()
                else:
                    name = str(artist_item).strip()
                if name:
                    artist_names.append(name)
            artist = ", ".join(artist_names)
    if title and artist:
        return f"{artist} - {title} audio"
    if title:
        return f"{title} audio"
    return None

async def _ytdl_extract(opts: dict, url: str) -> Optional[dict]:
    loop = asyncio.get_event_loop()
    def _run():
        with yt_dlp.YoutubeDL(opts) as ydl:
            return ydl.extract_info(url, download=False)
    try:
        return await loop.run_in_executor(None, _run)
    except Exception:
        return None

async def _enqueue_playlist(guild: discord.Guild, playlist_url: str) -> int:
    """Extract playlist/video URLs and queue them. Returns number added."""
    info = await _ytdl_extract(YTDL_PLAYLIST_OPTS, playlist_url)
    if not info:
        return 0

    added = 0
    extractor = info.get("extractor", "") or ""

    # Playlist
    if isinstance(info, dict) and info.get("entries"):
        for entry in info.get("entries", [])[:25]:
            url = _pick_entry_url(entry, fallback_extractor=extractor)
            search_query = _build_entry_search_query(entry)
            if not url and not search_query:
                continue
            if not url:
                title = entry.get("title") or search_query or "Unknown"
            else:
                title = entry.get("title") or "Unknown"
            music_queues[guild.id].append({"url": url, "title": title, "search_query": search_query})
            added += 1
        return added

    # Single
    url = info.get("webpage_url") or playlist_url
    music_queues[guild.id].append({"url": url, "title": info.get("title"), "search_query": _build_entry_search_query(info)})
    return 1

async def _resolve_stream_info(item: dict) -> tuple[Optional[dict], Optional[str]]:
    info = None
    stream_url = None

    item_url = item.get("url")
    if item_url:
        info = await _ytdl_extract(YTDL_STREAM_OPTS, item_url)
        if info and info.get("entries"):
            entries = info.get("entries") or []
            info = entries[0] if entries else info
        stream_url = info.get("url") if info else None

    if (not stream_url) and item.get("search_query"):
        info = await _ytdl_extract(YTDL_SEARCH_OPTS, item["search_query"])
        if info and info.get("entries"):
            entries = info.get("entries") or []
            info = entries[0] if entries else info
        stream_url = info.get("url") if info else None

    return info, stream_url

async def _play_next(guild: discord.Guild, text_channel: discord.TextChannel = None):
    """Play next item in queue if nothing is currently playing."""
    if not guild:
        return
    voice_client = guild.voice_client
    if not voice_client or not voice_client.is_connected():
        music_queues[guild.id].clear()
        return
    if voice_client.is_playing() or voice_client.is_paused():
        return

    if not music_queues[guild.id]:
        return

    item = music_queues[guild.id].popleft()
    url = item.get("url")
    if not url and not item.get("search_query"):
        return await _play_next(guild, text_channel=text_channel)

    info, stream_url = await _resolve_stream_info(item)
    if not info:
        if text_channel:
            try:
                await text_channel.send("I couldn't play that track. Skipping...")
            except:
                pass
        return await _play_next(guild, text_channel=text_channel)

    title = info.get("title") or item.get("title") or "Unknown"

    if not stream_url:
        return await _play_next(guild, text_channel=text_channel)

    source = discord.FFmpegPCMAudio(stream_url, before_options=FFMPEG_BEFORE_OPTS, options=FFMPEG_OPTS)

    def _after(err):
        try:
            asyncio.run_coroutine_threadsafe(_play_next(guild, text_channel=text_channel), bot.loop)
        except:
            pass

    try:
        voice_client.play(source, after=_after)
        if text_channel:
            try:
                msg = await text_channel.send(f"Now playing: **{title}**")
                await _maybe_add_music_controls(guild, msg)
            except:
                pass
    except Exception:
        return await _play_next(guild, text_channel=text_channel)

async def _maybe_add_music_controls(guild: discord.Guild, message: discord.Message):
    """Add auto-reactions for music controls if enabled."""
    if not guild or not message:
        return
    gid = str(guild.id)
    cfg = config_data.get(gid, {})
    if not cfg.get("music_autoreact", True):
        return
    try:
        music_control_messages[guild.id] = {"message_id": message.id, "channel_id": message.channel.id}
        # Add reactions in order
        for key in ("pause", "resume", "skip", "stop"):
            await message.add_reaction(MUSIC_CONTROL_EMOJIS[key])
    except Exception:
        pass

async def _handle_hp_playlist(message: discord.Message, playlist_url: str):
    """Handle 'h!p playlist <url>' message."""
    if not message.guild:
        return
    if not message.author.voice or not message.author.voice.channel:
        try:
            await message.channel.send(f"{message.author.mention} Join a voice channel first before using `h!p playlist`.", delete_after=6)
        except:
            pass
        return

    guild = message.guild
    async with music_locks[guild.id]:
        # connect if needed
        try:
            vc = guild.voice_client
            if not vc or not vc.is_connected():
                await message.author.voice.channel.connect()
            elif vc.channel and vc.channel.id != message.author.voice.channel.id:
                await vc.move_to(message.author.voice.channel)
        except Exception:
            try:
                await message.channel.send("I couldn't connect to the voice channel. Check my permissions.", delete_after=6)
            except:
                pass
            return

        added = await _enqueue_playlist(guild, playlist_url)
        if added <= 0:
            try:
                await message.channel.send("No playlist tracks were found. The link may be private or blocked.", delete_after=8)
            except:
                pass
            return

        try:
            await message.channel.send(f"Added **{added}** track(s) to queue.")
        except:
            pass

        await _play_next(guild, text_channel=message.channel)

# ==================== PH TIME ====================
PH_TIME = timezone(timedelta(hours=8))

# ==================== BOT OWNER ====================
BOT_OWNER_ID = 1238037974877212707
BOT_VERSION = "1.0.0"

YOUTUBE_API_KEY = os.environ.get("YOUTUBE_API_KEY", "")

# ==================== FILES ====================
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(BASE_DIR, "data")
os.makedirs(DATA_DIR, exist_ok=True)

WHITELIST_FILE    = os.path.join(DATA_DIR, "whitelist.json")
LOGS_FILE         = os.path.join(DATA_DIR, "logs.json")
CONFIG_FILE       = os.path.join(DATA_DIR, "config.json")
AUTORESPOND_FILE  = os.path.join(DATA_DIR, "autorespond.json")
BACKUP_FILE       = os.path.join(DATA_DIR, "backup.json")
WARNS_FILE        = os.path.join(DATA_DIR, "warns.json")
ECONOMY_FILE      = os.path.join(DATA_DIR, "economy.json")
LEVELS_FILE       = os.path.join(DATA_DIR, "levels.json")

# ==================== LOAD/SAVE ====================
def load_data(file_name):
    if not os.path.exists(file_name):
        return {}
    try:
        with open(file_name, "r") as f:
            return json.load(f)
    except:
        return {}

def save_data(file_name, data):
    with open(file_name, "w") as f:
        json.dump(data, f, indent=4)

# ==================== DATA ====================
whitelist_data   = load_data(WHITELIST_FILE)
log_channels     = load_data(LOGS_FILE)
config_data      = load_data(CONFIG_FILE)
autorespond_data = load_data(AUTORESPOND_FILE)
backup_store     = load_data(BACKUP_FILE)
warns_data       = load_data(WARNS_FILE)
economy_data     = load_data(ECONOMY_FILE)
levels_data      = load_data(LEVELS_FILE)

def save_levels():
    save_data(LEVELS_FILE, levels_data)

def save_warns():
    save_data(WARNS_FILE, warns_data)

def save_economy():
    save_data(ECONOMY_FILE, economy_data)

setup_guilds     = set()
afk_users        = {}
spam_tracker     = collections.defaultdict(list)
join_tracker     = collections.defaultdict(collections.deque)  # {guild_id: deque[(ts, member_id)]}
BOT_BANNER_URL   = None
sticky_messages  = {}
# Cooldown for "need Manage Messages" warning per guild
delete_permission_warn_ts = {}  # {guild_id: unix_ts}
# snipe_data: {channel_id: {"content": str, "author": str, "author_icon": str, "time": datetime}}
snipe_data       = {}

# Partnership request state:
# {guild_id: {user_id: {"stage": "link"|"confirm", "link": str}}}
partner_requests = collections.defaultdict(dict)

# ==================== PREFIX ====================
FIXED_PREFIXES = ["$", "/", "x", ","]
NO_PREFIX_PREFIX = ""

async def get_prefix(bot, message):
    if message.guild:
        gid = str(message.guild.id)
        custom = config_data.get(gid, {}).get("prefix")
        prefixes = list(FIXED_PREFIXES)
        if custom and custom not in prefixes:
            prefixes.append(custom)
        prefixes.append(NO_PREFIX_PREFIX)
        return list(dict.fromkeys(prefixes))
    return list(dict.fromkeys([*FIXED_PREFIXES, NO_PREFIX_PREFIX]))

# ==================== INTENTS ====================
intents = discord.Intents.default()
intents.members = True
intents.message_content = True
intents.guilds = True
intents.moderation = True
intents.presences = True

bot = commands.Bot(
    command_prefix=get_prefix,
    intents=intents,
    help_command=None
)
bot_start_time = datetime.now(PH_TIME)

# ==================== PREFIX-ONLY "/" COMMANDS ====================
# Discord only allows up to 100 global slash commands.
# To support ALL commands with a "/" prefix, we treat "/" as a normal text prefix
# and map hybrid decorators to regular prefix commands.
def _prefix_only_hybrid_command(*dargs, **dkwargs):
    kwargs = dict(dkwargs)
    kwargs.pop("with_app_command", None)
    return bot.command(*dargs, **kwargs)

def _prefix_only_hybrid_group(*dargs, **dkwargs):
    kwargs = dict(dkwargs)
    kwargs.pop("with_app_command", None)
    return bot.group(*dargs, **kwargs)

bot.hybrid_command = _prefix_only_hybrid_command
bot.hybrid_group = _prefix_only_hybrid_group

WELCOME_ACTION_CHOICES = [
    discord.app_commands.Choice(name="Set Channel", value="set"),
    discord.app_commands.Choice(name="Test", value="test"),
]

SETUP_ONLY_CHOICES = [
    discord.app_commands.Choice(name="Setup", value="setup"),
]

async def _send_interaction_embed(interaction: discord.Interaction, title: str, description: str, *, ephemeral: bool = True):
    embed = make_embed(title, description)
    avatar = bot.user.display_avatar.url if bot.user else None
    embed.set_footer(text="HunterX", icon_url=avatar)
    if interaction.response.is_done():
        await interaction.followup.send(embed=embed, ephemeral=ephemeral)
    else:
        await interaction.response.send_message(embed=embed, ephemeral=ephemeral)

async def _run_slash_command(interaction: discord.Interaction, command_obj, **kwargs):
    try:
        ctx = await commands.Context.from_interaction(interaction)
    except Exception as e:
        return await _send_interaction_embed(interaction, "Error", f"Failed to build command context: {e}")

    try:
        allowed = await command_obj.can_run(ctx)
        if allowed is False:
            return await _send_interaction_embed(interaction, "No Permission", "You can't use this command here.")
    except commands.MissingPermissions as e:
        missing = ", ".join(getattr(e, "missing_permissions", []) or [])
        return await _send_interaction_embed(interaction, "No Permission", f"Missing permissions: `{missing or 'unknown'}`")
    except commands.BotMissingPermissions as e:
        missing = ", ".join(getattr(e, "missing_permissions", []) or [])
        return await _send_interaction_embed(interaction, "Bot Missing Permission", f"I need: `{missing or 'unknown'}`")
    except commands.NoPrivateMessage:
        return await _send_interaction_embed(interaction, "Server Only", "This command can only be used inside a server.")
    except commands.CheckFailure:
        return await _send_interaction_embed(interaction, "No Permission", "You can't use this command.")

    try:
        return await command_obj.callback(ctx, **kwargs)
    except Exception as e:
        return await _send_interaction_embed(interaction, "Error", str(e))

# ==================== IMPORTANT NATIVE SLASH COMMANDS ====================
@bot.tree.command(name="help", description="Show the help menu")
async def slash_help(interaction: discord.Interaction):
    await _run_slash_command(interaction, help)

@bot.tree.command(name="setup", description="Create the main HunterX logs setup")
async def slash_setup(interaction: discord.Interaction):
    await _run_slash_command(interaction, setup)

@bot.tree.command(name="setup-server", description="Setup server template (setupsv)")
@discord.app_commands.guild_only()
async def slash_setup_server(interaction: discord.Interaction):
    await _run_slash_command(interaction, setupsv)

@bot.tree.command(name="setupsv", description="Setup server template (short)")
@discord.app_commands.guild_only()
async def slash_setupsv(interaction: discord.Interaction):
    await _run_slash_command(interaction, setupsv)

@bot.tree.command(name="setprefix", description="Change the bot prefix for this server")
@discord.app_commands.describe(new_prefix="New prefix to use")
async def slash_setprefix(interaction: discord.Interaction, new_prefix: str):
    await _run_slash_command(interaction, setprefix, new_prefix=new_prefix)

@bot.tree.command(name="config", description="Show the current server config")
async def slash_config(interaction: discord.Interaction):
    await _run_slash_command(interaction, config_cmd)

@bot.tree.command(name="verification-setup", description="Setup verification system")
@discord.app_commands.describe(action="Verification action")
@discord.app_commands.choices(action=SETUP_ONLY_CHOICES)
async def slash_verification(interaction: discord.Interaction, action: str = "setup"):
    await _run_slash_command(interaction, verification, action=action)

@bot.tree.command(name="ticket-setup", description="Setup the ticket panel")
async def slash_ticket_setup(interaction: discord.Interaction):
    await _run_slash_command(interaction, ticketsetup)

@bot.tree.command(name="ticket-logs", description="Setup the ticket logs channel")
async def slash_ticket_logs(interaction: discord.Interaction):
    await _run_slash_command(interaction, ticketlogs)

@bot.tree.command(name="ticket-reset", description="Reset the ticket panel channel")
async def slash_ticket_reset(interaction: discord.Interaction):
    await _run_slash_command(interaction, ticketreset)

@bot.tree.command(name="user-info", description="Show user information")
@discord.app_commands.describe(member="Member to inspect")
async def slash_userinfo(interaction: discord.Interaction, member: Optional[discord.Member] = None):
    await _run_slash_command(interaction, userinfo, member=member)

@bot.tree.command(name="server-info", description="Show server information")
async def slash_serverinfo(interaction: discord.Interaction):
    await _run_slash_command(interaction, serverinfo)

@bot.tree.command(name="warn", description="Warn a member")
@discord.app_commands.describe(member="Member to warn", reason="Reason for the warn")
async def slash_warn(interaction: discord.Interaction, member: discord.Member, reason: str = "No reason provided"):
    await _run_slash_command(interaction, warn, member=member, reason=reason)

@bot.tree.command(name="checkwarns", description="Check a member's warns")
@discord.app_commands.describe(member="Member to check")
async def slash_checkwarns(interaction: discord.Interaction, member: discord.Member):
    await _run_slash_command(interaction, checkwarns, member=member)

@bot.tree.command(name="clearwarns", description="Clear all warns for a member")
@discord.app_commands.describe(member="Member to clear")
async def slash_clearwarns(interaction: discord.Interaction, member: discord.Member):
    await _run_slash_command(interaction, clearwarns, member=member)

@bot.tree.command(name="unwarn", description="Remove the latest warn from a member")
@discord.app_commands.describe(member="Member to unwarn")
async def slash_unwarn(interaction: discord.Interaction, member: discord.Member):
    await _run_slash_command(interaction, unwarn, member=member)

@bot.tree.command(name="setpfpbot", description="Update the bot profile picture (server-only)")
@discord.app_commands.guild_only()
@discord.app_commands.describe(image="Image attachment")
async def slash_setpfpbot(interaction: discord.Interaction, image: discord.Attachment):
    await _run_slash_command(interaction, setpfpbot, image=image)

@bot.tree.command(name="svbotpfp", description="Update the server-only bot profile picture")
@discord.app_commands.guild_only()
@discord.app_commands.describe(image="Image attachment")
async def slash_svbotpfp(interaction: discord.Interaction, image: discord.Attachment):
    await _run_slash_command(interaction, svbotpfp, image=image)

@bot.tree.command(name="setbnrbot", description="Update the bot banner (server-only)")
@discord.app_commands.guild_only()
@discord.app_commands.describe(image="Image attachment")
async def slash_setbnrbot(interaction: discord.Interaction, image: discord.Attachment):
    await _run_slash_command(interaction, setbnrbot, image=image)

@bot.tree.command(name="svbotbnr", description="Update the server-only bot banner")
@discord.app_commands.guild_only()
@discord.app_commands.describe(image="Image attachment")
async def slash_svbotbnr(interaction: discord.Interaction, image: discord.Attachment):
    await _run_slash_command(interaction, svbotbnr, image=image)

@bot.tree.command(name="setup-goodbye", description="Create the goodbye channel setup")
async def slash_setup_goodbye(interaction: discord.Interaction):
    await _run_slash_command(interaction, setup_goodbye)

@bot.tree.command(name="goodbye", description="Set or view the goodbye message")
@discord.app_commands.describe(message="Message text, or use off/remove to clear")
async def slash_goodbye(interaction: discord.Interaction, message: Optional[str] = None):
    await _run_slash_command(interaction, goodbye_msg, message=message)

@bot.tree.command(name="setleavebg", description="Set the leave background image")
@discord.app_commands.describe(url="Image URL or reset/default/black", image="Image attachment")
async def slash_setleavebg(interaction: discord.Interaction, image: Optional[discord.Attachment] = None, url: Optional[str] = None):
    await _run_slash_command(interaction, setleavebg, url=url, image=image)

@bot.tree.command(name="set-message", description="Set the welcome message")
@discord.app_commands.describe(message="Welcome message text")
async def slash_setmessage(interaction: discord.Interaction, message: str):
    await _run_slash_command(interaction, setmessage, message=message)

@bot.tree.command(name="welcome", description="Set the welcome channel or send a test")
@discord.app_commands.describe(channel="Channel for welcome messages", action="Set channel or run test")
@discord.app_commands.choices(action=WELCOME_ACTION_CHOICES)
async def slash_welcome(interaction: discord.Interaction, action: Optional[str] = None, channel: Optional[discord.TextChannel] = None):
    action_value = action or ("set" if channel else None)
    await _run_slash_command(interaction, welcome, channel=channel, action=action_value)

@bot.tree.command(name="boost-channel", description="Setup the boost channel")
@discord.app_commands.describe(action="Boost setup action")
@discord.app_commands.choices(action=SETUP_ONLY_CHOICES)
async def slash_boost_channel(interaction: discord.Interaction, action: str = "setup"):
    await _run_slash_command(interaction, boostchannel, action=action)

@bot.tree.command(name="boost-message", description="Set the boost message")
@discord.app_commands.describe(message="Boost message text")
async def slash_boost_message(interaction: discord.Interaction, message: str):
    await _run_slash_command(interaction, boostmessage, message=message)

# ==================== HELPERS ====================
def get_guild_config(guild_id):
    gid = str(guild_id)
    if gid not in config_data:
        config_data[gid] = {}
    return config_data[gid]

def save_config():
    save_data(CONFIG_FILE, config_data)

def is_toggle_on(value: str) -> bool:
    return str(value or "").strip().lower() in {"on", "enable", "enabled"}

def is_toggle_off(value: str) -> bool:
    return str(value or "").strip().lower() in {"off", "disable", "disabled"}

def clean_embed_text(text):
    if text is None:
        return None
    cleaned = strip_emojis(str(text))
    cleaned = re.sub(r"\$([A-Za-z][\w!\-]*)", r"\1", cleaned)
    replacements = {
        "Enabled": "ON",
        "Disabled": "OFF",
        "enabled": "ON",
        "disabled": "OFF",
        "Enable": "ON",
        "Disable": "OFF",
        "enable": "ON",
        "disable": "OFF",
    }
    for old, new in replacements.items():
        cleaned = re.sub(rf"\b{old}\b", new, cleaned)
    return re.sub(r"[ \t]+", " ", cleaned).strip()

# Regular embeds → white sidebar
def auto_embed_color(title: str) -> int:
    t = (title or "").lower()
    error_words = ["no permission", "error", "invalid", "failed", "denied", "cannot", "can't", "not found", "already", "missing"]
    success_words = ["success", "enabled", "added", "created", "removed", "banned", "kicked", "unbanned", "cleared", "set", "updated"]
    if any(w in t for w in error_words):
        return 0xFF3B3B
    if any(w in t for w in success_words):
        return 0x57F287
    return 0xFFFFFF

def make_embed(title: str = "", description: str = "", color: int = None) -> discord.Embed:
    embed = discord.Embed(color=color if color is not None else auto_embed_color(title))
    if title:
        embed.title = clean_embed_text(title)
    if description:
        embed.description = clean_embed_text(description)
    embed.timestamp = datetime.now(PH_TIME)
    return embed

# Log embeds → black sidebar
def make_log_embed(title: str, description: str = "") -> discord.Embed:
    embed = discord.Embed(color=0x000000)
    if title:
        embed.title = clean_embed_text(title)
    if description:
        embed.description = clean_embed_text(description)
    embed.timestamp = datetime.now(PH_TIME)
    return embed

async def send_embed(ctx, title, description, color: int = None):
    embed = make_embed(title, description, color=color)
    avatar = bot.user.display_avatar.url if bot.user else None
    embed.set_footer(text="HunterX", icon_url=avatar)
    await ctx.send(embed=embed)

EMOJI_PATTERN = re.compile(
    "["
    "\U0001F300-\U0001FAFF"
    "\U00002600-\U000027BF"
    "\U0001F1E6-\U0001F1FF"
    "\U00002700-\U000027BF"
    "\U0001F900-\U0001F9FF"
    "\U00002300-\U000023FF"
    "]+", flags=re.UNICODE
)

def strip_emojis(text):
    if not text:
        return text
    text = re.sub(r"<a?:\w+:\d+>", "", text)
    text = EMOJI_PATTERN.sub("", text)
    return re.sub(r"[ \t]+", " ", text).strip()

INVISIBLE_CHARS_PATTERN = re.compile(r"[\u200b-\u200f\u2060\uFEFF]")

def is_bad_nickname(name: str) -> bool:
    if not name:
        return False
    n = str(name)
    lowered = n.lower()
    # common ad / spam patterns
    if "discord.gg" in lowered or "discord.com/invite" in lowered or "http://" in lowered or "https://" in lowered:
        return True
    # invisible chars / hangul filler
    if "ㅤ" in n or INVISIBLE_CHARS_PATTERN.search(n):
        return True
    # too many emojis
    emoji_count = len(EMOJI_PATTERN.findall(n))
    if emoji_count >= 4:
        return True
    # too long (discord nick limit is 32)
    if len(n) > 32:
        return True
    return False

def get_level_from_xp(xp: int) -> int:
    # Level curve: XP required = 100 * level^2
    xp = max(0, int(xp))
    return int((xp / 100) ** 0.5)

def get_user_level(guild_id: int, user_id: int) -> dict:
    gid = str(guild_id)
    uid = str(user_id)
    levels_data.setdefault(gid, {})
    levels_data[gid].setdefault(uid, {"xp": 0, "level": 0, "last_xp": 0})
    return levels_data[gid][uid]

URL_PATTERN = re.compile(r"https?://[^\s>]+", re.IGNORECASE)
PLAYLIST_ALLOWED_DOMAINS = (
    "youtube.com",
    "youtu.be",
    "music.youtube.com",
    "spotify.com",
    "open.spotify.com",
)

def is_allowed_playlist_url(url: str) -> bool:
    lowered = (url or "").lower()
    return any(domain in lowered for domain in PLAYLIST_ALLOWED_DOMAINS)

# ==================== BADWORDS HELPERS ====================
BADWORD_TOKEN_PATTERN = re.compile(r"[a-z0-9]+", re.IGNORECASE)
COMPACT_MATCH_PATTERN = re.compile(r"[^a-z0-9]+", re.IGNORECASE)

def _normalize_tokens(text: str) -> List[str]:
    if not text:
        return []
    return BADWORD_TOKEN_PATTERN.findall(text.lower())

def _normalize_compact(text: str) -> str:
    if not text:
        return ""
    return COMPACT_MATCH_PATTERN.sub("", text.lower())

def find_keyword_match(text: str, keywords: List[str]) -> Optional[str]:
    if not text or not keywords:
        return None
    lower_text = str(text).lower()
    compact_text = _normalize_compact(lower_text)
    for word in keywords:
        ww = (word or "").strip().lower()
        if not ww:
            continue
        compact_word = _normalize_compact(ww)
        if ww in lower_text:
            return ww
        if compact_word and compact_word in compact_text:
            return ww
    return None

def find_badword_match(text: str, badwords: List[str]) -> Optional[str]:
    """Return the matched badword token if found, otherwise None."""
    if not text or not badwords:
        return None
    lower_text = str(text).lower()
    compact_text = _normalize_compact(lower_text)
    tokens = set(_normalize_tokens(lower_text))
    for w in badwords:
        ww = (w or "").strip().lower()
        if not ww:
            continue
        compact_word = _normalize_compact(ww)
        # Match either as a standalone token (strict) OR as a substring (looser, catches variants like "putangina")
        if ww in tokens or ww in lower_text or (compact_word and compact_word in compact_text):
            return ww
    return None

def patch_discord_embed_text():
    if getattr(discord.Embed, "_hunterx_clean_patch", False):
        return

    original_init = discord.Embed.__init__
    original_add_field = discord.Embed.add_field
    original_set_author = discord.Embed.set_author
    original_set_footer = discord.Embed.set_footer

    def clean_init(self, *args, **kwargs):
        if kwargs.get("title"):
            kwargs["title"] = clean_embed_text(kwargs["title"])
        if kwargs.get("description"):
            kwargs["description"] = clean_embed_text(kwargs["description"])
        return original_init(self, *args, **kwargs)

    def clean_add_field(self, *, name, value, inline=True):
        return original_add_field(
            self,
            name=clean_embed_text(name),
            value=clean_embed_text(value),
            inline=inline
        )

    def clean_set_author(self, *, name, url=None, icon_url=None):
        return original_set_author(self, name=clean_embed_text(name), url=url, icon_url=icon_url)

    def clean_set_footer(self, *, text=None, icon_url=None):
        cleaned_text = clean_embed_text(text) if text is not None else None
        return original_set_footer(self, text=cleaned_text, icon_url=icon_url)

    discord.Embed.__init__ = clean_init
    discord.Embed.add_field = clean_add_field
    discord.Embed.set_author = clean_set_author
    discord.Embed.set_footer = clean_set_footer
    discord.Embed._hunterx_clean_patch = True

patch_discord_embed_text()

def get_user_economy(user_id: int):
    uid = str(user_id)
    if uid not in economy_data:
        economy_data[uid] = {
            "cash": 0,
            "last_daily": None,
            "inventory": {},   # {item_key: qty}
            "last_rob": None,  # ISO datetime
        }
    # Backward compatibility for old profiles
    economy_data[uid].setdefault("inventory", {})
    economy_data[uid].setdefault("last_rob", None)
    return economy_data[uid]

def format_cash(amount: int) -> str:
    return f"{amount:,}"

async def log_action(guild, title, description, thumbnail_url=None):
    channel_id = log_channels.get(str(guild.id))
    if not channel_id:
        channel = None
    else:
        channel = guild.get_channel(channel_id)
    try:
        embed = make_log_embed(title, description)
        avatar = bot.user.display_avatar.url if bot.user else None
        embed.set_footer(text="HunterX", icon_url=avatar)
        if thumbnail_url:
            embed.set_thumbnail(url=thumbnail_url)
        if channel:
            await channel.send(embed=embed)

        # Mirror everything to /all-logs if configured
        gid = str(guild.id)
        all_logs_id = config_data.get(gid, {}).get("all_logs_channel")
        if all_logs_id:
            all_ch = guild.get_channel(all_logs_id)
            if all_ch and (not channel or all_ch.id != channel.id):
                await all_ch.send(embed=embed)
    except:
        pass

async def log_mod_action(guild, title, description, thumbnail_url=None):
    gid = str(guild.id)
    ch_id = config_data.get(gid, {}).get("modlogs_channel")
    if not ch_id:
        channel = None
    else:
        channel = guild.get_channel(ch_id)
    try:
        embed = make_log_embed(title, description)
        embed.timestamp = datetime.now(PH_TIME)
        avatar = bot.user.display_avatar.url if bot.user else None
        embed.set_footer(text="HunterX", icon_url=avatar)
        if thumbnail_url:
            embed.set_thumbnail(url=thumbnail_url)
        if channel:
            await channel.send(embed=embed)

        # Mirror to /all-logs if configured
        all_logs_id = config_data.get(gid, {}).get("all_logs_channel")
        if all_logs_id:
            all_ch = guild.get_channel(all_logs_id)
            if all_ch and (not channel or all_ch.id != channel.id):
                await all_ch.send(embed=embed)
    except:
        pass

def is_whitelisted(guild, member):
    gid = str(guild.id)
    if gid not in whitelist_data:
        return False
    return str(member.id) in whitelist_data[gid].get("users", {})

def is_owner(ctx):
    return ctx.author.id == ctx.guild.owner_id or ctx.author.id == BOT_OWNER_ID

def is_antinuke_enabled(guild_id):
    return get_guild_config(guild_id).get("antinuke", False)

def is_guild_owner_or_bot(guild, user):
    return user.id == guild.owner_id or user.id == bot.user.id

def is_higher_role_than_bot(guild, user):
    try:
        bot_top = guild.me.top_role.position
        user_top = user.top_role.position
        return user_top > bot_top
    except:
        return False

# ==================== GIF HELPER ====================
async def fetch_gif(action: str) -> str:
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://nekos.best/api/v2/{action}", timeout=aiohttp.ClientTimeout(total=8)) as resp:
                data = await resp.json()
                return data["results"][0]["url"]
    except:
        return None

# ==================== FORMAT NUMBERS ====================
def _fmt_num(n):
    if n is None:
        return "N/A"
    if n >= 1_000_000:
        return f"{n / 1_000_000:.1f}M"
    if n >= 1_000:
        return f"{n / 1_000:.1f}K"
    return str(n)

# ==================== VERIFY VIEW ====================
class VerifyView(View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="✅ VERIFY!", style=discord.ButtonStyle.secondary, custom_id="hunterx_verify")
    async def verify_button(self, interaction: discord.Interaction, button: Button):
        guild = interaction.guild
        member = interaction.user
        role = discord.utils.get(guild.roles, name="VERIFIED")
        if not role:
            await interaction.response.send_message("VERIFIED role not found. Ask an admin to run $verification setup again.", ephemeral=True)
            return
        if role in member.roles:
            await interaction.response.send_message("You are already verified!", ephemeral=True)
            return
        try:
            await member.add_roles(role)
            await interaction.response.send_message("You have been verified! You can now access all channels.", ephemeral=True)
        except Exception as e:
            await interaction.response.send_message(f"<:HunterX:1512787796635422934> Error: {e}", ephemeral=True)

# ==================== INVITE VIEW ====================
class InviteView(View):
    def __init__(self):
        super().__init__(timeout=None)

    def build(self):
        invite_url = f"https://discord.com/oauth2/authorize?client_id={bot.user.id}&permissions=8&scope=bot"
        self.add_item(discord.ui.Button(label="INVITE", url=invite_url, style=discord.ButtonStyle.link))
        self.add_item(discord.ui.Button(label="WEBSITE", url="https://hunterx2.netlify.app", style=discord.ButtonStyle.link))
        return self

# ==================== TICKET LOG HELPER ====================
async def log_ticket(guild, title, description):
    gid = str(guild.id)
    ch_id = config_data.get(gid, {}).get("ticketlog_channel")
    if not ch_id:
        ch = None
    else:
        ch = guild.get_channel(ch_id)
    try:
        embed = make_log_embed(title, description)
        embed.set_footer(text="HunterX")
        embed.timestamp = datetime.now(PH_TIME)
        if ch:
            await ch.send(embed=embed)

        # Mirror to /all-logs if configured
        all_logs_id = config_data.get(gid, {}).get("all_logs_channel")
        if all_logs_id:
            all_ch = guild.get_channel(all_logs_id)
            if all_ch and (not ch or all_ch.id != ch.id):
                await all_ch.send(embed=embed)
    except:
        pass

# ==================== TICKET CLOSE VIEW ====================
class CloseView(View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="Close", style=discord.ButtonStyle.danger, custom_id="hunterx_ticket_close")
    async def close_button(self, interaction: discord.Interaction, button: Button):
        channel = interaction.channel
        guild = interaction.guild
        closer = interaction.user
        ticket_name = channel.name
        owner_id = None
        if channel.topic and "Ticket owner:" in channel.topic:
            try:
                owner_id = int(channel.topic.split("Ticket owner:")[1].strip())
            except:
                pass
        await interaction.response.send_message("Closing ticket in 3 seconds...", ephemeral=False)
        owner_mention = f"<@{owner_id}>" if owner_id else "Unknown"
        await log_ticket(guild, "<:HunterXapp:1512783709198225459> Ticket Closed",
            f"*Ticket:* #{ticket_name}\n**Closed by:** {closer.mention} ({closer.id})\n**Ticket Owner:** {owner_mention}")
        await asyncio.sleep(3)
        try:
            await channel.delete(reason="Ticket closed")
        except:
            pass

# ==================== TICKET CREATE VIEW ====================
class TicketView(View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="SUPPORT", style=discord.ButtonStyle.secondary, custom_id="hunterx_ticket_create")
    async def create_ticket(self, interaction: discord.Interaction, button: Button):
        guild = interaction.guild
        member = interaction.user
        gid = str(guild.id)
        cfg = get_guild_config(guild.id)

        category = discord.utils.get(guild.categories, name="helpdesk")
        if not category:
            await interaction.response.send_message("Ticket category not found. Ask an admin to run $ticketsetup.", ephemeral=True)
            return

        for ch in category.text_channels:
            if ch.topic and str(member.id) in ch.topic:
                await interaction.response.send_message(f"You already have an open ticket: {ch.mention}", ephemeral=True)
                return

        count = cfg.get("ticket_count", 0) + 1
        cfg["ticket_count"] = count
        save_config()

        ticket_name = f"ticket-{count:02d}"

        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False),
            guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, manage_channels=True),
            member: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True),
        }
        for role in guild.roles:
            if role.permissions.administrator:
                overwrites[role] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True, manage_channels=True)

        try:
            ticket_ch = await guild.create_text_channel(
                name=ticket_name,
                category=category,
                overwrites=overwrites,
                topic=f"Ticket owner: {member.id}",
                reason=f"Ticket created by {member}"
            )
        except Exception as e:
            await interaction.response.send_message(f"Failed to create ticket: {e}", ephemeral=True)
            return

        await interaction.response.send_message(f"Your ticket has been created: {ticket_ch.mention}", ephemeral=True)

        await log_ticket(guild, "<:HunterXmail:1514600414941675550> Ticket Opened",
            f"*Ticket:* #{ticket_name}\n**Opened by:** {member.mention} ({member.id})\n**Channel:** {ticket_ch.mention}")

        sv_name_caps = guild.name.split()[0].upper() if guild.name.split() else guild.name.upper()
        ticket_embed = discord.Embed(
            description=(
                f"# {sv_name_caps}\n\n"
                f"# TICKET RULES\n\n"
                f"- 5 Hours no reply Ticket Close\n\n"
                f"- If you dont have need dont create ticket\n\n"
                f"- Troll ticket = Timeout 5hrs\n\n"
                f"- Spam ticket = Timeout 3days\n\n"
                f"1st attempt timeout 1 day\n"
                f"2nd attempt timeout 2 days\n"
                f"3rd attempt timeout 5days\n"
                f"4th attempt = Ban"
            ),
            color=0xFFFFFF
        )
        if guild.icon:
            ticket_embed.set_image(url=guild.icon.url)
        ticket_embed.set_footer(text="HunterX")
        await ticket_ch.send(content=member.mention, embed=ticket_embed, view=CloseView())

# ==================== ECONOMY SHOP ====================
# You said "you decide" for the items; you can edit these any time.
SHOP_ITEMS = {
    "vip": {"name": "VIP Pass", "price": 15000, "desc": "Just for flex. No perks yet."},
    "medkit": {"name": "Medkit", "price": 2500, "desc": "Roleplay item / collection."},
    "gun": {"name": "Pistol", "price": 8000, "desc": "Collection item."},
    "car": {"name": "Car", "price": 30000, "desc": "Collection item."},
    "house": {"name": "House", "price": 75000, "desc": "Collection item."},
}

def _inv_add(profile: dict, item_key: str, qty: int = 1):
    inv = profile.setdefault("inventory", {})
    inv[item_key] = int(inv.get(item_key, 0)) + int(qty)
    if inv[item_key] <= 0:
        inv.pop(item_key, None)

def _inv_str(profile: dict) -> str:
    inv = profile.get("inventory") or {}
    if not inv:
        return "Empty"
    lines = []
    for key, qty in sorted(inv.items(), key=lambda x: (-int(x[1]), x[0])):
        item = SHOP_ITEMS.get(key)
        name = item["name"] if item else key
        lines.append(f"• **{name}** x`{qty}`")
    return "\n".join(lines)[:4096]

def _safe_channel_name(name: str) -> str:
    name = (name or "").strip().lower()
    name = name.replace("&", "and")
    name = re.sub(r"\s+", "-", name)
    name = re.sub(r"[^a-z0-9\-_]", "", name)
    name = re.sub(r"-{2,}", "-", name).strip("-")
    return name[:90] or "channel"

async def _ensure_admin_overwrites(guild: discord.Guild, *, view_for_everyone: bool):
    overwrites = {
        guild.default_role: discord.PermissionOverwrite(view_channel=view_for_everyone),
        guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True, manage_channels=True),
    }
    for role in guild.roles:
        if role.permissions.administrator:
            overwrites[role] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True, manage_channels=True)
    return overwrites

def _format_duration(delta: timedelta) -> str:
    seconds = int(delta.total_seconds())
    if seconds < 0:
        seconds = 0
    days, rem = divmod(seconds, 86400)
    hours, rem = divmod(rem, 3600)
    minutes, _ = divmod(rem, 60)
    if days >= 30:
        months = days // 30
        days_left = days % 30
        return f"{months} month(s) {days_left} day(s)"
    if days > 0:
        return f"{days} day(s) {hours} hour(s)"
    if hours > 0:
        return f"{hours} hour(s) {minutes} minute(s)"
    return f"{minutes} minute(s)"

async def _fetch_bytes(url: str) -> Optional[bytes]:
    if not url:
        return None
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=8)) as resp:
                if resp.status != 200:
                    return None
                return await resp.read()
    except Exception:
        return None

async def _render_ship_image(member1: discord.Member, member2: discord.Member, percent: int) -> Optional[discord.File]:
    if Image is None:
        return None

    bg = Image.new("RGB", (900, 300), (20, 20, 20))
    draw = ImageDraw.Draw(bg)
    # simple gradient bar
    for x in range(900):
        c = int(40 + (x / 900) * 60)
        draw.line([(x, 0), (x, 300)], fill=(c, 20, 40))

    # load avatars
    a1 = await _fetch_bytes(member1.display_avatar.replace(size=256).url)
    a2 = await _fetch_bytes(member2.display_avatar.replace(size=256).url)
    if not a1 or not a2:
        return None
    im1 = Image.open(io.BytesIO(a1)).convert("RGBA").resize((220, 220))
    im2 = Image.open(io.BytesIO(a2)).convert("RGBA").resize((220, 220))

    def circle(im):
        mask = Image.new("L", im.size, 0)
        ImageDraw.Draw(mask).ellipse((0, 0, im.size[0], im.size[1]), fill=255)
        out = Image.new("RGBA", im.size)
        out.paste(im, (0, 0), mask)
        return out

    im1 = circle(im1)
    im2 = circle(im2)
    bg.paste(im1, (70, 40), im1)
    bg.paste(im2, (610, 40), im2)

    # text (fallback to default font)
    try:
        font_big = ImageFont.truetype("DejaVuSans.ttf", 64)
        font_small = ImageFont.truetype("DejaVuSans.ttf", 28)
    except Exception:
        font_big = font_small = ImageFont.load_default()

    mid_text = f"{percent}%"
    w, h = draw.textbbox((0, 0), mid_text, font=font_big)[2:]
    draw.text(((900 - w) // 2, 95), mid_text, font=font_big, fill=(255, 255, 255))
    draw.text((380, 180), "FUN SHIP", font=font_small, fill=(255, 255, 255))

    out = io.BytesIO()
    bg.save(out, format="PNG")
    out.seek(0)
    return discord.File(out, filename="ship.png")

async def _render_leave_image(member: discord.Member, bg_url: Optional[str]) -> Optional[discord.File]:
    if Image is None:
        return None

    base = Image.new("RGB", (900, 300), (0, 0, 0))
    if bg_url:
        bg_bytes = await _fetch_bytes(bg_url)
        if bg_bytes:
            try:
                bg = Image.open(io.BytesIO(bg_bytes)).convert("RGB")
                bg = bg.resize((900, 300))
                base.paste(bg, (0, 0))
            except Exception:
                pass

    overlay = Image.new("RGBA", (900, 300), (0, 0, 0, 120))
    base = base.convert("RGBA")
    base.alpha_composite(overlay)

    draw = ImageDraw.Draw(base)
    try:
        font_big = ImageFont.truetype("DejaVuSans.ttf", 52)
        font_small = ImageFont.truetype("DejaVuSans.ttf", 28)
    except Exception:
        font_big = font_small = ImageFont.load_default()

    draw.text((40, 40), "GOODBYE", font=font_big, fill=(255, 255, 255))
    draw.text((40, 110), member.display_name[:20], font=font_small, fill=(255, 255, 255))

    out = io.BytesIO()
    base.convert("RGB").save(out, format="PNG")
    out.seek(0)
    return discord.File(out, filename="leave.png")

# ==================== HELP EMBED BUILDER ====================
def build_help_embed(category: str, prefix: str, bot_thumb: str) -> discord.Embed:
    p = prefix

    if category == "hunterx":
        desc = '\n'.join([
            '# __Anti Nuke__',
            '',
            f'>>> `setup`',
            f'`securityall enable/disable`',
            f'`whitelist @user`',
            f'`rwhitelist @user`',
            f'`whitelistboard`',
            f'`antinuke enable/disable`',
            f'`antibotadd enable/disable`',
            f'`antikick enable/disable`',
            f'`antiban enable/disable`',
            f'`anticrtwebhook enable/disable`',
            f'`anticrtadminrole enable/disable`',
            f'`antibadwords enable/disable`',
            f'`badwords add/remove/list/clear`',
            f'`antinickname enable/disable`',
            f'`antilink enable/disable`',
            f'`allowedlink @user/role`',
            f'`allowedlinkboard`',
            f'`antispam enable/disable`',
            f'`antimassmention enable/disable`',
            f'`antimassraid enable/disable`',
            f'`anticrtchannel enable/disable`',
            f'`antidelchannel enable/disable`',
            f'`antidelrole enable/disable`',
        ])
    elif category == "moderation":
        desc = '\n'.join([
            '# __Moderation__',
            '',
            f'>>> `ban @user [reason]`',
            f'`unban <id> [reason]`',
            f'`unban all`',
            f'`banlist`',
            f'`kick @user [reason]`',
            f'`timeout @user <duration> [reason]`',
            f'`rtimeout @user`',
            f'`role @user @role`',
            f'`drole @user @role`',
            f'`warn @user [reason]`',
            f'`unwarn @user`',
            f'`checkwarns @user`',
            f'`clearwarns @user`',
            f'`lock / unlock #channel`',
            f'`lockdown / unlockdown`',
            f'`purge <amount>`',
            f'`snipe`',
            f'`setprefix <prefix>`',
            f'`config`',
            f'`serverinfo`',
            f'`botstats`',
            f'`whois @user`',
            f'`hide`',
            f'`unhide`',
            f'`autoroleadd @role`',
            f'`setsvicon`',
            f'`setsvbnr`',
            f'`sticky <msg>`',
            f'`unsticky`',
            '',
            '**Setup & Config**',
            '',
            f'`ticketsetup`',
            f'`ticketreset`',
            f'`ticketlogs`',
            f'`deletemsglog`',
            f'`editmsglogs`',
            f'`modlogs`',
            f'`all-logs`',
            f'`voice-logs`',
            f'`setwelcome`',
            f'`welcome #channel`',
            f'`setmessage <msg>`',
            f'`welcome test`',
            f'`verification setup`',
            f'`setup-goodbye`',
            f'`goodbye <msg>`',
            f'`set-leave-bg <url>`',
            f'`setup-server`',
            f'`boostchannel setup`',
            f'`boostmessage <msg>`',
            f'`leveling enable/disable`',
            f'`leveling cooldown <seconds>`',
            f'`leveling channel #channel`',
            f'`rank [@user]`',
            f'`level-top`',
            f'`partnership-post #channel`',
            f'`auto-react enable/disable`',
            f'`autorespondadd <trigger> <reply>`',
            f'`autorespondedit <trigger> <reply>`',
            f'`autorespondremove <trigger>`',
            f'`backupcreate` / `backup`',
            f'`backuprestore <id>` / `restore <id>`',
        ])
    elif category == "utility":
        desc = '\n'.join([
            '# __Utility__',
            '',
            f'>>> `av @user` - View avatar/pfp.',
            f'`banner @user` - View banner.',
            f'`userinfo @user` - View userinfo',
            f'`rinfo @role` - View role info',
            f'`svpfp` - View server pfp.',
            f'`svbnr` - View server banner.',
            f'`set-bot-pfp [image]` - Set the bot profile picture for this server.',
            f'`set-bot-banner [image]` - Set the bot banner for this server.',
            f'`server-bot-pfp` / `sbp` - Set the server bot profile picture.',
            f'`server-bot-bnr` / `sbb` - Set the server bot banner.',
            f'`ig <username>` - Instagram Acc.',
            f'`tt <username>` - Tiktok Acc.',
            f'`yt <channel name>` - Youtube',
            f'`rbx <username>` - Roblox Acc.',
            f'`afk [reason]` - Set AFK',
            f'`cash [@user]` - View cash balance.',
            f'`daily` - Claim daily reward.',
            f'`shop` - View shop items.',
            f'`buy <item> [qty]` - Buy shop items.',
            f'`inventory [@user]` - View inventory.',
            f'`give @user <amount>` - Send cash.',
            f'`slot <amount>` - Play slot.',
            f'`rob @user` - Attempt to rob.',
            f'`top-cash` - Cash leaderboard.',
            f'`ship @user1 @user2` - Ship two users.',
            f'`jvc` - Join your voice channel.',
            f'`dvc` - Disconnect from voice.',
            f'`music-stop / music-skip / music-pause / music-resume`',
            f'`ping` - Latency.',
            f'`botinfo` - Bot information.',
        ])
    else:  # fun
        desc = '\n'.join([
            '# __Fun__',
            '',
            f'>>> `kiss @user` - Kiss someone.',
            f'`hug @user` - Hug someone.',
            f'`cuddle @user` - Cuddle someone.',
            f'`slap @user` - Slap someone.',
            f'`punch @user` - Punch someone.',
            f'`boxing @user` - Boxing someone.',
            f'`throw @user` - Throw someone.',
            f'`hcf <amount> <h/t>` / `coinflip` - Flip a coin for cash.',
        ])

    embed = discord.Embed(description=desc, color=0xFFFFFF)
    embed.set_author(name="HunterX", icon_url=bot_thumb)
    embed.set_thumbnail(url=bot_thumb)
    embed.set_footer(text="HunterX")
    return embed

# ==================== HELP PAGES ====================
HELP_PAGES = [
    {
        "section": "Anti Nuke",
        "commands": [
            "╰›Setup",
            "╰›Securityall ON/OFF",
            "╰›Whitelist @user",
            "╰›Rwhitelist @user",
            "╰›Whitelistboard",
            "╰›Anti-Nuke ON/OFF",
            "╰›Anti-Bot ON/OFF",
            "╰›Anti-Kick ON/OFF",
            "╰›Anti-Ban ON/OFF",
            "╰›Anti-Create Webhook ON/OFF",
            "╰›Anti-Create Role ON/OFF",
            "╰›Anti-Badwords ON/OFF",
            "╰›Badwords Add/Remove/List/Clear",
            "╰›Anti-Nickname ON/OFF",
            "╰›Anti-Link ON/OFF",
            "╰›Allowed-Link @user/role",
            "╰›Allowed-Linkboard",
            "╰›Anti-Spam ON/OFF",
            "╰›Anti-Mass-Mention ON/OFF",
            "╰›Anti-Mass-Raid ON/OFF",
            "╰›Anti-Create Channel ON/OFF",
            "╰›Anti-Delete Channel ON/OFF",
            "╰›Anti-Delete Role ON/OFF",
        ],
    },
    {
        "section": "Moderation",
        "commands": [
            "╰›Ban @user [reason]",
            "╰›Unban <id> [reason]",
            "╰›Unban all",
            "╰›Banlist",
            "╰›Kick @user [reason]",
            "╰›Timeout @user <duration> [reason]",
            "╰›Rtimeout @user",
            "╰›Role @user @role",
            "╰›Drole @user @role",
            "╰›Warn @user [reason]",
            "╰›Unwarn @user",
            "╰›Checkwarns @user",
            "╰›Clearwarns @user",
            "╰›Lock / Unlock #channel",
            "╰›Lockdown / Unlockdown",
            "╰›Purge <amount>",
            "╰›Snipe",
            "╰›Setprefix <prefix>",
            "╰›Config",
            "╰›Who-Is @user",
            "╰›Hide",
            "╰›Unhide",
            "╰›Add-Keywords <add/remove/view/clear> [keyword]",
            "╰›Auto-Role Add @role",
            "╰›Sticky <msg>",
            "╰›Unsticky",
        ],
    },
    {
        "section": "Server / Bot",
        "commands": [
            "╰›Server-Info (si)",
            "╰›Set-Server-Icon (setsvpfp/spfp)",
            "╰›Set-Server-Banner (setsvbnr/sbnr)",
            "╰›Set-Bot-Pfp [image]",
            "╰›Set-Bot-Banner [image]",
            "╰›Server-Bot-pfp (sbp)",
            "╰›Server-Bot-Bnr (sbb)",
            "╰›Setup-Server",
            "╰›Server-List (sl)",
            "╰›Bot-Stats (bs)",
            "╰›Bot-Info (bi)",
        ],
    },
    {
        "section": "Setup / Welcome",
        "commands": [
            "╰›Verification-Setup",
            "╰›Delete-Msg Log (dml)",
            "╰›Edit-Msg Log (eml)",
            "╰›Modlogs",
            "╰›All-Logs",
            "╰›Voice-Logs",
            "╰›Set-Welcome",
            "╰›Welcome #channel",
            "╰›Welcome-Test",
            "╰›Set-Message",
            "╰›Setup-Goodbye",
            "╰›Goodbye <msg>",
            "╰›Set-Leave-Bg",
            "╰›Leveling ON/OFF",
            "╰›Leveling Cooldown <seconds>",
            "╰›Leveling Channel #channel",
            "╰›Rank [@user]",
            "╰›Level-Top",
            "╰›Backup / Backupcreate",
            "╰›Restore / Backuprestore <id>",
        ],
    },
    {
        "section": "Ticket / Respond",
        "commands": [
            "╰›Ticket-Setup",
            "╰›Ticket-Logs",
            "╰›Ticket-Reset",
            "╰›Partnership-Post #channel",
            "╰›Respond-Add",
            "╰›Respond-Edit",
            "╰›Respond-Remove",
        ],
    },
    {
        "section": "Boost / JoinVc",
        "commands": [
            "╰›Boost-Channel",
            "╰›Boost-Message",
            "╰›Auto-React ON/OFF",
            "╰›Jvc",
            "╰›Dvc",
            "╰›H!p Playlist <spotify/youtube link>",
            "╰›Music-Stop / Skip / Pause / Resume",
        ],
    },
    {
        "section": "Social / Info",
        "commands": [
            "╰›Youtube @chname (yt)",
            "╰›Instagram @user (ig)",
            "╰›Tiktok @user (tt)",
            "╰›Roblox @user (rbx)",
            "╰›User-Info (ui)",
            "╰›Role-Info (rinfo)",
            "╰›Server-Profile (svpfp)",
            "╰›Server-Banner (svbnr)",
            "╰›Avatar (av)",
            "╰›Banner (bnr)",
            "╰›Afk",
            "╰›Ping",
            "╰›Ship @user1 @user2",
        ],
    },
    {
        "section": "Fun",
        "commands": [
            "╰›Kiss @user",
            "╰›Hug @user",
            "╰›Cuddle @user",
            "╰›Slap @user",
            "╰›Boxing @user",
            "╰›Punch @user",
            "╰›Throw @user",
            "╰›Hcf <amount> <h/t>",
            "╰›Cash / Hcash",
            "╰›Daily / Hdaily",
            "╰›Shop / Hshop",
            "╰›Buy / Hbuy",
            "╰›Inventory / Hinv",
            "╰›Give / Hgive",
            "╰›Slot / Hslot",
            "╰›Rob / Hrob",
            "╰›Top-Cash / Htopcash",
        ],
    },
]

def build_help_page_embed(page: int, bot_thumb: str) -> discord.Embed:
    total_pages = len(HELP_PAGES)
    page = max(1, min(page, total_pages))
    page_data = HELP_PAGES[page - 1]
    divider = f"⎯⎯⎯⎯⎯⎯⎯⎯  {page_data['section']} ⎯⎯⎯⎯⎯⎯⎯⎯"
    description = "\n".join([
        "╰› HunterX — Security & Moderation",
        divider,
        "",
        *page_data["commands"],
    ])

    embed = discord.Embed(title="╰› HunterX | Commands", description=description, color=0xFFFFFF)
    embed.set_thumbnail(url=bot_thumb)
    embed.set_footer(text=f"HunterX • Page {page}/{total_pages}")
    return embed

class HelpPaginatorView(View):
    def __init__(self, author_id: int, page: int = 1):
        super().__init__(timeout=180)
        self.author_id = author_id
        self.page = page
        self.total_pages = len(HELP_PAGES)
        self._sync_buttons()

    def _sync_buttons(self):
        self.prev_button.disabled = self.page <= 1
        self.next_button.disabled = self.page >= self.total_pages

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message("Only you can use these help buttons.", ephemeral=True)
            return False
        return True

    @discord.ui.button(label="PREVIOUS", style=discord.ButtonStyle.secondary)
    async def prev_button(self, interaction: discord.Interaction, button: Button):
        if self.page > 1:
            self.page -= 1
        self._sync_buttons()
        embed = build_help_page_embed(self.page, bot.user.display_avatar.url)
        await interaction.response.edit_message(embed=embed, view=self)

    @discord.ui.button(label="NEXT", style=discord.ButtonStyle.secondary)
    async def next_button(self, interaction: discord.Interaction, button: Button):
        if self.page < self.total_pages:
            self.page += 1
        self._sync_buttons()
        embed = build_help_page_embed(self.page, bot.user.display_avatar.url)
        await interaction.response.edit_message(embed=embed, view=self)

# ==================== BOTSTATS PAGINATOR ====================
SERVERS_PER_PAGE = 6

def build_serverlist_embed(page: int) -> discord.Embed:
    guilds = list(bot.guilds)
    total = len(guilds)
    total_pages = max(1, (total + SERVERS_PER_PAGE - 1) // SERVERS_PER_PAGE)
    page = max(1, min(page, total_pages))

    start = (page - 1) * SERVERS_PER_PAGE
    end = start + SERVERS_PER_PAGE
    page_guilds = guilds[start:end]

    lines = [
        f"**__SERVER LIST__**\n",
        f"╰ **Total Servers:** `{total}`",
        f"╰ **Page {page}/{total_pages}**\n",
    ]
    for i, g in enumerate(page_guilds, start=start + 1):
        status = "Normal"
        members = g.member_count or 0
        lines.append(f"**{i}.** **{g.name}**")
        lines.append(f"└─ **ID:** `{g.id}` | **Members:** `{members}` | **Status:** {status}")
    lines.append(f"\n╰ **Auto-updates every 15 seconds**")

    embed = discord.Embed(description="\n".join(lines), color=0xFFFFFF)
    embed.set_thumbnail(url=bot.user.display_avatar.url)
    embed.set_footer(text="HunterX")
    embed.timestamp = datetime.now(PH_TIME)
    return embed

class ServerListView(View):
    def __init__(self, page: int):
        super().__init__(timeout=60)
        self.page = page
        guilds = list(bot.guilds)
        total = len(guilds)
        total_pages = max(1, (total + SERVERS_PER_PAGE - 1) // SERVERS_PER_PAGE)
        self.total_pages = total_pages

    @discord.ui.button(label="< Previous", style=discord.ButtonStyle.secondary, custom_id="serverlist_prev")
    async def prev_button(self, interaction: discord.Interaction, button: Button):
        if self.page > 1:
            self.page -= 1
        embed = build_serverlist_embed(self.page)
        await interaction.response.edit_message(embed=embed, view=self)

    @discord.ui.button(label="Next >", style=discord.ButtonStyle.secondary, custom_id="serverlist_next")
    async def next_button(self, interaction: discord.Interaction, button: Button):
        if self.page < self.total_pages:
            self.page += 1
        embed = build_serverlist_embed(self.page)
        await interaction.response.edit_message(embed=embed, view=self)


# ==================== KEEPALIVE ====================
async def keepalive_loop():
    """Ping the public URL every 10 minutes to prevent Render free-tier spin-down."""
    await bot.wait_until_ready()
    external_url = os.environ.get("RENDER_EXTERNAL_URL")
    if external_url:
        url = external_url
    else:
        port = int(os.environ.get("PORT", 8000))
        url = f"http://localhost:{port}/"
    while not bot.is_closed():
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    pass
        except Exception:
            pass
        await asyncio.sleep(600)

@bot.event
async def on_disconnect():
    print(f"[{datetime.now(PH_TIME).strftime('%Y-%m-%d %H:%M:%S')}] Bot disconnected. Reconnecting...")

# ==================== READY ====================
@bot.event
async def on_ready():
    global BOT_BANNER_URL
    for gid in log_channels:
        setup_guilds.add(int(gid))
    bot.add_view(VerifyView())
    bot.add_view(TicketView())
    bot.add_view(CloseView())
    try:
        bot_user = await bot.fetch_user(bot.user.id)
        if bot_user.banner:
            BOT_BANNER_URL = bot_user.banner.url
    except:
        pass
    try:
        synced = await bot.tree.sync()
        print(f"Synced {len(synced)} slash command(s)")
    except Exception as e:
        print(f"Failed to sync slash commands: {e}")
    bot.loop.create_task(keepalive_loop())
    try:
        await bot.change_presence(
            status=discord.Status.dnd,
            activity=discord.Streaming(name="HunterX", url="https://www.twitch.tv/hunterx")
        )
    except Exception as e:
        print(f"Failed to set presence: {e}")
    print(f"Logged in as {bot.user}")

# ==================== BOT JOIN ====================
@bot.event
async def on_guild_join(guild):
    inviter = None
    try:
        async for entry in guild.audit_logs(limit=5, action=discord.AuditLogAction.bot_add):
            if entry.target and entry.target.id == bot.user.id:
                inviter = entry.user
                break
    except:
        pass

    if inviter:
        try:
            dm_embed = discord.Embed(
                description=(
                    f"**HunterX**\n\n"
                    f"Thankyou For Using <:cutei:1515273052315713617> **HunterX**\n\n"
                    f"Thanks for adding me to **{guild.name}**!\n\n"
                    f"Command\n"
                    f"`$setup`\nTo setup logs\n\n"
                    f"`$help`\nShow Commands\n\n"
                    f"**Prefixes** — `$` | `/` | `x` | `,`"
                ),
                color=0xFFFFFF
            )
            dm_embed.set_thumbnail(url=guild.me.display_avatar.url)
            dm_embed.set_footer(text="HunterX")
            await inviter.send(embed=dm_embed)
        except:
            pass

    for channel in guild.text_channels:
        try:
            if channel.permissions_for(guild.me).send_messages:
                embed = discord.Embed(
                    description=(
                        f"**HunterX**\n\n"
                        f"Thankyou For Using <:cutei:1515273052315713617> **HunterX**\n\n"
                        f"Command\n"
                        f"`$setup`\nTo setup logs\n\n"
                        f"`$help`\nShow Commands\n\n"
                f"**Prefixes** — `$` | `/` | `x` | `,`"
                    ),
                )
                embed.set_thumbnail(url=guild.me.display_avatar.url)
                embed.set_footer(text="HunterX")
                await channel.send(embed=embed)
                break
        except:
            pass

# ==================== PING ====================
@bot.hybrid_command()
async def ping(ctx):
    latency_ms = round(bot.latency * 1000)
    embed = make_embed("<a:HappyHunterXemoji:1512831297582661803> Ping!", f"**Latency:** `{latency_ms}ms`")
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== SETUP ====================
@bot.hybrid_command()
@commands.has_permissions(administrator=True)
async def setup(ctx):
    guild = ctx.guild
    try:
        gid = str(guild.id)
        tracked_id = log_channels.get(gid)
        tracked_channel = guild.get_channel(tracked_id) if tracked_id else None
        existing = tracked_channel or discord.utils.get(guild.text_channels, name="hunterx-logs")
        if existing:
            log_channels[gid] = existing.id
            save_data(LOGS_FILE, log_channels)
            setup_guilds.add(guild.id)
            embed = discord.Embed(
                description=(
                    f"**Already Setup**\n\n"
                    f"{existing.mention} is already set as the log channel."
                ),
                color=0xFFFFFF
            )
            embed.set_footer(text="HunterX")
            return await ctx.send(embed=embed)
        overwrites = {guild.default_role: discord.PermissionOverwrite(view_channel=False)}
        channel = await guild.create_text_channel(name="hunterx-logs", overwrites=overwrites)
        for role in guild.roles:
            if role.permissions.administrator:
                await channel.set_permissions(role, view_channel=True, send_messages=True, read_message_history=True)
        await channel.set_permissions(guild.me, view_channel=True, send_messages=True, read_message_history=True)
        log_channels[str(guild.id)] = channel.id
        save_data(LOGS_FILE, log_channels)
        setup_guilds.add(guild.id)
        await send_embed(ctx, "Setup Successful", f"{channel.mention} created successfully.")
    except Exception as e:
        await ctx.send(f"```{e}```")

# ==================== HELP ====================
@bot.hybrid_command(aliases=["h!p"])
async def help(ctx):
    if not ctx.guild:
        return
    embed = build_help_page_embed(1, bot.user.display_avatar.url)
    view = HelpPaginatorView(ctx.author.id, page=1)
    await ctx.send(embed=embed, view=view)

# ==================== WHITELIST (Owner Only) ====================
# Supports:
# - `whitelist @user` (legacy add)
# - `whitelist add @user`
# - `whitelist remove @user`
# - `whitelist list`
@bot.hybrid_group(name="whitelist", invoke_without_command=True)
async def whitelist(ctx, member: discord.Member = None):
    if not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")
    if ctx.author.id != ctx.guild.owner_id:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the server owner can use this command.")
    # Legacy usage: `whitelist @user`
    if member is not None:
        return await ctx.invoke(whitelist_add, member=member)
    # Default: show list
    return await ctx.invoke(whitelist_list)

@whitelist.command(name="add")
async def whitelist_add(ctx, member: discord.Member):
    if not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")
    if ctx.author.id != ctx.guild.owner_id:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the server owner can use this command.")
    gid = str(ctx.guild.id)
    whitelist_data.setdefault(gid, {"users": {}})
    whitelist_data[gid].setdefault("users", {})
    whitelist_data[gid]["users"][str(member.id)] = ["all"]
    save_data(WHITELIST_FILE, whitelist_data)
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> User Whitelisted", f"{member.mention} is now whitelisted.")

@whitelist.command(name="remove", aliases=["rm", "del"])
async def whitelist_remove(ctx, member: discord.Member):
    if not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")
    if ctx.author.id != ctx.guild.owner_id:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the server owner can use this command.")
    gid = str(ctx.guild.id)
    if gid in whitelist_data and str(member.id) in whitelist_data[gid].get("users", {}):
        del whitelist_data[gid]["users"][str(member.id)]
        save_data(WHITELIST_FILE, whitelist_data)
        return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Removed", f"{member.mention} removed from whitelist.")
    await send_embed(ctx, "<:HunterX:1512787796635422934> Not Found", f"{member.mention} is not whitelisted.")

@whitelist.command(name="list", aliases=["board"])
async def whitelist_list(ctx):
    if not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")
    if ctx.author.id != ctx.guild.owner_id:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the server owner can use this command.")
    gid = str(ctx.guild.id)
    users = whitelist_data.get(gid, {}).get("users", {})
    if not users:
        return await send_embed(ctx, "<:249630gradientgear:1512782705522245712> Whitelist", "No whitelisted users.")
    desc = ""
    for i, uid in enumerate(users.keys(), 1):
        member = ctx.guild.get_member(int(uid))
        name = member.mention if member else f"<@{uid}>"
        desc += f"**{i}.** {name}\n"
    embed = make_embed("<:249630gradientgear:1512782705522245712> Whitelist Board", desc)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# Backward compatible commands
@bot.hybrid_command(name="rwhitelist")
async def rwhitelist(ctx, member: discord.Member):
    if not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")
    if ctx.author.id != ctx.guild.owner_id:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the server owner can use this command.")
    return await ctx.invoke(whitelist_remove, member=member)

@bot.hybrid_command(name="whitelistboard")
async def whitelistboard(ctx):
    if not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")
    if ctx.author.id != ctx.guild.owner_id:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the server owner can use this command.")
    return await ctx.invoke(whitelist_list)

# ==================== SET PREFIX ====================
@bot.hybrid_command()
@commands.has_permissions(administrator=True)
async def setprefix(ctx, new_prefix: str):
    gid = str(ctx.guild.id)
    if gid not in config_data:
        config_data[gid] = {}
    config_data[gid]["prefix"] = new_prefix
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Prefix Updated", f"New prefix is now `{new_prefix}`")

# ==================== AUTO ROLE ADD ====================
@bot.hybrid_command()
@commands.has_permissions(administrator=True)
async def autoroleadd(ctx, role: discord.Role):
    gid = str(ctx.guild.id)
    if gid not in config_data:
        config_data[gid] = {}
    config_data[gid]["autorole"] = role.id
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AutoRole Set", f"{role.mention} will now be given to all new members.")

# ==================== AVATAR (aliases: av) ====================
@bot.hybrid_command(aliases=["avatar"])
async def av(ctx, member: discord.Member = None):
    target = member or ctx.author
    embed = discord.Embed(
        description=f"**{target.display_name}'s Avatar**",
        color=0xFFFFFF
    )
    embed.set_image(url=target.display_avatar.url)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== BANNER ====================
@bot.hybrid_command(aliases=["bnr"])
async def banner(ctx, member: discord.Member = None):
    target = member or ctx.author
    try:
        user = await bot.fetch_user(target.id)
        if not user.banner:
            return await send_embed(ctx, "No Banner", f"{target.display_name} has no banner.")
        embed = discord.Embed(
            description=f"**{target.display_name}'s Banner**",
            color=0xFFFFFF
        )
        embed.set_image(url=user.banner.url)
        embed.set_footer(text="HunterX")
        await ctx.send(embed=embed)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== ROLE INFO ====================
@bot.hybrid_command(name="role-info", aliases=["rinfo"])
async def rinfo(ctx, role: discord.Role = None):
    if not role:
        return await send_embed(ctx, "Usage", "`$rinfo @role` — mention a role to view its info")
    created = role.created_at.strftime("%B %d, %Y")
    members_with_role = len([m for m in ctx.guild.members if role in m.roles])
    perms = []
    if role.permissions.administrator:
        perms.append("Administrator")
    if role.permissions.manage_guild:
        perms.append("Manage Server")
    if role.permissions.manage_channels:
        perms.append("Manage Channels")
    if role.permissions.manage_roles:
        perms.append("Manage Roles")
    if role.permissions.ban_members:
        perms.append("Ban Members")
    if role.permissions.kick_members:
        perms.append("Kick Members")
    if role.permissions.moderate_members:
        perms.append("Moderate Members")
    if role.permissions.manage_messages:
        perms.append("Manage Messages")
    perms_str = ", ".join(perms) if perms else "No special permissions"

    embed = discord.Embed(
        description=f"**Role Info — {role.mention}**",
        color=role.color if role.color.value else 0xFFFFFF
    )
    embed.add_field(name="Role Name", value=role.name, inline=True)
    embed.add_field(name="Role ID", value=str(role.id), inline=True)
    embed.add_field(name="Color", value=str(role.color), inline=True)
    embed.add_field(name="Position", value=str(role.position), inline=True)
    embed.add_field(name="Members", value=str(members_with_role), inline=True)
    embed.add_field(name="Mentionable", value="Yes" if role.mentionable else "No", inline=True)
    embed.add_field(name="Hoisted", value="Yes" if role.hoist else "No", inline=True)
    embed.add_field(name="Created", value=created, inline=True)
    embed.add_field(name="Key Permissions", value=perms_str, inline=False)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== SERVER PFP ====================
@bot.hybrid_command()
async def svpfp(ctx):
    guild = ctx.guild
    if not guild.icon:
        return await send_embed(ctx, "No Server Icon", "This server has no icon set.")
    embed = discord.Embed(
        description=f"**{guild.name} — Server Icon**",
        color=0xFFFFFF
    )
    embed.set_image(url=guild.icon.url)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== SERVER BANNER ====================
@bot.hybrid_command()
async def svbnr(ctx):
    guild = ctx.guild
    if not guild.banner:
        return await send_embed(ctx, "No Server Banner", "This server has no banner set.")
    embed = discord.Embed(
        description=f"**{guild.name} — Server Banner**",
        color=0xFFFFFF
    )
    embed.set_image(url=guild.banner.url)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== SET SERVER ICON ====================
@bot.hybrid_command(name="setsvpfp", aliases=["setsvicon", "spfp", "set-server-icon"])
@commands.has_permissions(manage_guild=True)
async def setsvicon(ctx, image: discord.Attachment = None):
    attachment = image or _ctx_attachment(ctx, "image")
    if not attachment:
        return await send_embed(ctx, "Usage", "`$setsvicon` — attach an image to set as the server icon")
    if not any(attachment.filename.lower().endswith(ext) for ext in [".png", ".jpg", ".jpeg", ".gif", ".webp"]):
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid", "Please attach a valid image (PNG, JPG, GIF, WEBP).")
    try:
        image_data = await attachment.read()
        await ctx.guild.edit(icon=image_data, reason=f"HunterX setsvicon by {ctx.author}")
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Server Icon Updated", "The server icon has been changed successfully.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== SET SERVER BANNER ====================
@bot.hybrid_command(name="setsvbnr", aliases=["sbnr", "set-server-banner"])
@commands.has_permissions(manage_guild=True)
async def setsvbnr(ctx, image: discord.Attachment = None):
    attachment = image or _ctx_attachment(ctx, "image")
    if not attachment:
        return await send_embed(ctx, "Usage", "`$setsvbnr` — attach an image to set as the server banner")
    if not any(attachment.filename.lower().endswith(ext) for ext in [".png", ".jpg", ".jpeg", ".gif", ".webp"]):
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid", "Please attach a valid image (PNG, JPG, GIF, WEBP).")
    try:
        image_data = await attachment.read()
        await ctx.guild.edit(banner=image_data, reason=f"HunterX setsvbnr by {ctx.author}")
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Server Banner Updated", "The server banner has been changed successfully.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== SET BOT PFP / BANNER ====================
def can_use_serveronly_bot_media(ctx) -> bool:
    try:
        return ctx.author.id == BOT_OWNER_ID or ctx.author.id == ctx.guild.owner_id
    except Exception:
        return False

def _valid_image_attachment(attachment) -> bool:
    if not attachment:
        return False
    return any((attachment.filename or "").lower().endswith(ext) for ext in [".png", ".jpg", ".jpeg", ".gif", ".webp"])

def _attachment_mime_type(filename: str) -> str:
    lowered = (filename or "").lower()
    if lowered.endswith(".png"):
        return "image/png"
    if lowered.endswith(".jpg") or lowered.endswith(".jpeg"):
        return "image/jpeg"
    if lowered.endswith(".gif"):
        return "image/gif"
    if lowered.endswith(".webp"):
        return "image/webp"
    return "application/octet-stream"

def _to_data_uri(image_data: bytes, filename: str) -> str:
    mime = _attachment_mime_type(filename)
    encoded = base64.b64encode(image_data).decode("ascii")
    return f"data:{mime};base64,{encoded}"

async def _update_current_guild_bot_profile(guild: discord.Guild, payload: dict):
    auth_token = os.environ.get("DISCORD_TOKEN") or globals().get("token")
    if not auth_token:
        raise RuntimeError("DISCORD_TOKEN is not set.")

    url = f"https://discord.com/api/v10/guilds/{guild.id}/members/@me"
    headers = {
        "Authorization": f"Bot {auth_token}",
        "Content-Type": "application/json",
    }
    async with aiohttp.ClientSession() as session:
        async with session.patch(url, headers=headers, json=payload) as resp:
            if 200 <= resp.status < 300:
                return
            error_text = await resp.text()
            raise RuntimeError(f"Discord API error {resp.status}: {error_text[:500]}")

def _ctx_attachment(ctx, *names):
    message = getattr(ctx, "message", None)
    if message and getattr(message, "attachments", None):
        return message.attachments[0]
    interaction = getattr(ctx, "interaction", None)
    namespace = getattr(interaction, "namespace", None) if interaction else None
    if namespace:
        for name in names:
            attachment = getattr(namespace, name, None)
            if attachment:
                return attachment
    return None

async def _set_bot_avatar(ctx, image: discord.Attachment = None, *, command_name: str = "set-bot-pfp"):
    """Set the bot avatar for the current server context only."""
    if not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")
    if not can_use_serveronly_bot_media(ctx):
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the server owner or bot owner can use this command.")

    usage_text = f"`{command_name}` with an attached image."
    attachment = image or _ctx_attachment(ctx, "image")
    if not attachment:
        return await send_embed(ctx, "Usage", usage_text)
    if not _valid_image_attachment(attachment):
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid", "Please attach a valid image (PNG, JPG, GIF, WEBP).")
    try:
        image_data = await attachment.read()
        avatar_data_uri = _to_data_uri(image_data, attachment.filename or "avatar.png")
        await _update_current_guild_bot_profile(ctx.guild, {"avatar": avatar_data_uri})
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Server Bot PFP Updated", "The bot profile picture was updated successfully for this server.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

@bot.hybrid_command(name="setpfpbot", aliases=["set-bot-pfp", "bot-pfp"])
async def setpfpbot(ctx, image: discord.Attachment = None):
    await _set_bot_avatar(ctx, image=image, command_name="set-bot-pfp")

@bot.hybrid_command(name="svbotpfp", aliases=["server-bot-pfp", "sbp"])
async def svbotpfp(ctx, image: discord.Attachment = None):
    await _set_bot_avatar(ctx, image=image, command_name="server-bot-pfp")

async def _set_bot_banner(ctx, image: discord.Attachment = None, *, command_name: str = "set-bot-banner"):
    """Set the bot banner for the current server context only."""
    if not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")
    if not can_use_serveronly_bot_media(ctx):
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the server owner or bot owner can use this command.")

    usage_text = f"`{command_name}` with an attached image."
    attachment = image or _ctx_attachment(ctx, "image")
    if not attachment:
        return await send_embed(ctx, "Usage", usage_text)
    if not _valid_image_attachment(attachment):
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid", "Please attach a valid image (PNG, JPG, GIF, WEBP).")
    try:
        image_data = await attachment.read()
        banner_data_uri = _to_data_uri(image_data, attachment.filename or "banner.png")
        await _update_current_guild_bot_profile(ctx.guild, {"banner": banner_data_uri})
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Server Bot Banner Updated", "The bot banner was updated successfully for this server.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

@bot.hybrid_command(name="setbnrbot", aliases=["set-bot-banner", "bot-banner"])
async def setbnrbot(ctx, image: discord.Attachment = None):
    await _set_bot_banner(ctx, image=image, command_name="set-bot-banner")

@bot.hybrid_command(name="svbotbnr", aliases=["server-bot-banner", "server-bot-bnr", "sbb"])
async def svbotbnr(ctx, image: discord.Attachment = None):
    await _set_bot_banner(ctx, image=image, command_name="server-bot-bnr")

# ==================== STICKY ====================
@bot.hybrid_command()
@commands.has_permissions(manage_messages=True)
async def sticky(ctx, *, message: str):
    channel_id = ctx.channel.id
    if channel_id in sticky_messages:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Already Set",
            "This channel already has a sticky message. Use `$unsticky` first before setting a new one.")
    sent = await ctx.send(f"{message}")
    sticky_messages[channel_id] = {"text": message, "msg_id": sent.id}
    try:
        await ctx.message.delete()
    except:
        pass

# ==================== UNSTICKY ====================
@bot.hybrid_command()
@commands.has_permissions(manage_messages=True)
async def unsticky(ctx):
    channel_id = ctx.channel.id
    if channel_id not in sticky_messages:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Sticky", "There is no sticky message in this channel.")
    old = sticky_messages.pop(channel_id)
    try:
        old_msg = await ctx.channel.fetch_message(old["msg_id"])
        await old_msg.delete()
    except:
        pass
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Sticky Removed", "The sticky message has been removed.")

# ==================== SNIPE ====================
@bot.hybrid_command()
async def snipe(ctx):
    data = snipe_data.get(ctx.channel.id)
    if not data:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Nothing to Snipe", "No recently deleted messages found in this channel.")
    embed = discord.Embed(
        description=data["content"] or "*[No text content]*",
        color=0xFFFFFF,
        timestamp=data["time"]
    )
    embed.set_author(name=data["author"], icon_url=data["author_icon"])
    embed.set_footer(text="HunterX • Deleted message")
    await ctx.send(embed=embed)

# ==================== ROBLOX ====================
@bot.hybrid_command()
async def rbx(ctx, *, username: str):
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(
                "https://users.roblox.com/v1/usernames/users",
                json={"usernames": [username], "excludeBannedUsers": False}
            ) as resp:
                data = await resp.json()
                if not data.get("data"):
                    return await send_embed(ctx, "<:HunterX:1512787796635422934> Not Found", f"Roblox user `{username}` not found.")
                user_data = data["data"][0]
                user_id = user_data["id"]
                display_name = user_data["displayName"]
                roblox_name = user_data["name"]

            async with session.get(f"https://users.roblox.com/v1/users/{user_id}") as resp:
                profile = await resp.json()
                created_raw = profile.get("created", "")
                if created_raw:
                    created_dt = datetime.fromisoformat(created_raw.replace("Z", "+00:00"))
                    created_str = created_dt.strftime("%B %d, %Y")
                else:
                    created_str = "Unknown"

            async with session.get(f"https://friends.roblox.com/v1/users/{user_id}/followers/count") as resp:
                followers = (await resp.json()).get("count", 0)
            async with session.get(f"https://friends.roblox.com/v1/users/{user_id}/followings/count") as resp:
                following = (await resp.json()).get("count", 0)
            async with session.get(f"https://friends.roblox.com/v1/users/{user_id}/friends/count") as resp:
                friends = (await resp.json()).get("count", 0)
            async with session.get(
                f"https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds={user_id}&size=150x150&format=Png&isCircular=false"
            ) as resp:
                thumb_data = await resp.json()
                avatar_url = thumb_data.get("data", [{}])[0].get("imageUrl", "")

            embed = make_embed("Roblox Profile")
            embed.add_field(name="Roblox User", value=f"{display_name} (@{roblox_name})", inline=True)
            embed.add_field(name="Roblox ID", value=str(user_id), inline=True)
            embed.add_field(name="\u200b", value="\u200b", inline=False)
            embed.add_field(name="Followers", value=f"{followers:,}", inline=True)
            embed.add_field(name="Following", value=f"{following:,}", inline=True)
            embed.add_field(name="Friends", value=f"{friends:,}", inline=True)
            embed.add_field(name="Account Created", value=created_str, inline=False)
            if avatar_url:
                embed.set_thumbnail(url=avatar_url)
            embed.set_footer(text="HunterX")
            await ctx.send(embed=embed)
        except Exception as e:
            await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== PURGE ====================
@bot.hybrid_command()
@commands.has_permissions(manage_messages=True)
async def purge(ctx, amount: int):
    if amount < 1 or amount > 500:
        return await send_embed(ctx, "Invalid", "Amount must be between 1 and 500.")
    try:
        await ctx.message.delete()
        deleted = await ctx.channel.purge(limit=amount)
        embed = make_embed("<:HunteXCheck:1518422536558481448> Purged", f"Deleted **{len(deleted)}** messages.")
        embed.set_footer(text="HunterX")
        await ctx.send(embed=embed)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== VERIFICATION ====================
@bot.hybrid_command(name="verification-setup", aliases=["verification"])
@commands.has_permissions(administrator=True)
async def verification(ctx, action: str = "setup"):
    if action.lower() != "setup":
        return await send_embed(ctx, "Invalid", "Usage: $verification setup")
    guild = ctx.guild
    try:
        verified_role = discord.utils.get(guild.roles, name="VERIFIED")
        if not verified_role:
            verified_role = await guild.create_role(name="VERIFIED", color=discord.Color.green(), reason="HunterX Verification")
        existing = discord.utils.get(guild.text_channels, name="hunterx-verify")
        if existing:
            await existing.delete()
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=True, send_messages=False),
            guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True),
            verified_role: discord.PermissionOverwrite(view_channel=True, send_messages=False),
        }
        verify_channel = await guild.create_text_channel(name="hunterx-verify", overwrites=overwrites)
        embed = discord.Embed(
            description=(
                f"**Welcome To {guild.name}** <a:kuromicheer:1513064044221960282>\n\n"
                f"Click the button below to verify yourself\n"
                f"and gain access to all channels."
            ),
            color=0xFFFFFF
        )
        embed.set_footer(text="HunterX")
        await verify_channel.send(embed=embed, view=VerifyView())

        # These channels should stay hidden even after verification (admin/log channels).
        protected_names = {
            "hunterx-logs",
            "hunterx-modlogs",
            "hunterx-dltmsglogs",
            "hunterx-editmsglogs",
            "hunterx-ticketlogs",
            "all-logs",
            "vclogs",
            "ticket-logs",
            "modlogs",
            "deletemsglogs",
            "editmsglogs",
            "ticket-logs",
            "vclogs",
        }

        hidden = 0
        for ch in list(guild.text_channels) + list(guild.voice_channels):
            if ch.id == verify_channel.id:
                continue
            try:
                await ch.set_permissions(guild.default_role, view_channel=False)
                if (ch.name or "").lower() in protected_names:
                    await ch.set_permissions(verified_role, view_channel=False)
                else:
                    await ch.set_permissions(verified_role, view_channel=True)
                hidden += 1
            except:
                pass
        for cat in guild.categories:
            try:
                await cat.set_permissions(guild.default_role, view_channel=False)
                await cat.set_permissions(verified_role, view_channel=True)
            except:
                pass

        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Verification Setup",
            f"{verify_channel.mention} created.\nVerified role: {verified_role.mention}\n\nHidden **{hidden}** channels from @everyone.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== TICKET SETUP ====================
@bot.hybrid_command(name="ticket-setup", aliases=["ticketsetup", "ticketsetup1"])
@commands.has_permissions(administrator=True)
async def ticketsetup(ctx):
    guild = ctx.guild
    gid = str(guild.id)
    try:
        category = discord.utils.get(guild.categories, name="helpdesk")
        if not category:
            category = await guild.create_category("helpdesk")

        existing = discord.utils.get(guild.text_channels, name="hunterx-support")
        if existing:
            embed = discord.Embed(
                description=(
                    f"**<:HunteXCheck:1518422536558481448> Already Setup**\n\n"
                    f"Ticket system is already setup at {existing.mention}."
                ),
                color=0xFFFFFF
            )
            embed.set_footer(text="HunterX")
            return await ctx.send(embed=embed)

        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=True, send_messages=False),
            guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, manage_channels=True),
        }
        for role in guild.roles:
            if role.permissions.administrator:
                overwrites[role] = discord.PermissionOverwrite(view_channel=True, send_messages=True)

        support_ch = await guild.create_text_channel(
            name="hunterx-support",
            category=category,
            overwrites=overwrites,
            reason="HunterX Ticket Setup"
        )

        config_data.setdefault(gid, {})["ticket_channel"] = support_ch.id
        save_config()

        sv_name_caps = guild.name.split()[0].upper() if guild.name.split() else guild.name.upper()
        panel_embed = discord.Embed(
            description=(
                f"# {sv_name_caps}\n\n"
                f"# TICKET RULES\n\n"
                f"- 5 Hours no reply Ticket Close\n\n"
                f"- If you dont have need dont create ticket\n\n"
                f"- Troll ticket = Timeout 5hrs\n\n"
                f"- Spam ticket = Timeout 3days\n\n"
                f"1st attempt timeout 1 day\n"
                f"2nd attempt timeout 2 days\n"
                f"3rd attempt timeout 5days\n"
                f"4th attempt = Ban"
            ),
            color=0xFFFFFF
        )
        if guild.icon:
            panel_embed.set_image(url=guild.icon.url)
        panel_embed.set_footer(text="HunterX")
        await support_ch.send(embed=panel_embed, view=TicketView())

        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Ticket Setup Complete",
            f"Category *helpdesk* and {support_ch.mention} created.\nAll tickets will be created here.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== TICKET LOGS ====================
@bot.hybrid_command(name="ticket-logs", aliases=["ticketlogs"])
@commands.has_permissions(administrator=True)
async def ticketlogs(ctx):
    guild = ctx.guild
    gid = str(guild.id)
    try:
        category = discord.utils.get(guild.categories, name="helpdesk")
        if not category:
            category = await guild.create_category("helpdesk")

        existing = discord.utils.get(guild.text_channels, name="hunterx-ticketlogs")
        if existing:
            config_data.setdefault(gid, {})["ticketlog_channel"] = existing.id
            save_config()
            return await send_embed(ctx, "Already Exists", f"{existing.mention} already exists and is set as the ticket log channel.")

        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False),
            guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True),
        }
        for role in guild.roles:
            if role.permissions.administrator:
                overwrites[role] = discord.PermissionOverwrite(view_channel=True, send_messages=False, read_message_history=True)

        log_ch = await guild.create_text_channel(
            name="hunterx-ticketlogs",
            category=category,
            overwrites=overwrites,
            reason="HunterX Ticket Log Setup"
        )
        config_data.setdefault(gid, {})["ticketlog_channel"] = log_ch.id
        save_config()
        await send_embed(ctx, "Ticket Log Setup",
            f"{log_ch.mention} created inside *helpdesk* category.\nAll ticket opens and closes will be logged here.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== TICKET RESET ====================
@bot.hybrid_command(name="ticket-reset", aliases=["ticketreset"])
@commands.has_permissions(administrator=True)
async def ticketreset(ctx):
    guild = ctx.guild
    gid = str(guild.id)
    try:
        support_ch = discord.utils.get(guild.text_channels, name="hunterx-support")
        if not support_ch:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> Not Found",
                "No `hunterx-support` channel found. Run `$ticketsetup` first.")

        await support_ch.purge(limit=20)

        sv_name_caps = guild.name.split()[0].upper() if guild.name.split() else guild.name.upper()
        panel_embed = discord.Embed(
            description=(
                f"# {sv_name_caps}\n\n"
                f"# TICKET RULES\n\n"
                f"- 5 Hours no reply Ticket Close\n\n"
                f"- If you dont have need dont create ticket\n\n"
                f"- Troll ticket = Timeout 5hrs\n\n"
                f"- Spam ticket = Timeout 3days\n\n"
                f"1st attempt timeout 1 day\n"
                f"2nd attempt timeout 2 days\n"
                f"3rd attempt timeout 5days\n"
                f"4th attempt = Ban"
            ),
            color=0xFFFFFF
        )
        if guild.icon:
            panel_embed.set_image(url=guild.icon.url)
        panel_embed.set_footer(text="HunterX")
        await support_ch.send(embed=panel_embed, view=TicketView())

        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Ticket Panel Reset",
            f"Support panel in {support_ch.mention} has been refreshed.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== USER INFO ====================
@bot.hybrid_command(name="user-info", aliases=["userinfo", "ui"])
async def userinfo(ctx, member: discord.Member = None):
    target = member or ctx.author
    roles = [r.mention for r in target.roles if r.name != "@everyone"]
    roles_str = ", ".join(roles) if roles else "None"
    joined = target.joined_at.strftime("%B %d, %Y") if target.joined_at else "Unknown"
    created = target.created_at.strftime("%B %d, %Y")
    embed = discord.Embed(
        description=f"**User Info — {target.mention}**",
        color=target.color if target.color.value else 0xFFFFFF
    )
    embed.set_author(name=str(target), icon_url=target.display_avatar.url)
    embed.set_thumbnail(url=target.display_avatar.url)
    embed.add_field(name="Display Name", value=target.display_name, inline=True)
    embed.add_field(name="UserID", value=str(target.id), inline=True)
    embed.add_field(name="Bot", value="Yes" if target.bot else "No", inline=True)
    embed.add_field(name="Account Created", value=created, inline=True)
    embed.add_field(name="Joined Server", value=joined, inline=True)
    embed.add_field(name=f"Roles ({len(roles)})", value=roles_str[:1024] if roles_str else "None", inline=False)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== LOCK / UNLOCK ====================
@bot.hybrid_command()
@commands.has_permissions(manage_channels=True)
async def lock(ctx, channel: discord.TextChannel = None):
    channel = channel or ctx.channel
    try:
        await channel.set_permissions(ctx.guild.default_role, send_messages=False)
        embed = make_embed("<:HunteXCheck:1518422536558481448> Channel Locked", f"{channel.mention} has been locked.")
        embed.set_footer(text="HunterX")
        msg = await ctx.send(embed=embed)
        await asyncio.sleep(5)
        try:
            await msg.delete()
            await ctx.message.delete()
        except:
            pass
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

@bot.hybrid_command()
@commands.has_permissions(manage_channels=True)
async def unlock(ctx, channel: discord.TextChannel = None):
    channel = channel or ctx.channel
    try:
        await channel.set_permissions(ctx.guild.default_role, send_messages=None)
        embed = make_embed("<:HunteXCheck:1518422536558481448> Channel Unlocked", f"{channel.mention} has been unlocked.")
        embed.set_footer(text="HunterX")
        msg = await ctx.send(embed=embed)
        await asyncio.sleep(5)
        try:
            await msg.delete()
            await ctx.message.delete()
        except:
            pass
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== LOCKDOWN / UNLOCKDOWN ====================
@bot.hybrid_command()
@commands.has_permissions(administrator=True)
async def lockdown(ctx):
    count = 0
    for channel in ctx.guild.text_channels:
        try:
            await channel.set_permissions(ctx.guild.default_role, send_messages=False)
            count += 1
        except:
            pass
    embed = make_embed("<:HunteXCheck:1518422536558481448> Lockdown Active", f"Locked **{count}** channels.")
    embed.set_footer(text="HunterX")
    msg = await ctx.send(embed=embed)
    await asyncio.sleep(5)
    try:
        await msg.delete()
        await ctx.message.delete()
    except:
        pass

@bot.hybrid_command()
@commands.has_permissions(administrator=True)
async def unlockdown(ctx):
    count = 0
    for channel in ctx.guild.text_channels:
        try:
            await channel.set_permissions(ctx.guild.default_role, send_messages=None)
            count += 1
        except:
            pass
    embed = make_embed("<:HunteXCheck:1518422536558481448> Lockdown Lifted", f"Unlocked **{count}** channels.")
    embed.set_footer(text="HunterX")
    msg = await ctx.send(embed=embed)
    await asyncio.sleep(5)
    try:
        await msg.delete()
        await ctx.message.delete()
    except:
        pass

# ==================== ANTINUKE ====================
ON_OFF_CHOICES = [
    discord.app_commands.Choice(name="ON", value="on"),
    discord.app_commands.Choice(name="OFF", value="off"),
]

KEYWORD_ACTION_CHOICES = [
    discord.app_commands.Choice(name="Add Keyword", value="add"),
    discord.app_commands.Choice(name="Remove Keyword", value="remove"),
    discord.app_commands.Choice(name="View List", value="view"),
    discord.app_commands.Choice(name="Clear Keyword", value="clear"),
]

# ==================== SLASH: KEYWORDS / BADWORDS ====================
@bot.tree.command(name="add-keywords", description="Manage auto-delete keywords")
@discord.app_commands.guild_only()
@discord.app_commands.describe(operation="Keyword action", keyword="Keyword to add/remove")
@discord.app_commands.choices(operation=KEYWORD_ACTION_CHOICES)
async def slash_add_keywords(interaction: discord.Interaction, operation: str, keyword: Optional[str] = None):
    await _run_slash_command(interaction, addkeywords, operation=operation, keyword=keyword)

@bot.tree.command(name="anti-badwords", description="Enable or disable badwords auto-delete")
@discord.app_commands.guild_only()
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def slash_anti_badwords(interaction: discord.Interaction, action: str):
    await _run_slash_command(interaction, antibadwords, action=action)

# /badwords <add|remove|list|clear>
badwords_group = discord.app_commands.Group(name="badwords", description="Manage badwords list (auto delete)")

@badwords_group.command(name="add", description="Add a badword")
@discord.app_commands.guild_only()
@discord.app_commands.describe(word="Badword to add")
async def slash_badwords_add(interaction: discord.Interaction, word: str):
    await _run_slash_command(interaction, badwords_add, word=word)

@badwords_group.command(name="remove", description="Remove a badword")
@discord.app_commands.guild_only()
@discord.app_commands.describe(word="Badword to remove")
async def slash_badwords_remove(interaction: discord.Interaction, word: str):
    await _run_slash_command(interaction, badwords_remove, word=word)

@badwords_group.command(name="list", description="Show the badwords list")
@discord.app_commands.guild_only()
async def slash_badwords_list(interaction: discord.Interaction):
    await _run_slash_command(interaction, badwords_list)

@badwords_group.command(name="clear", description="Clear the badwords list")
@discord.app_commands.guild_only()
async def slash_badwords_clear(interaction: discord.Interaction):
    await _run_slash_command(interaction, badwords_clear)

try:
    bot.tree.add_command(badwords_group)
except Exception:
    # Avoid crashing if the group is already registered (e.g., hot reload)
    pass

@bot.hybrid_command(name="anti-nuke", aliases=["antinuke"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antinuke(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antinuke"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiNuke Enabled", "Anti Nuke protection is now **enabled**.")
    elif is_toggle_off(action):
        config_data[gid]["antinuke"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiNuke Disabled", "Anti Nuke protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antinuke enable/disable`")

@bot.hybrid_command(name="anti-kick", aliases=["antikick"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antikick(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antikick"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiKick Enabled", "Unauthorized kickers will be **banned**.")
    elif is_toggle_off(action):
        config_data[gid]["antikick"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiKick Disabled", "AntiKick protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antikick enable/disable`")

@bot.hybrid_command(name="anti-ban", aliases=["antiban"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antiban(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antiban"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiBan Enabled", "Unauthorized banners will be **banned**.")
    elif is_toggle_off(action):
        config_data[gid]["antiban"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiBan Disabled", "Anti Ban protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antiban enable/disable`")

@bot.hybrid_command(name="anti-bot", aliases=["antibotadd"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antibotadd(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antibotadd"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiBotAdd Enabled", "Unauthorized bot additions will result in a **ban**.")
    elif is_toggle_off(action):
        config_data[gid]["antibotadd"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiBotAdd Disabled", "AntiBot Add protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antibotadd enable/disable`")

@bot.hybrid_command(name="anti-create-role", aliases=["anticrtadminrole", "anticreateadminrole"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def anticrtadminrole(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["anticreateadminrole"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiCrtAdminRole Enabled", "Creating an admin role without whitelist will result in a **ban**.")
    elif is_toggle_off(action):
        config_data[gid]["anticreateadminrole"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiCrtAdminRole Disabled", "Protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$anticrtadminrole enable/disable`")

@bot.hybrid_command(name="anti-create-webhook", aliases=["anticrtwebhook", "antiwebhookcreate"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def anticrtwebhook(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antiwebhookcreate"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiCrtWebhook Enabled", "Unauthorized webhook creation will result in a **ban**.")
    elif is_toggle_off(action):
        config_data[gid]["antiwebhookcreate"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiCrtWebhook Disabled", "Protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$anticrtwebhook enable/disable`")

@bot.hybrid_command(name="anti-create-channel", aliases=["anticrtchannel", "anticreatechannel"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def anticrtchannel(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["anticreatechannel"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiCrtChannel Enabled", "Unauthorized channel creation will be blocked and deleted.")
    elif is_toggle_off(action):
        config_data[gid]["anticreatechannel"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiCrtChannel Disabled", "Protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$anticrtchannel enable/disable`")

@bot.hybrid_command(name="anti-delete-channel", aliases=["antidelchannel", "antideletechannel"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antidelchannel(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antideletechannel"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiDeleteChannel Enabled", "Unauthorized channel deletion will be logged and the user banned.")
    elif is_toggle_off(action):
        config_data[gid]["antideletechannel"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiDeleteChannel Disabled", "Protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antidelchannel enable/disable`")

# ==================== ANTI DELETE ROLE ====================
@bot.hybrid_command(name="anti-delete-role", aliases=["antidelrole", "antideleterole"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antidelrole(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antideleterole"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiDeleteRole Enabled", "Deleting a role without whitelist will result in a **ban**.")
    elif is_toggle_off(action):
        config_data[gid]["antideleterole"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiDeleteRole Disabled", "Protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antidelrole enable/disable`")

# ==================== ENABLE/DISABLE ALL SECURITY ====================
SECURITY_KEYS = [
    "antinuke",
    "antikick",
    "antiban",
    "antibotadd",
    "anticreateadminrole",
    "antiwebhookcreate",
    "anticreatechannel",
    "antideletechannel",
    "antideleterole",
    "antilink",
    "antispam",
]

SECURITY_LABELS = {
    "antinuke": "Antinuke",
    "antikick": "Antikick",
    "antiban": "Antiban",
    "antibotadd": "Antiaddbot",
    "anticreateadminrole": "Anticreaterole",
    "antiwebhookcreate": "Anticreatewebhook",
    "anticreatechannel": "Anticreatechannel",
    "antideletechannel": "Antideletechannel",
    "antideleterole": "Antideleterole",
    "antilink": "Antilink",
    "antispam": "Antispam",
}

@bot.hybrid_command(name="securityall", aliases=["enableall", "antinukeall"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def securityall(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        for key in SECURITY_KEYS:
            config_data[gid][key] = True
        save_config()
        await send_embed(ctx, "All Security Enabled",
            "All Anti Nuke protections are now **enabled**:\n\n" +
            "\n".join(f"`{SECURITY_LABELS[key]}`" for key in SECURITY_KEYS))
    elif is_toggle_off(action):
        for key in SECURITY_KEYS:
            config_data[gid][key] = False
        save_config()
        await send_embed(ctx, "All Security Disabled",
            "All Anti Nuke protections are now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$securityall enable/disable`")

# ==================== WARNS SYSTEM ====================
@bot.hybrid_command(name="warn")
@commands.has_permissions(manage_messages=True)
async def warn(ctx, member: discord.Member, *, reason: str = "No reason provided"):
    if member.bot:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid", "You cannot warn a bot.")
    if member.id == ctx.guild.owner_id:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid", "You cannot warn the server owner.")
    gid = str(ctx.guild.id)
    uid = str(member.id)
    warns_data.setdefault(gid, {}).setdefault(uid, [])
    warns_data[gid][uid].append({"reason": reason, "mod": str(ctx.author), "time": datetime.now(PH_TIME).strftime("%Y-%m-%d %H:%M")})
    save_warns()
    count = len(warns_data[gid][uid])

    action_taken = ""
    try:
        if count == 1:
            until = discord.utils.utcnow() + timedelta(hours=1)
            await member.timeout(until, reason=f"HunterX Warn #{count}: {reason}")
            action_taken = "\n**Auto Action:** Muted for 1 hour"
        elif count == 2:
            await member.kick(reason=f"HunterX Warn #{count}: {reason}")
            action_taken = "\n**Auto Action:** Kicked from server"
        elif count >= 3:
            await member.ban(reason=f"HunterX Warn #{count}: {reason}")
            action_taken = "\n**Auto Action:** Banned from server"
    except Exception as e:
        action_taken = f"\n**Auto Action failed:** {e}"

    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> User Warned",
        f"**User:** {member.mention}\n**Warns:** `{count}`\n**Reason:** {reason}{action_taken}")
    await log_action(ctx.guild, "<:HunterXwarn:1515422027521986720> Member Warned",
        f"**User:** {member} (`{member.id}`)\n**Warns:** `{count}`\n**Reason:** {reason}\n**Mod:** {ctx.author}",
        member.display_avatar.url)
    await log_mod_action(ctx.guild, "<:HunterXwarn:1515422027521986720> Member Warned",
        f"**User:** {member} (`{member.id}`)\n**Warns:** `{count}`\n**Reason:** {reason}\n**Auto Action:** {action_taken.strip() if action_taken else 'None'}\n**Mod:** {ctx.author}",
        member.display_avatar.url)

@bot.hybrid_command(name="checkwarns")
@commands.has_permissions(manage_messages=True)
async def checkwarns(ctx, member: discord.Member):
    gid = str(ctx.guild.id)
    uid = str(member.id)
    user_warns = warns_data.get(gid, {}).get(uid, [])
    if not user_warns:
        return await send_embed(ctx, "No Warns", f"{member.mention} has no warnings.")
    lines = []
    for i, w in enumerate(user_warns, 1):
        lines.append(f"**{i}.** {w['reason']} — by {w['mod']} ({w['time']})")
    await send_embed(ctx, f"<:HunterXwarn:1515422027521986720> Warns for {member.display_name} ({len(user_warns)})",
        "\n".join(lines))

@bot.hybrid_command(name="clearwarns")
@commands.has_permissions(manage_messages=True)
async def clearwarns(ctx, member: discord.Member):
    gid = str(ctx.guild.id)
    uid = str(member.id)
    if not warns_data.get(gid, {}).get(uid):
        return await send_embed(ctx, "No Warns", f"{member.mention} has no warnings to clear.")
    warns_data[gid][uid] = []
    save_warns()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Warns Cleared",
        f"All warnings for {member.mention} have been cleared.")

@bot.hybrid_command(name="unwarn")
@commands.has_permissions(manage_messages=True)
async def unwarn(ctx, member: discord.Member):
    gid = str(ctx.guild.id)
    uid = str(member.id)
    user_warns = warns_data.get(gid, {}).get(uid, [])
    if not user_warns:
        return await send_embed(ctx, "No Warns", f"{member.mention} has no warnings to remove.")
    removed = user_warns.pop()
    warns_data[gid][uid] = user_warns
    save_warns()
    remaining = len(user_warns)
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Warn Removed",
        f"Removed last warning for {member.mention}.\n**Removed reason:** {removed['reason']}\n**Remaining warns:** `{remaining}`")

# ==================== ALLOWED LINK ====================
@bot.hybrid_command(name="allowed-link", aliases=["allowedlink"])
@commands.has_permissions(administrator=True)
async def allowedlink(ctx, user: discord.Member = None, role: discord.Role = None):
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if "allowed_link_users" not in config_data[gid]:
        config_data[gid]["allowed_link_users"] = []
    if "allowed_link_roles" not in config_data[gid]:
        config_data[gid]["allowed_link_roles"] = []

    users = []
    roles = []
    if user:
        users.append(user)
    if role:
        roles.append(role)

    message = getattr(ctx, "message", None)
    if message:
        for mentioned_user in message.mentions:
            if mentioned_user.id not in {u.id for u in users}:
                users.append(mentioned_user)
        for mentioned_role in message.role_mentions:
            if mentioned_role.id not in {r.id for r in roles}:
                roles.append(mentioned_role)

    if not users and not roles:
        return await send_embed(ctx, "Usage", "`$allowedlink @user or @role` — mention a user or role")

    added = []
    for target_user in users:
        uid = target_user.id
        if uid not in config_data[gid]["allowed_link_users"]:
            config_data[gid]["allowed_link_users"].append(uid)
        added.append(target_user.mention)
    for target_role in roles:
        rid = target_role.id
        if rid not in config_data[gid]["allowed_link_roles"]:
            config_data[gid]["allowed_link_roles"].append(rid)
        added.append(target_role.mention)

    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Allowed Link Updated",
        "These can now send Discord invite links:\n" + "\n".join(added))

# ==================== ALLOWED LINK BOARD ====================
@bot.hybrid_command(name="allowed-linkboard", aliases=["allowedlinkboard"])
@commands.has_permissions(administrator=True)
async def allowedlinkboard(ctx):
    gid = str(ctx.guild.id)
    cfg = config_data.get(gid, {})
    user_ids = cfg.get("allowed_link_users", [])
    role_ids = cfg.get("allowed_link_roles", [])
    legacy_role_id = cfg.get("allowed_link_role")
    if legacy_role_id and legacy_role_id not in role_ids:
        role_ids = [legacy_role_id] + role_ids

    lines = []
    for uid in user_ids:
        member = ctx.guild.get_member(uid)
        lines.append(f"{member.mention if member else f'<@{uid}>'}")
    for rid in role_ids:
        role = ctx.guild.get_role(rid)
        lines.append(f"{role.mention if role else f'<@&{rid}>'}")

    if not lines:
        return await send_embed(ctx, "<:249630gradientgear:1512782705522245712> Allowed Link Board", "No users or roles are allowed to send links.")

    embed = make_embed("<:249630gradientgear:1512782705522245712> Allowed Link Board", "\n".join(lines))
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== ANTI LINK ====================
@bot.hybrid_command(name="anti-link", aliases=["antilink"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antilink(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antilink"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiLink Enabled", "Discord invite links will be auto deleted.")
    elif is_toggle_off(action):
        config_data[gid]["antilink"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiLink Disabled", "Discord invite links will NOT be deleted.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antilink enable/disable`")

# ==================== ANTI SPAM ====================
@bot.hybrid_command(name="anti-spam", aliases=["antispam"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antispam(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antispam"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiSpam Enabled", "Members who spam 10+ messages rapidly will be timed out for 5 minutes.")
    elif is_toggle_off(action):
        config_data[gid]["antispam"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiSpam Disabled", "AntiSpam protection is now **disabled**.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antispam enable/disable`")

# ==================== KEYWORD FILTER ====================
@bot.hybrid_command(name="add-keywords", aliases=["addkeywords", "keywords"])
@commands.has_permissions(administrator=True)
@discord.app_commands.describe(operation="Select keyword action", keyword="Keyword to add or remove")
@discord.app_commands.choices(operation=KEYWORD_ACTION_CHOICES)
async def addkeywords(ctx, operation: str, *, keyword: str = None):
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    keywords = config_data[gid].setdefault("keywords", [])
    operation = (operation or "").lower().strip()
    normalized = (keyword or "").strip().lower()

    if operation == "add":
        if not normalized:
            return await send_embed(ctx, "Usage", "`$add-keywords add <keyword>`")
        if normalized in keywords:
            return await send_embed(ctx, "Already Added", f"`{normalized}` is already in the keyword list.")
        keywords.append(normalized)
        keywords.sort()
        save_config()
        return await send_embed(ctx, "Keyword Added", f"`{normalized}` will now be auto-deleted when detected.")

    if operation == "remove":
        if not normalized:
            return await send_embed(ctx, "Usage", "`$add-keywords remove <keyword>`")
        if normalized not in keywords:
            return await send_embed(ctx, "Not Found", f"`{normalized}` is not in the keyword list.")
        keywords.remove(normalized)
        save_config()
        return await send_embed(ctx, "Keyword Removed", f"`{normalized}` has been removed from the keyword list.")

    if operation == "view":
        if not keywords:
            return await send_embed(ctx, "Keyword List", "No keywords are currently set.")
        return await send_embed(ctx, "Keyword List", "\n".join(f"• `{word}`" for word in keywords))

    if operation == "clear":
        config_data[gid]["keywords"] = []
        save_config()
        return await send_embed(ctx, "Keyword List Cleared", "All auto-delete keywords have been removed.")

    await send_embed(ctx, "Invalid", "Choose one of these actions: `add`, `remove`, `view`, `clear`.")

# ==================== ANTI BADWORDS ====================
@bot.hybrid_command(name="anti-badwords", aliases=["antibadwords"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antibadwords(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antibadwords"] = True
        config_data[gid].setdefault("badwords", [])
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiBadwords Enabled", "Badwords will be auto deleted.")
    elif is_toggle_off(action):
        config_data[gid]["antibadwords"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiBadwords Disabled", "Badwords will NOT be deleted.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antibadwords enable/disable`")

@bot.hybrid_group(name="badwords", invoke_without_command=True)
@commands.has_permissions(administrator=True)
async def badwords(ctx):
    await send_embed(ctx, "Badwords", "Usage:\n`badwords add <word>`\n`badwords remove <word>`\n`badwords list`\n`badwords clear`")

@badwords.command(name="add")
@commands.has_permissions(administrator=True)
async def badwords_add(ctx, *, word: str):
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    words = cfg.setdefault("badwords", [])
    w = (word or "").strip().lower()
    if not w:
        return await send_embed(ctx, "Usage", "`badwords add <word>`")
    if w in words:
        return await send_embed(ctx, "Already Added", f"`{w}` is already in the badwords list.")
    words.append(w)
    words.sort()
    cfg["antibadwords"] = True
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Badword Added", f"`{w}` added. Auto-delete is now enabled.")

@badwords.command(name="remove", aliases=["rm", "del"])
@commands.has_permissions(administrator=True)
async def badwords_remove(ctx, *, word: str):
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    words = cfg.setdefault("badwords", [])
    w = (word or "").strip().lower()
    if not w:
        return await send_embed(ctx, "Usage", "`badwords remove <word>`")
    if w not in words:
        return await send_embed(ctx, "Not Found", f"`{w}` is not in the badwords list.")
    words.remove(w)
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Badword Removed", f"`{w}` removed.")

@badwords.command(name="list")
@commands.has_permissions(administrator=True)
async def badwords_list(ctx):
    gid = str(ctx.guild.id)
    cfg = config_data.get(gid, {})
    words = cfg.get("badwords", [])
    if not words:
        return await send_embed(ctx, "Badwords List", "Empty.")
    await send_embed(ctx, "Badwords List", "\n".join(f"• `{w}`" for w in words[:80]))

@badwords.command(name="clear")
@commands.has_permissions(administrator=True)
async def badwords_clear(ctx):
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    cfg["badwords"] = []
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Badwords Cleared", "All badwords removed.")

# ==================== ANTI NICKNAME ====================
@bot.hybrid_command(name="anti-nickname", aliases=["antinickname", "antinick"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antinickname(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antinickname"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiNickname Enabled", "Bad nicknames (links/invisible/too many emojis) will be auto-reset.")
    elif is_toggle_off(action):
        config_data[gid]["antinickname"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiNickname Disabled", "Nickname protection is now disabled.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antinickname enable/disable`")

# ==================== PARTNERSHIP POST CHANNEL ====================
@bot.hybrid_command(name="pspost", aliases=["partnership-post", "post-channel"])
@commands.has_permissions(administrator=True)
async def pspost(ctx, channel: discord.TextChannel = None):
    """Set channel where `.req` posts will be sent."""
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    if channel is None:
        ch_id = cfg.get("partnership_post_channel")
        ch = ctx.guild.get_channel(int(ch_id)) if ch_id else None
        return await send_embed(ctx, "Partnership Post Channel", f"Current: {ch.mention if ch else 'Not Set'}\nSet: `$pspost #channel`")
    cfg["partnership_post_channel"] = channel.id
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Partnership Channel Set", f"Requests will post in {channel.mention}.")

# ==================== MUSIC AUTOREACT TOGGLE ====================
@bot.hybrid_command(name="autoreact", aliases=["musicreact", "auto-react"])
@commands.has_permissions(administrator=True)
async def autoreact(ctx, action: str = None):
    """Toggle auto-reactions for music controls."""
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    if action is None:
        status = "ON" if cfg.get("music_autoreact", True) else "OFF"
        return await send_embed(ctx, "Music AutoReact", f"Status: `{status}`\nUsage: `autoreact enable/disable`")
    if is_toggle_on(action):
        cfg["music_autoreact"] = True
        save_config()
        return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Music AutoReact", "Enabled.")
    if is_toggle_off(action):
        cfg["music_autoreact"] = False
        save_config()
        return await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> Music AutoReact", "Disabled.")
    await send_embed(ctx, "Invalid", "Usage: `autoreact enable/disable`")

# ==================== LEVELING SYSTEM ====================
@bot.hybrid_group(name="leveling", invoke_without_command=True)
@commands.has_permissions(administrator=True)
async def leveling(ctx, action: str = None):
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    enabled = bool(cfg.get("leveling_enabled", False))
    if action is None:
        status = "ON" if enabled else "OFF"
        cooldown = int(cfg.get("leveling_cooldown", 60))
        ch_id = cfg.get("leveling_channel")
        ch = ctx.guild.get_channel(int(ch_id)) if ch_id else None
        return await send_embed(ctx, "Leveling",
            f"Status: `{status}`\nCooldown: `{cooldown}s`\nAnnounce: {ch.mention if ch else 'Current channel'}\n\n"
            f"Usage:\n`leveling enable/disable`\n`leveling cooldown <seconds>`\n`leveling channel #channel`")
    if is_toggle_on(action):
        cfg["leveling_enabled"] = True
        cfg.setdefault("leveling_cooldown", 60)
        save_config()
        return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Leveling", "Enabled.")
    if is_toggle_off(action):
        cfg["leveling_enabled"] = False
        save_config()
        return await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> Leveling", "Disabled.")
    await send_embed(ctx, "Invalid", "Usage: `leveling enable/disable`")

@leveling.command(name="cooldown")
@commands.has_permissions(administrator=True)
async def leveling_cooldown(ctx, seconds: int = 60):
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    cfg["leveling_cooldown"] = max(5, int(seconds))
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Leveling Cooldown", f"Cooldown set to `{cfg['leveling_cooldown']}s`.")

@leveling.command(name="channel")
@commands.has_permissions(administrator=True)
async def leveling_channel(ctx, channel: discord.TextChannel = None):
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    if channel is None:
        cfg.pop("leveling_channel", None)
        save_config()
        return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Leveling Channel", "Announce channel reset to current channel.")
    cfg["leveling_channel"] = channel.id
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Leveling Channel", f"Announce channel set to {channel.mention}.")

@bot.hybrid_command(name="rank", aliases=["level-rank"])
async def rank(ctx, member: discord.Member = None):
    target = member or ctx.author
    data = get_user_level(ctx.guild.id, target.id)
    xp = int(data.get("xp", 0))
    lvl = int(data.get("level", 0))
    await send_embed(ctx, "Rank", f"User: {target.mention}\nLevel: `{lvl}`\nXP: `{xp}`")

@bot.hybrid_command(name="leveltop", aliases=["levelboard", "toplevels", "level-top", "level-leaderboard"])
async def leveltop(ctx):
    gid = str(ctx.guild.id)
    users = levels_data.get(gid, {})
    if not users:
        return await send_embed(ctx, "Level Leaderboard", "No data yet.")
    ranked = sorted(users.items(), key=lambda kv: int(kv[1].get("xp", 0)), reverse=True)[:10]
    lines = []
    for i, (uid, data) in enumerate(ranked, 1):
        member = ctx.guild.get_member(int(uid))
        name = member.mention if member else f"<@{uid}>"
        lines.append(f"**{i}.** {name} — Lvl `{int(data.get('level', 0))}` | XP `{int(data.get('xp', 0))}`")
    await send_embed(ctx, "Level Leaderboard", "\n".join(lines))

# ==================== DELETE MSG LOG COMMAND ====================
@bot.hybrid_command(name="delete-msg-log", aliases=["deletemsglog", "dml"])
@commands.has_permissions(administrator=True)
async def deletemsglog(ctx):
    guild = ctx.guild
    gid = str(guild.id)
    try:
        existing = discord.utils.get(guild.text_channels, name="hunterx-dltmsglogs")
        if existing:
            config_data.setdefault(gid, {})["deletemsg_channel"] = existing.id
            save_config()
            return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Already Exists",
                f"{existing.mention} is already set as the deleted message log channel.")
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False),
            guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True),
        }
        for role in guild.roles:
            if role.permissions.administrator:
                overwrites[role] = discord.PermissionOverwrite(view_channel=True, send_messages=False, read_message_history=True)
        log_ch = await guild.create_text_channel(
            name="hunterx-dltmsglogs",
            overwrites=overwrites,
            reason="HunterX Delete Message Log Setup"
        )
        config_data.setdefault(gid, {})["deletemsg_channel"] = log_ch.id
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Delete Log Setup",
            f"{log_ch.mention} created. Deleted messages will be logged here (admin-only).")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== EDIT MSG LOG COMMAND ====================
@bot.hybrid_command(name="edit-msg-log", aliases=["editmsglogs", "eml"])
@commands.has_permissions(administrator=True)
async def editmsglogs(ctx):
    guild = ctx.guild
    gid = str(guild.id)
    try:
        existing = discord.utils.get(guild.text_channels, name="hunterx-editmsglogs")
        if existing:
            config_data.setdefault(gid, {})["editmsg_channel"] = existing.id
            save_config()
            return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Already Exists",
                f"{existing.mention} is already set as the edited message log channel.")
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False),
            guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True),
        }
        for role in guild.roles:
            if role.permissions.administrator:
                overwrites[role] = discord.PermissionOverwrite(view_channel=True, send_messages=False, read_message_history=True)
        log_ch = await guild.create_text_channel(
            name="hunterx-editmsglogs",
            overwrites=overwrites,
            reason="HunterX Edit Message Log Setup"
        )
        config_data.setdefault(gid, {})["editmsg_channel"] = log_ch.id
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Edit Log Setup",
            f"{log_ch.mention} created. Edited messages will be logged here (admin-only).")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== MOD LOGS COMMAND ====================
@bot.hybrid_command()
@commands.has_permissions(administrator=True)
async def modlogs(ctx):
    guild = ctx.guild
    gid = str(guild.id)
    try:
        existing = discord.utils.get(guild.text_channels, name="hunterx-modlogs")
        if existing:
            config_data.setdefault(gid, {})["modlogs_channel"] = existing.id
            save_config()
            return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Already Exists",
                f"{existing.mention} is already set as the mod logs channel.")
        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False),
            guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True),
        }
        for role in guild.roles:
            if role.permissions.administrator:
                overwrites[role] = discord.PermissionOverwrite(view_channel=True, send_messages=False, read_message_history=True)
        log_ch = await guild.create_text_channel(
            name="hunterx-modlogs",
            overwrites=overwrites,
            reason="HunterX Mod Logs Setup"
        )
        config_data.setdefault(gid, {})["modlogs_channel"] = log_ch.id
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Mod Logs Setup",
            f"{log_ch.mention} created. All mod actions (ban, kick, timeout, warn) will be logged here (admin-only).")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== CONFIG ====================
@bot.hybrid_command(name="config")
async def config_cmd(ctx):
    gid = str(ctx.guild.id)
    cfg = config_data.get(gid, {})

    def status(val):
        return "ON" if val else "OFF"

    log_ch = ctx.guild.get_channel(log_channels.get(gid, 0))
    welcome_ch_id = cfg.get("welcome_channel")
    welcome_ch = ctx.guild.get_channel(welcome_ch_id) if welcome_ch_id else None
    autorole_id = cfg.get("autorole")
    autorole = ctx.guild.get_role(autorole_id) if autorole_id else None
    deletemsg_ch_id = cfg.get("deletemsg_channel")
    deletemsg_ch = ctx.guild.get_channel(deletemsg_ch_id) if deletemsg_ch_id else None
    boost_ch_id = cfg.get("boost_channel")
    boost_ch = ctx.guild.get_channel(boost_ch_id) if boost_ch_id else None

    editmsg_ch_id = cfg.get("editmsg_channel")
    editmsg_ch = ctx.guild.get_channel(editmsg_ch_id) if editmsg_ch_id else None
    modlogs_ch_id = cfg.get("modlogs_channel")
    modlogs_ch = ctx.guild.get_channel(modlogs_ch_id) if modlogs_ch_id else None
    all_logs_id = cfg.get("all_logs_channel")
    all_logs_ch = ctx.guild.get_channel(all_logs_id) if all_logs_id else None
    vclogs_id = cfg.get("vclogs_channel")
    vclogs_ch = ctx.guild.get_channel(vclogs_id) if vclogs_id else None
    goodbye_id = cfg.get("goodbye_channel")
    goodbye_ch = ctx.guild.get_channel(goodbye_id) if goodbye_id else None
    not_set = "Not Set"
    embed = make_embed(
        "Server Configuration",
        f"**Anti-Nuke Settings**\n"
        f"Anti Nuke: `{status(cfg.get('antinuke', False))}`\n"
        f"Anti Kick: `{status(cfg.get('antikick', False))}`\n"
        f"Anti Ban: `{status(cfg.get('antiban', False))}`\n"
        f"Anti Bot Add: `{status(cfg.get('antibotadd', False))}`\n"
        f"Anti Admin Role: `{status(cfg.get('anticreateadminrole', False))}`\n"
        f"Anti Webhook: `{status(cfg.get('antiwebhookcreate', False))}`\n"
        f"Anti Link: `{status(cfg.get('antilink', False))}`\n"
        f"Anti Spam: `{status(cfg.get('antispam', False))}`\n"
        f"Anti Mass Mention: `{status(cfg.get('antimassmention', False))}`\n"
        f"Anti Mass Raid: `{status(cfg.get('antimassraid', False))}`\n"
        f"Anti Create Channel: `{status(cfg.get('anticreatechannel', False))}`\n"
        f"Anti Delete Channel: `{status(cfg.get('antideletechannel', False))}`\n"
        f"Anti Delete Role: `{status(cfg.get('antideleterole', False))}`\n\n"
        f"**Channels**\n"
        f"Log: {log_ch.mention if log_ch else not_set}\n"
        f"Welcome: {welcome_ch.mention if welcome_ch else not_set}\n"
        f"Delete Msg Log: {deletemsg_ch.mention if deletemsg_ch else not_set}\n"
        f"Edit Msg Log: {editmsg_ch.mention if editmsg_ch else not_set}\n"
        f"Mod Logs: {modlogs_ch.mention if modlogs_ch else not_set}\n"
        f"VC Logs: {vclogs_ch.mention if vclogs_ch else not_set}\n"
        f"All Logs: {all_logs_ch.mention if all_logs_ch else not_set}\n"
        f"Goodbye: {goodbye_ch.mention if goodbye_ch else not_set}\n"
        f"Boost Channel: {boost_ch.mention if boost_ch else not_set}\n\n"
        f"**Other**\n"
        f"Auto Role: {autorole.mention if autorole else not_set}\n"
        f"Prefix: `{cfg.get('prefix', '$')}`"
    )
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== ALL LOGS ====================
@bot.hybrid_command(name="all-logs", aliases=["alllogs", "logs-all"])
@commands.has_permissions(administrator=True)
async def all_logs(ctx):
    guild = ctx.guild
    gid = str(guild.id)
    try:
        existing = discord.utils.get(guild.text_channels, name="all-logs")
        if existing:
            config_data.setdefault(gid, {})["all_logs_channel"] = existing.id
            save_config()
            return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> All Logs",
                f"{existing.mention} is set as **All Logs**.")

        overwrites = await _ensure_admin_overwrites(guild, view_for_everyone=False)
        ch = await guild.create_text_channel(name="all-logs", overwrites=overwrites, reason="HunterX All Logs Setup")
        config_data.setdefault(gid, {})["all_logs_channel"] = ch.id
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> All Logs",
            f"{ch.mention} created. All logs will be mirrored here (admin-only).")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== VC LOGS ====================
@bot.hybrid_command(name="vclogs", aliases=["voice-logs", "vc-logs"])
@commands.has_permissions(administrator=True)
async def vclogs(ctx):
    guild = ctx.guild
    gid = str(guild.id)
    try:
        existing = discord.utils.get(guild.text_channels, name="vclogs")
        if existing:
            config_data.setdefault(gid, {})["vclogs_channel"] = existing.id
            save_config()
            return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> VC Logs",
                f"{existing.mention} is set as **VC Logs**.")

        overwrites = await _ensure_admin_overwrites(guild, view_for_everyone=False)
        ch = await guild.create_text_channel(name="vclogs", overwrites=overwrites, reason="HunterX VC Logs Setup")
        config_data.setdefault(gid, {})["vclogs_channel"] = ch.id
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> VC Logs",
            f"{ch.mention} created. All voice logs will be sent here (admin-only).")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== GOODBYE SETUP ====================
@bot.hybrid_command(name="setup-goodbye", aliases=["setupgoodbye", "goodbye-setup"])
@commands.has_permissions(administrator=True)
async def setup_goodbye(ctx):
    guild = ctx.guild
    gid = str(guild.id)
    try:
        existing = discord.utils.get(guild.text_channels, name="goodbye")
        if existing:
            config_data.setdefault(gid, {})["goodbye_channel"] = existing.id
            config_data[gid].setdefault("goodbye_message", "Babye see u soon")
            config_data[gid].setdefault("leave_bg", "")
            save_config()
            return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Goodbye Setup",
                f"{existing.mention} is set as the goodbye channel.")

        overwrites = {
            guild.default_role: discord.PermissionOverwrite(view_channel=True, send_messages=False, read_message_history=True),
            guild.me: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True, manage_channels=True),
        }
        for role in guild.roles:
            if role.permissions.administrator:
                overwrites[role] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True, manage_channels=True)

        ch = await guild.create_text_channel(name="goodbye", overwrites=overwrites, reason="HunterX Goodbye Setup")
        config_data.setdefault(gid, {})["goodbye_channel"] = ch.id
        config_data[gid].setdefault("goodbye_message", "Babye see u soon")
        config_data[gid].setdefault("leave_bg", "")
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Goodbye Setup",
            f"{ch.mention} created. Leave logs will be sent there.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

@bot.hybrid_command(name="goodbye", aliases=["goodbye-message"])
@commands.has_permissions(administrator=True)
async def goodbye_msg(ctx, *, message: str = None):
    """Set the goodbye message. Use `off` to remove the embed message."""
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    if message is None:
        current = (cfg.get("goodbye_message") or "").strip()
        if not current:
            return await send_embed(ctx, "Goodbye Message", "No goodbye message is currently set.")
        return await send_embed(ctx, "Goodbye Message", f"Current:\n{current}")
    msg = message.strip()
    if msg.lower() in {"off", "disable", "none", "remove"}:
        cfg["goodbye_message"] = ""
        save_config()
        return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Goodbye Message", "The goodbye message has been removed.")
    cfg["goodbye_message"] = msg
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Goodbye Message", "Goodbye message updated.")

@bot.hybrid_command(name="setleavebg", aliases=["set-leave-bg", "leave-bg"])
@commands.has_permissions(administrator=True)
async def setleavebg(ctx, url: str = None, image: discord.Attachment = None):
    """Set the leave background image. Attach an image or pass a URL. Use `reset` for the default black background."""
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    if url and url.lower() in {"reset", "default", "black"}:
        cfg["leave_bg"] = ""
        save_config()
        return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Leave BG", "Leave background reset to black.")

    bg_url = None
    att = image or _ctx_attachment(ctx, "image")
    if att and any(att.filename.lower().endswith(ext) for ext in [".png", ".jpg", ".jpeg", ".gif", ".webp"]):
        bg_url = att.url
    if not bg_url and url:
        bg_url = url.strip()

    if not bg_url:
        return await send_embed(ctx, "Usage", "`$setleavebg <image_url>` or attach an image.\n`$setleavebg reset`")

    cfg["leave_bg"] = bg_url
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Leave BG", "Leave background updated.")

# ==================== ANTI MASS MENTION / RAID ====================
@bot.hybrid_command(name="anti-mass-mention", aliases=["antimassmention"])
@discord.app_commands.describe(action="ON or OFF", threshold="How many mentions before delete (default 6)")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antimassmention(ctx, action: str, threshold: int = 6):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antimassmention"] = True
        config_data[gid]["mass_mention_threshold"] = max(2, int(threshold))
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Anti-Mass Mention",
            f"Enabled. Threshold: `{config_data[gid]['mass_mention_threshold']}` mentions.")
    elif is_toggle_off(action):
        config_data[gid]["antimassmention"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> Anti-Mass Mention", "Disabled.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$anti-mass-mention on/off [threshold]`")

@bot.hybrid_command(name="anti-mass-raid", aliases=["antimassraid"])
@discord.app_commands.describe(action="ON or OFF")
@discord.app_commands.choices(action=ON_OFF_CHOICES)
async def antimassraid(ctx, action: str):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})
    if is_toggle_on(action):
        config_data[gid]["antimassraid"] = True
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Anti-Mass Raid",
            "Enabled. Trigger: `>15 joins / 5 seconds` → kick latest 5.")
    elif is_toggle_off(action):
        config_data[gid]["antimassraid"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> Anti-Mass Raid", "Disabled.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$anti-mass-raid on/off`")

# ==================== WHOIS + HIDE/UNHIDE ====================
@bot.hybrid_command(name="whois", aliases=["who-is"])
async def whois(ctx, member: discord.Member = None):
    # Reuse existing userinfo output
    return await userinfo(ctx, member)

@bot.hybrid_command(name="hide")
@commands.has_permissions(manage_channels=True)
async def hide(ctx, channel: discord.TextChannel = None):
    if channel is None:
        return await send_embed(ctx, "Usage", "`$hide #channel`")
    try:
        await channel.set_permissions(ctx.guild.default_role, view_channel=False)
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Hidden", f"{channel.mention} hidden.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

@bot.hybrid_command(name="unhide")
@commands.has_permissions(manage_channels=True)
async def unhide(ctx, channel: discord.TextChannel = None):
    if channel is None:
        return await send_embed(ctx, "Usage", "`$unhide #channel`")
    try:
        await channel.set_permissions(ctx.guild.default_role, view_channel=True)
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Unhidden", f"{channel.mention} unhidden.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== SETUP SERVER (setupsv) ====================
SETUPSV_TEXT_TEMPLATES = {
    "roblox_gang": {
        "IMPORTANT": ["news", "rules", "promotion", "role-requests"],
        "GENERAL": ["gang-chat", "media", "reppin", "community"],
        "HR (IMPORTANT)": ["hr-news", "hr-rules", "proof-blood-in", "proof-blood-out"],
        "HR (GENERAL)": ["hr-chat", "hr-media", "blood-in", "blood-out"],
    },
    "roleplay": {
        "CITY INFORMATION": ["Announcement", "City Update", "City Information", "Guide"],
        "CITY RULES": ["General rules", "Greenzone rules", "Redzone rules", "Hostage rules", "Kid rules",
                       "Traphouse rules", "Pd rules", "Gang rules", "Sanction"],
        "GENERAL": ["Main", "Media", "Suggestions", "Highlights"],
        "APPLICATION": ["Admin application", "Police application", "Job application", "Streamer application", "Gang application"],
        "GANG IMPORTANT": ["Gang announcement", "Gang Sanction", "Gang Folder", "Gang points", "Sanction list", "Role-Unrole-requests"],
    },
    "packing": {
        "@ANC": ["psa", "hos", "law", "fame"],
        "@BOOSTER": ["luv", "perks"],
        "@CLIPS": ["dead", "polls", "batte . clips"],
        "@HOME": ["wrld", "cmds", "socmed", "memories", "random"],
    },
}

SETUPSV_VOICE_TEMPLATES = {
    "roblox_voice": {
        "Vc": [("Meeting", False), ("Tambayan", False), ("Radio 1", False), ("Radio 2", False), ("Radio 3", False)],
    },
    "roleplay_voice": {
        "Vc": [("Tambayan", False), ("Room 1", False), ("Room 2", False), ("Admin", True), ("Pc Check", True)],
    },
    "packing_voice": {
        "@Vc": [("Room 1", False), ("Room 2", False), ("Room 3", False), ("Room 4", False)],
    },
}

async def _get_or_create_category(guild: discord.Guild, name: str) -> discord.CategoryChannel:
    existing = discord.utils.get(guild.categories, name=name)
    if existing:
        return existing
    return await guild.create_category(name=name, reason="HunterX setupsv")

async def _create_text_template(guild: discord.Guild, template_key: str):
    template = SETUPSV_TEXT_TEMPLATES.get(template_key)
    if not template:
        return
    for cat_name, channels in template.items():
        category = await _get_or_create_category(guild, cat_name)
        for ch_name in channels:
            safe = _safe_channel_name(ch_name)
            if discord.utils.get(category.text_channels, name=safe):
                continue
            await guild.create_text_channel(name=safe, category=category, reason="HunterX setupsv")

async def _create_voice_template(guild: discord.Guild, template_key: str):
    template = SETUPSV_VOICE_TEMPLATES.get(template_key)
    if not template:
        return
    for cat_name, channels in template.items():
        category = await _get_or_create_category(guild, cat_name)
        for ch_name, admin_lock in channels:
            safe = _safe_channel_name(ch_name)
            if discord.utils.get(category.voice_channels, name=safe):
                continue
            overwrites = None
            if admin_lock:
                overwrites = {
                    guild.default_role: discord.PermissionOverwrite(view_channel=True, connect=False),
                    guild.me: discord.PermissionOverwrite(view_channel=True, connect=True, manage_channels=True, move_members=True),
                }
                for role in guild.roles:
                    if role.permissions.administrator:
                        overwrites[role] = discord.PermissionOverwrite(view_channel=True, connect=True, move_members=True, manage_channels=True)
            await guild.create_voice_channel(name=safe, category=category, overwrites=overwrites, reason="HunterX setupsv")

def _setupsv_embed_main() -> discord.Embed:
    desc = (
        "## Setup Server\n"
        "Setup your server\n\n"
        "**Navigation**\n"
        "- Click Button Text Channels For Text Channels Settings\n"
        "- Click Button Voice Channels For Voice Channels Settings\n"
        "- Click Button Overview To See All Current Settings"
    )
    return discord.Embed(description=desc, color=0xFFFFFF)

def _setupsv_embed_text() -> discord.Embed:
    desc = (
        "## Text Channels Setup\n"
        "Setup your channels\n\n"
        "Choose Category\n"
        "Then select a category button below."
    )
    return discord.Embed(description=desc, color=0xFFFFFF)

def _setupsv_embed_voice() -> discord.Embed:
    desc = (
        "## Voice Setup\n"
        "Setup your voice channels\n\n"
        "Click The Category Button Below\n"
        "Then select a category."
    )
    return discord.Embed(description=desc, color=0xFFFFFF)

def _setupsv_embed_overview() -> discord.Embed:
    desc = (
        "## Setup Server\n"
        "Setup your server\n\n"
        "**Category**\n"
        "Roleplay Server\n"
        "Roblox Gang Server\n"
        "Packing Server\n\n"
        "Select Category"
    )
    return discord.Embed(description=desc, color=0xFFFFFF)

class SetupSVMainView(View):
    def __init__(self, author_id: int):
        super().__init__(timeout=600)
        self.author_id = author_id

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message("Only you can use this.", ephemeral=True)
            return False
        return True

    @discord.ui.button(label="Text Channels", style=discord.ButtonStyle.secondary)
    async def text_btn(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(embed=_setupsv_embed_text(), view=SetupSVTextView(self.author_id))

    @discord.ui.button(label="Voice Channels", style=discord.ButtonStyle.secondary)
    async def voice_btn(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(embed=_setupsv_embed_voice(), view=SetupSVVoiceView(self.author_id))

    @discord.ui.button(label="Overview", style=discord.ButtonStyle.secondary)
    async def overview_btn(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(embed=_setupsv_embed_overview(), view=SetupSVOverviewView(self.author_id))

class SetupSVTextView(View):
    def __init__(self, author_id: int):
        super().__init__(timeout=600)
        self.author_id = author_id
        select = Select(
            placeholder="Select a category",
            options=[
                discord.SelectOption(label="Roblox Gang Channels", value="roblox_gang"),
                discord.SelectOption(label="Packing Community Channels", value="packing"),
                discord.SelectOption(label="Roleplay Community Channels", value="roleplay"),
            ],
            min_values=1,
            max_values=1,
        )
        select.callback = self._on_select
        self.add_item(select)

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message("Only you can use this.", ephemeral=True)
            return False
        return True

    async def _on_select(self, interaction: discord.Interaction):
        choice = interaction.data.get("values", [""])[0]
        await interaction.response.defer(ephemeral=True, thinking=True)
        try:
            await _create_text_template(interaction.guild, choice)
            await interaction.followup.send("Text channels created. Existing channels were skipped.", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"Error: {e}", ephemeral=True)

    @discord.ui.button(label="Back", style=discord.ButtonStyle.secondary)
    async def back_btn(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(embed=_setupsv_embed_main(), view=SetupSVMainView(self.author_id))

class SetupSVVoiceView(View):
    def __init__(self, author_id: int):
        super().__init__(timeout=600)
        self.author_id = author_id
        select = Select(
            placeholder="Select a category",
            options=[
                discord.SelectOption(label="Roblox Voice Gang Channels", value="roblox_voice"),
                discord.SelectOption(label="Roleplay Voice Channels", value="roleplay_voice"),
                discord.SelectOption(label="Packing Voice Channels", value="packing_voice"),
            ],
            min_values=1,
            max_values=1,
        )
        select.callback = self._on_select
        self.add_item(select)

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message("Only you can use this.", ephemeral=True)
            return False
        return True

    async def _on_select(self, interaction: discord.Interaction):
        choice = interaction.data.get("values", [""])[0]
        await interaction.response.defer(ephemeral=True, thinking=True)
        try:
            await _create_voice_template(interaction.guild, choice)
            await interaction.followup.send("Voice channels created. Existing channels were skipped.", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"Error: {e}", ephemeral=True)

    @discord.ui.button(label="Back", style=discord.ButtonStyle.secondary)
    async def back_btn(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(embed=_setupsv_embed_main(), view=SetupSVMainView(self.author_id))

class SetupSVOverviewView(View):
    def __init__(self, author_id: int):
        super().__init__(timeout=600)
        self.author_id = author_id
        select = Select(
            placeholder="Select a category",
            options=[
                discord.SelectOption(label="Roleplay Server", value="roleplay"),
                discord.SelectOption(label="Roblox Gang Server", value="roblox_gang"),
                discord.SelectOption(label="Packing Server", value="packing"),
            ],
            min_values=1,
            max_values=1,
        )
        select.callback = self._on_select
        self.add_item(select)

    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message("Only you can use this.", ephemeral=True)
            return False
        return True

    async def _on_select(self, interaction: discord.Interaction):
        choice = interaction.data.get("values", [""])[0]
        await interaction.response.defer(ephemeral=True, thinking=True)
        try:
            if choice == "roleplay":
                await _create_text_template(interaction.guild, "roleplay")
                await _create_voice_template(interaction.guild, "roleplay_voice")
            elif choice == "roblox_gang":
                await _create_text_template(interaction.guild, "roblox_gang")
                await _create_voice_template(interaction.guild, "roblox_voice")
            elif choice == "packing":
                await _create_text_template(interaction.guild, "packing")
                await _create_voice_template(interaction.guild, "packing_voice")
            await interaction.followup.send("Overview setup done (text + voice).", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"Error: {e}", ephemeral=True)

    @discord.ui.button(label="Back", style=discord.ButtonStyle.secondary)
    async def back_btn(self, interaction: discord.Interaction, button: Button):
        await interaction.response.edit_message(embed=_setupsv_embed_main(), view=SetupSVMainView(self.author_id))

@bot.hybrid_command(name="setupsv", aliases=["setup-server", "server-template"])
@commands.has_permissions(administrator=True)
async def setupsv(ctx):
    embed = _setupsv_embed_main()
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed, view=SetupSVMainView(ctx.author.id))

# ==================== AUTO RESPOND ====================
@bot.hybrid_command(name="respond-add", aliases=["autorespondadd"])
@commands.has_permissions(administrator=True)
async def autorespondadd(ctx, trigger: str, *, reply: str):
    gid = str(ctx.guild.id)
    if gid not in autorespond_data:
        autorespond_data[gid] = {}
    trigger_key = trigger.lower()
    if trigger_key in autorespond_data[gid]:
        return await send_embed(ctx, "Already Exists", f"Trigger `{trigger}` already exists. Use `$autorespondedit` to change it.")
    autorespond_data[gid][trigger_key] = reply
    save_data(AUTORESPOND_FILE, autorespond_data)
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AutoRespond Added", f"Trigger: `{trigger}`\nReply: {reply}")

@bot.hybrid_command(name="respond-edit", aliases=["autorespondedit"])
@commands.has_permissions(administrator=True)
async def autorespondedit(ctx, trigger: str, *, reply: str):
    gid = str(ctx.guild.id)
    trigger_key = trigger.lower()
    if gid not in autorespond_data or trigger_key not in autorespond_data.get(gid, {}):
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Not Found", f"Trigger `{trigger}` not found. Use `$autorespondadd` to create it.")
    autorespond_data[gid][trigger_key] = reply
    save_data(AUTORESPOND_FILE, autorespond_data)
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AutoRespond Updated", f"Trigger: `{trigger}`\nNew Reply: {reply}")

@bot.hybrid_command(name="respond-remove", aliases=["autorespondremove"])
@commands.has_permissions(administrator=True)
async def autorespondremove(ctx, trigger: str):
    gid = str(ctx.guild.id)
    trigger_key = trigger.lower()
    if gid not in autorespond_data or trigger_key not in autorespond_data.get(gid, {}):
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Not Found", f"Trigger `{trigger}` not found.")
    del autorespond_data[gid][trigger_key]
    save_data(AUTORESPOND_FILE, autorespond_data)
    await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AutoRespond Removed", f"Trigger `{trigger}` has been removed.")

# ==================== BACKUP CREATE ====================
import secrets as _secrets

@bot.hybrid_command(name="backupcreate", aliases=["backup", "backup-create"])
async def backupcreate(ctx):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    guild = ctx.guild
    gid = str(guild.id)

    backup = {
        "guild_id": gid,
        "guild_name": guild.name,
        "created_by": str(ctx.author.id),
        "created_at": datetime.now(PH_TIME).strftime("%B %d, %Y %I:%M %p"),
        "verification_level": str(guild.verification_level),
        "roles": [],
        "categories": [],
        "channels": [],
        "messages": {},
    }

    for role in guild.roles:
        if role.is_default():
            continue
        backup["roles"].append({
            "name": role.name,
            "color": role.color.value,
            "permissions": role.permissions.value,
            "hoist": role.hoist,
            "mentionable": role.mentionable,
            "position": role.position,
        })

    for cat in guild.categories:
        backup["categories"].append({"name": cat.name, "position": cat.position})

    total_messages = 0
    for ch in guild.channels:
        if isinstance(ch, discord.CategoryChannel):
            continue
        ch_data = {
            "name": ch.name,
            "type": str(ch.type),
            "category_name": ch.category.name if ch.category else None,
            "position": ch.position,
        }
        if isinstance(ch, discord.TextChannel):
            ch_data["topic"] = ch.topic
            ch_data["slowmode_delay"] = ch.slowmode_delay
            ch_data["nsfw"] = ch.nsfw
            try:
                msgs = []
                async for msg in ch.history(limit=100, oldest_first=True):
                    if msg.author.bot:
                        continue
                    attachments = [a.url for a in msg.attachments]
                    msgs.append({
                        "author_name": msg.author.display_name,
                        "author_id": str(msg.author.id),
                        "author_avatar": str(msg.author.display_avatar.url),
                        "content": msg.content,
                        "timestamp": msg.created_at.strftime("%B %d, %Y %I:%M %p"),
                        "attachments": attachments,
                    })
                if msgs:
                    backup["messages"][ch.name] = msgs
                    total_messages += len(msgs)
            except Exception:
                pass
        backup["channels"].append(ch_data)

    backup_id = _secrets.token_hex(4).upper()
    if gid not in backup_store:
        backup_store[gid] = {}
    backup_store[gid][backup_id] = backup
    save_data(BACKUP_FILE, backup_store)

    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Backup Created",
        f"**Backup ID:** `{backup_id}`\n"
        f"Use `$backuprestore {backup_id}` to restore.\n\n"
        f"**Roles:** {len(backup['roles'])}\n"
        f"**Categories:** {len(backup['categories'])}\n"
        f"**Channels:** {len(backup['channels'])}\n"
        f"**Messages saved:** {total_messages}\n"
        f"**Date:** {backup['created_at']}")
    try:
        await ctx.message.delete()
    except:
        pass

# ==================== BACKUP RESTORE ====================
@bot.hybrid_command(name="backuprestore", aliases=["restore", "backup-restore"])
async def backuprestore(ctx, backup_id: str = None):
    if not is_owner(ctx) and not ctx.author.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "You need administrator permission.")
    if not backup_id:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Missing ID", "Usage: `$backuprestore <ID>`")

    guild = ctx.guild
    gid = str(guild.id)
    backup_id = backup_id.upper()
    guild_backups = backup_store.get(gid, {})
    backup = guild_backups.get(backup_id)
    if not backup:
        ids = ", ".join([f"`{i}`" for i in guild_backups.keys()]) if guild_backups else "None"
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Backup Not Found",
            f"No backup with ID `{backup_id}`.\n\n**Available IDs:** {ids}")

    await send_embed(ctx, "<a:HunterxRestore:1514609270463791187> Restoring Backup",
        f"Restoring from backup `{backup_id}` ({backup.get('created_at', 'Unknown date')})...\nThis may take a moment.")

    created_roles, created_cats, created_chs, restored_msgs = 0, 0, 0, 0
    existing_role_names = {r.name for r in guild.roles}
    for role_data in sorted(backup.get("roles", []), key=lambda r: r["position"]):
        if role_data["name"] not in existing_role_names:
            try:
                await guild.create_role(
                    name=role_data["name"],
                    color=discord.Color(role_data["color"]),
                    permissions=discord.Permissions(role_data["permissions"]),
                    hoist=role_data["hoist"],
                    mentionable=role_data["mentionable"],
                    reason=f"HunterX Backup Restore [{backup_id}]",
                )
                created_roles += 1
            except:
                pass

    existing_cat_names = {c.name for c in guild.categories}
    for cat_data in sorted(backup.get("categories", []), key=lambda c: c["position"]):
        if cat_data["name"] not in existing_cat_names:
            try:
                await guild.create_category(name=cat_data["name"], reason=f"HunterX Backup Restore [{backup_id}]")
                created_cats += 1
            except:
                pass

    existing_ch_names = {c.name for c in guild.channels}
    restored_channels = {}
    for ch_data in backup.get("channels", []):
        category = discord.utils.get(guild.categories, name=ch_data["category_name"]) if ch_data.get("category_name") else None
        if ch_data["name"] not in existing_ch_names:
            try:
                if ch_data["type"] == "text":
                    new_ch = await guild.create_text_channel(
                        name=ch_data["name"], category=category,
                        topic=ch_data.get("topic"), slowmode_delay=ch_data.get("slowmode_delay", 0),
                        nsfw=ch_data.get("nsfw", False), reason=f"HunterX Backup Restore [{backup_id}]",
                    )
                    restored_channels[ch_data["name"]] = new_ch
                    created_chs += 1
                elif ch_data["type"] == "voice":
                    await guild.create_voice_channel(name=ch_data["name"], category=category,
                        reason=f"HunterX Backup Restore [{backup_id}]")
                    created_chs += 1
            except:
                pass
        else:
            existing_ch = discord.utils.get(guild.text_channels, name=ch_data["name"])
            if existing_ch:
                restored_channels[ch_data["name"]] = existing_ch

    saved_messages = backup.get("messages", {})
    for ch_name, msgs in saved_messages.items():
        target_ch = restored_channels.get(ch_name)
        if not target_ch:
            continue
        for msg_data in msgs:
            try:
                embed = discord.Embed(
                    description=msg_data.get("content", ""),
                    color=0x2F3136,
                    timestamp=datetime.now(PH_TIME)
                )
                embed.set_author(name=msg_data["author_name"], icon_url=msg_data.get("author_avatar", ""))
                embed.set_footer(text=f"Restored • {msg_data.get('timestamp', '')}")
                await target_ch.send(embed=embed)
                restored_msgs += 1
                await asyncio.sleep(0.5)
            except:
                pass

    await send_embed(ctx, "Backup Restored",
        f"Restore from `{backup_id}` complete!\n\n"
        f"*Roles created:* {created_roles}\n"
        f"*Categories created:* {created_cats}\n"
        f"*Channels created:* {created_chs}\n"
        f"*Messages restored:* {restored_msgs}")
    try:
        await ctx.message.delete()
    except:
        pass

# ==================== SET WELCOME ====================
@bot.hybrid_command(name="set-welcome", aliases=["setwelcome"])
@commands.has_permissions(administrator=True)
async def setwelcome(ctx):
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})

    def check(m):
        return m.author == ctx.author and m.channel == ctx.channel

    async def ask(question):
        embed = discord.Embed(description=question, color=0xFFFFFF)
        embed.set_footer(text="HunterX • Mention a channel or type 'skip' — timeout: 60s")
        await ctx.send(embed=embed)
        try:
            msg = await bot.wait_for("message", check=check, timeout=60)
            return msg.content
        except asyncio.TimeoutError:
            return None

    await send_embed(ctx, "Welcome Setup", "I will ask you for up to **2 channels** to link in the welcome message.\nType 'skip' to skip any.")

    channels_set = []
    for i in range(1, 3):
        ans = await ask(f"Mention Channel {i} to add as a link in the welcome message:")
        if ans is None:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> Timed Out", "Setup Cancelled.")
        if ans.lower() == "skip":
            continue
        match = re.search(r"<#(\d+)>", ans)
        if match:
            ch = ctx.guild.get_channel(int(match.group(1)))
            if ch:
                channels_set.append(ch.id)

    config_data[gid]["welcome_channels"] = channels_set
    save_config()

    ch_mentions = " ".join([f"<#{cid}>" for cid in channels_set]) or "None"
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Welcome Setup Complete",
        f"Channels linked: {ch_mentions}\n\nNow set the welcome channel: `$welcome #channel`\nSet custom message: `$setmessage <your message>`\nTest it: `$welcome test`")

# ==================== SET MESSAGE ====================
@bot.hybrid_command(name="set-message", aliases=["setmessage"])
@commands.has_permissions(administrator=True)
async def setmessage(ctx, *, message: str):
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})["welcome_message"] = message
    save_config()
    embed = make_embed("<:HunteXCheck:1518422536558481448> Welcome Message Set", f"Custom message saved:\n\n{message}")
    embed.set_footer(text="HunterX • Use {user} for mention, {server} for server name, {count} for member number")
    await ctx.send(embed=embed)

# ==================== WELCOME ====================
@bot.hybrid_command(aliases=["welcome-test"])
@commands.has_permissions(administrator=True)
async def welcome(ctx, channel: Optional[discord.TextChannel] = None, action: str = None):
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})

    if action and action.lower() == "test":
        welcome_ch_id = config_data.get(gid, {}).get("welcome_channel")
        if not welcome_ch_id:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> Not Set", "Set a welcome channel first with `$welcome #channel`")
        welcome_ch = ctx.guild.get_channel(welcome_ch_id)
        if not welcome_ch:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> Channel Not Found", "Welcome channel not found.")
        await send_welcome_embed(ctx.guild, ctx.author, welcome_ch)
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Test Sent", f"Test welcome sent to {welcome_ch.mention}")
        return

    if channel:
        config_data[gid]["welcome_channel"] = channel.id
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Welcome Channel Set", f"Welcome messages will be sent to {channel.mention}")
    else:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid", "Usage: `$welcome #channel` or `$welcome test`")

# ==================== SEND WELCOME EMBED (GREEN + Channel Mentions) ====================
async def send_welcome_embed(guild, member, channel):
    gid = str(guild.id)
    cfg = config_data.get(gid, {})
    # Support both old and new key names
    linked_channel_ids = cfg.get("welcome_channels", cfg.get("WELCOME_CHANNELS <:catwave:1515279541147406416>", []))
    custom_message = cfg.get("welcome_message", "")

    desc = f"**WELCOME {member.mention} TO {guild.name}** <a:HappyHunterXemoji:1512831297582661803>\n\n"

    # Use proper Discord channel mentions (<#id>) so they are clickable
    for cid in linked_channel_ids:
        ch = guild.get_channel(cid)
        if ch:
            desc += f"• {ch.mention}\n"

    if custom_message:
        formatted = custom_message.replace("{user}", member.mention).replace("{server}", guild.name).replace("{count}", str(guild.member_count))
        desc += f"\n{formatted}"

    # GREEN embed color
    embed = discord.Embed(description=desc, timestamp=datetime.now(PH_TIME), color=0x57F287)
    embed.set_thumbnail(url=member.display_avatar.url)
    embed.set_footer(text=f"Member #{guild.member_count}")
    await channel.send(content=member.mention, embed=embed)

# ==================== BOOST CHANNEL SETUP ====================
@bot.hybrid_command(name="boost-channel", aliases=["boostchannel"])
@commands.has_permissions(administrator=True)
async def boostchannel(ctx, action: str = "setup"):
    if action.lower() != "setup":
        return await send_embed(ctx, "Usage", "`$boostchannel setup`")
    guild = ctx.guild
    gid = str(guild.id)

    booster_role = discord.utils.get(guild.roles, name="Booster")
    if not booster_role:
        try:
            booster_role = await guild.create_role(
                name="Booster",
                color=discord.Color.from_rgb(255, 105, 180),
                hoist=True,
                reason="HunterX Boost Setup"
            )
        except Exception as e:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> Error", f"Could not create Booster role: {e}")

    existing = discord.utils.get(guild.text_channels, name="rich")
    if existing:
        config_data.setdefault(gid, {})["boost_channel"] = existing.id
        config_data[gid]["boost_role"] = booster_role.id
        save_config()
        return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Already Setup",
            f"Boost channel {existing.mention} and {booster_role.mention} role are ready.")

    try:
        boost_ch = await guild.create_text_channel(name="rich", reason="HunterX Boost Channel Setup")
        config_data.setdefault(gid, {})["boost_channel"] = boost_ch.id
        config_data[gid]["boost_role"] = booster_role.id
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Boost Setup Complete",
            f"**Channel:** {boost_ch.mention}\n"
            f"**Role:** {booster_role.mention}\n\n"
            f"Set custom boost message with `$boostmessage <msg>`")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== BOOST MESSAGE ====================
@bot.hybrid_command(name="boost-message", aliases=["boostmessage"])
@commands.has_permissions(administrator=True)
async def boostmessage(ctx, *, message: str):
    gid = str(ctx.guild.id)
    config_data.setdefault(gid, {})["boost_message"] = message
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Boost Message Set",
        f"Custom boost message saved:\n\n{message}\n\n-# Use {{user}} for the booster mention.")

# ==================== SERVER INFO ====================
@bot.hybrid_command(name="server-info", aliases=["serverinfo", "si"])
async def serverinfo(ctx):
    guild = ctx.guild
    humans = len([m for m in guild.members if not m.bot])
    bots_count = len([m for m in guild.members if m.bot])
    boost_level = guild.premium_tier
    boost_count = guild.premium_subscription_count or 0
    created = guild.created_at.strftime("%B %d, %Y")
    emojis = guild.emojis
    emoji_str = " ".join(str(e) for e in emojis[:30]) if emojis else "None"
    if len(emojis) > 30:
        emoji_str += f" ...+{len(emojis) - 30} more"

    embed = discord.Embed(color=0xFFFFFF)
    embed.set_author(name=guild.name, icon_url=guild.icon.url if guild.icon else None)
    if guild.icon:
        embed.set_thumbnail(url=guild.icon.url)

    embed.add_field(name="Server Owner", value=str(guild.owner), inline=True)
    embed.add_field(name="ID", value=str(guild.owner.id), inline=True)
    embed.add_field(name="\u200b", value="\u200b", inline=True)
    embed.add_field(name="Members", value=str(guild.member_count), inline=True)
    embed.add_field(name="Channels", value=str(len(guild.channels)), inline=True)
    embed.add_field(name="Roles", value=str(len(guild.roles)), inline=True)
    embed.add_field(name="Bot", value=str(bots_count), inline=True)
    embed.add_field(name="Human", value=str(humans), inline=True)
    embed.add_field(name="Server Boost", value=f"{boost_count} Boosts (Level {boost_level})", inline=True)
    embed.add_field(name="Server Created", value=created, inline=True)
    embed.add_field(name=f"Emoji List ({len(emojis)})", value=emoji_str, inline=False)
    embed.set_footer(text="HunterX")
    embed.timestamp = datetime.now(PH_TIME)
    await ctx.send(embed=embed)

# ==================== SERVERLIST ====================
@bot.hybrid_command(name="server-list", aliases=["serverlist", "sl"])
async def serverlist(ctx):
    guilds = list(bot.guilds)
    total = len(guilds)
    embed = build_serverlist_embed(1)

    # Show buttons only if more than 6 servers
    if total > 6:
        view = ServerListView(page=1)
        await ctx.send(embed=embed, view=view)
    else:
        await ctx.send(embed=embed)


# ==================== BOT STATS ====================
@bot.hybrid_command(name="bot-stats", aliases=["botstats", "bs"])
async def botstats(ctx):
    """Show bot uptime, ping, and server count."""
    now = datetime.now(PH_TIME)
    delta = now - bot_start_time
    days = delta.days
    hours, remainder = divmod(delta.seconds, 3600)
    minutes, seconds = divmod(remainder, 60)
    uptime_str = f"`{days}d {hours}h {minutes}m {seconds}s`"

    ping_ms = round(bot.latency * 1000)
    server_count = len(bot.guilds)
    user_count = sum(g.member_count or 0 for g in bot.guilds)

    embed = make_embed(
        "<:249630gradientgear:1512782705522245712> Bot Status",
        f"**Uptime:** {uptime_str}\n"
        f"**Ping:** `{ping_ms}ms`\n"
        f"**Servers:** `{server_count}`\n"
        f"**Total Users:** `{user_count:,}`"
    )
    if bot.user:
        embed.set_thumbnail(url=bot.user.display_avatar.url)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== BOT INFO ====================
@bot.hybrid_command(name="bot-info", aliases=["botinfo", "bi", "info"])
async def botinfo(ctx):
    """Show detailed bot information."""
    now = datetime.now(PH_TIME)
    delta = now - bot_start_time
    days = delta.days
    hours, remainder = divmod(delta.seconds, 3600)
    minutes, seconds = divmod(remainder, 60)
    uptime_str = f"{days}d {hours}h {minutes}m {seconds}s"

    ping_ms = round(bot.latency * 1000)
    server_count = len(bot.guilds)
    user_count = sum(g.member_count or 0 for g in bot.guilds)
    command_count = len(bot.commands)

    mem_kb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    mem_mb = mem_kb / 1024

    embed = discord.Embed(title="HunterX / Bot Info", color=0xFFFFFF)
    embed.add_field(name="Version", value=f"`{BOT_VERSION}`", inline=True)
    embed.add_field(name="Uptime", value=f"`{uptime_str}`", inline=True)
    embed.add_field(name="Memory", value=f"`{mem_mb:.2f} MB`", inline=True)
    embed.add_field(name="Ping", value=f"`{ping_ms}ms`", inline=True)
    embed.add_field(name="Guilds", value=f"`{server_count}`", inline=True)
    embed.add_field(name="Users", value=f"`{user_count:,}`", inline=True)
    embed.add_field(name="Commands", value=f"`{command_count}`", inline=True)
    embed.add_field(name="Website", value="[hunterx2.netlify.app](https://hunterx2.netlify.app)", inline=True)
    embed.add_field(name="Server Support", value="[HunterX Support](https://discord.gg/k8jj7HCc9)", inline=True)
    embed.add_field(name="Invite", value="[Add HunterX](https://discord.com/oauth2/authorize?client_id=1480996341894217901)", inline=True)
    embed.timestamp = datetime.now(PH_TIME)
    if bot.user:
        embed.set_thumbnail(url=bot.user.display_avatar.url)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== FUN / UTILITY ACTIONS ====================
async def fun_action(ctx, target: discord.Member, action: str, past: str):
    gif_url = await fetch_gif(action)
    desc = f"{ctx.author.mention} *{past}* {target.mention}"
    embed = discord.Embed(description=desc, color=0xFFFFFF)
    embed.set_footer(text="HunterX")
    if gif_url:
        embed.set_image(url=gif_url)
    await ctx.send(embed=embed)

COINFLIP_CHOICES = [
    discord.app_commands.Choice(name="Head", value="head"),
    discord.app_commands.Choice(name="Tails", value="tails"),
]

@bot.hybrid_command(name="jvc")
@discord.app_commands.describe(voice_channel="Select the voice channel to join")
async def jvc(ctx, voice_channel: discord.VoiceChannel = None):
    target_channel = voice_channel
    if target_channel is None:
        if not ctx.author.voice or not ctx.author.voice.channel:
            return await send_embed(ctx, "No Voice Channel", "Join a voice channel first or provide one with `/jvc #voice-channel`.")
        target_channel = ctx.author.voice.channel

    voice_client = ctx.guild.voice_client

    try:
        if voice_client and voice_client.is_connected():
            if voice_client.channel.id == target_channel.id:
                return await send_embed(ctx, "Already Connected", f"I'm already connected to {target_channel.mention}.")
            await voice_client.move_to(target_channel)
            return await send_embed(ctx, "Moved Voice Channel", f"I moved to {target_channel.mention}.")

        await target_channel.connect()
        await send_embed(ctx, "Joined Voice Channel", f"I joined {target_channel.mention}.")
    except Exception as e:
        await send_embed(ctx, "Error", str(e))

@bot.hybrid_command(name="dvc")
async def dvc(ctx):
    voice_client = ctx.guild.voice_client
    if not voice_client or not voice_client.is_connected():
        return await send_embed(ctx, "Not Connected", "I'm not connected to any voice channel right now.")

    try:
        channel_name = voice_client.channel.mention if getattr(voice_client, "channel", None) else "voice channel"
        await voice_client.disconnect(force=True)
        await send_embed(ctx, "Disconnected", f"I left {channel_name}.")
    except Exception as e:
        await send_embed(ctx, "Error", str(e))

# ==================== MUSIC CONTROLS (NO-PREFIX COMMAND NAMES) ====================
def _can_control_music(member: discord.Member, voice_client: discord.VoiceProtocol) -> bool:
    try:
        if not member or not voice_client or not voice_client.is_connected():
            return False
        if not member.voice or not member.voice.channel:
            return False
        return voice_client.channel and member.voice.channel.id == voice_client.channel.id
    except Exception:
        return False

@bot.hybrid_command(name="music-stop", aliases=["h!stop"])
async def h_stop(ctx):
    vc = ctx.guild.voice_client
    if not vc or not vc.is_connected():
        return await send_embed(ctx, "Music", "I am not connected to a voice channel right now.")
    if not _can_control_music(ctx.author, vc):
        return await send_embed(ctx, "Music", "Join the same voice channel first to use music controls.")
    try:
        music_queues[ctx.guild.id].clear()
        vc.stop()
        await send_embed(ctx, "Music", "Stopped and cleared queue.")
    except Exception as e:
        await send_embed(ctx, "Error", str(e))

@bot.hybrid_command(name="music-skip", aliases=["h!skip"])
async def h_skip(ctx):
    vc = ctx.guild.voice_client
    if not vc or not vc.is_connected():
        return await send_embed(ctx, "Music", "I am not connected to a voice channel right now.")
    if not _can_control_music(ctx.author, vc):
        return await send_embed(ctx, "Music", "Join the same voice channel first to use music controls.")
    try:
        vc.stop()  # triggers after() -> play next
        await send_embed(ctx, "Music", "Skipped.")
    except Exception as e:
        await send_embed(ctx, "Error", str(e))

@bot.hybrid_command(name="music-pause", aliases=["h!pause"])
async def h_pause(ctx):
    vc = ctx.guild.voice_client
    if not vc or not vc.is_connected():
        return await send_embed(ctx, "Music", "I am not connected to a voice channel right now.")
    if not _can_control_music(ctx.author, vc):
        return await send_embed(ctx, "Music", "Join the same voice channel first to use music controls.")
    try:
        if vc.is_playing():
            vc.pause()
        await send_embed(ctx, "Music", "Paused.")
    except Exception as e:
        await send_embed(ctx, "Error", str(e))

@bot.hybrid_command(name="music-resume", aliases=["h!resume"])
async def h_resume(ctx):
    vc = ctx.guild.voice_client
    if not vc or not vc.is_connected():
        return await send_embed(ctx, "Music", "I am not connected to a voice channel right now.")
    if not _can_control_music(ctx.author, vc):
        return await send_embed(ctx, "Music", "Join the same voice channel first to use music controls.")
    try:
        if vc.is_paused():
            vc.resume()
        await send_embed(ctx, "Music", "Resumed.")
    except Exception as e:
        await send_embed(ctx, "Error", str(e))

@bot.hybrid_command(name="hcf", aliases=["coinflip"])
@discord.app_commands.describe(amount="Bet amount or `all`", side="Choose h/head or t/tails")
@discord.app_commands.choices(side=COINFLIP_CHOICES)
async def hcf(ctx, amount: str = None, side: str = None):
    if side is None:
        return await send_embed(ctx, "Invalid", "Usage: `hcf <amount> <h/t>`")

    profile = get_user_economy(ctx.author.id)
    current_cash = int(profile.get("cash", 0))
    amount_raw = (amount or "").lower().strip()

    if amount_raw == "all":
        bet_amount = current_cash
    else:
        try:
            bet_amount = int(amount_raw.replace(",", ""))
        except ValueError:
            return await send_embed(ctx, "Invalid", "Use a valid amount like `hcf 500 h` or `hcf all t`.")

    if bet_amount <= 0:
        return await send_embed(ctx, "Invalid", "Bet amount must be greater than `0`.")
    if current_cash <= 0:
        return await send_embed(ctx, "No Cash", "You don't have enough cash yet. Use `hdaily` first.")
    if bet_amount > current_cash:
        return await send_embed(
            ctx,
            "Not Enough Cash",
            f"Your balance is only `{format_cash(current_cash)}` cash."
        )

    guess = (side or "").lower().strip()
    guess_map = {
        "h": "head",
        "head": "head",
        "heads": "head",
        "t": "tails",
        "tail": "tails",
        "tails": "tails",
    }
    normalized = guess_map.get(guess)
    if not normalized:
        return await send_embed(ctx, "Invalid", "Use `h`, `t`, `head`, or `tails`.")

    result = random.choice(["head", "tails"])
    won = normalized == result
    result_label = "HEAD" if result == "head" else "TAILS"
    user_label = "HEAD" if normalized == "head" else "TAILS"
    if won:
        profile["cash"] = current_cash + bet_amount
        status_line = f"You won `{format_cash(bet_amount)}` cash."
    else:
        profile["cash"] = current_cash - bet_amount
        status_line = f"You lost `{format_cash(bet_amount)}` cash."
    save_economy()
    description = (
        f"**Bet:** `{format_cash(bet_amount)}`\n"
        f"**Choice:** `{user_label}`\n"
        f"**Result:** `{result_label}`\n"
        f"**Status:** {status_line}\n"
        f"**Balance:** `{format_cash(int(profile['cash']))}`"
    )
    await send_embed(ctx, "HunterX Coinflip", description)

@bot.hybrid_command(name="hcash", aliases=["balance", "cash"])
async def hcash(ctx, member: discord.Member = None):
    target = member or ctx.author
    profile = get_user_economy(target.id)
    description = (
        f"**User:** {target.mention}\n"
        f"**Cash:** `${format_cash(int(profile.get('cash', 0)))}`"
    )
    await send_embed(ctx, "HunterX Cash", description)

@bot.hybrid_command(name="hdaily", aliases=["daily", "claim-daily"])
async def hdaily(ctx):
    profile = get_user_economy(ctx.author.id)
    now = datetime.now(PH_TIME)
    last_daily_raw = profile.get("last_daily")

    if last_daily_raw:
        try:
            last_daily = datetime.fromisoformat(last_daily_raw)
            if last_daily.tzinfo is None:
                last_daily = last_daily.replace(tzinfo=PH_TIME)
            next_claim = last_daily + timedelta(days=1)
            if now < next_claim:
                remaining = next_claim - now
                hours, remainder = divmod(int(remaining.total_seconds()), 3600)
                minutes, seconds = divmod(remainder, 60)
                return await send_embed(
                    ctx,
                    "Daily Cooldown",
                    f"You already claimed your daily reward.\nCome back in `{hours}h {minutes}m {seconds}s`."
                )
        except Exception:
            pass

    reward = random.randint(100, 2000)
    profile["cash"] = int(profile.get("cash", 0)) + reward
    profile["last_daily"] = now.isoformat()
    save_economy()

    description = (
        f"**Reward:** `${format_cash(reward)}`\n"
        f"**New Balance:** `${format_cash(int(profile['cash']))}`\n"
        f"**Next Daily:** `24 hours`"
    )
    await send_embed(ctx, "Daily Claimed", description)

# ==================== ECONOMY (EXTRA) ====================
@bot.hybrid_command(name="hshop", aliases=["shop", "economy-shop"])
async def hshop(ctx):
    lines = []
    for key, item in SHOP_ITEMS.items():
        lines.append(f"• `{key}` — **{item['name']}** — `${format_cash(item['price'])}`")
        if item.get("desc"):
            lines.append(f"  {item['desc']}")
    await send_embed(ctx, "HunterX Shop", "\n".join(lines) if lines else "No items.")

@bot.hybrid_command(name="hbuy", aliases=["buy", "purchase"])
async def hbuy(ctx, item_key: str = None, qty: int = 1):
    if not item_key:
        return await send_embed(ctx, "Usage", "`hbuy <item> [qty]` (check `hshop`)")
    key = item_key.lower().strip()
    if key not in SHOP_ITEMS:
        return await send_embed(ctx, "Not Found", f"Item `{key}` not found. Use `hshop`.")
    qty = max(1, int(qty))
    item = SHOP_ITEMS[key]
    cost = int(item["price"]) * qty
    profile = get_user_economy(ctx.author.id)
    cash = int(profile.get("cash", 0))
    if cash < cost:
        return await send_embed(ctx, "Not Enough Cash",
            f"Need `${format_cash(cost)}` but you only have `${format_cash(cash)}`.")
    profile["cash"] = cash - cost
    _inv_add(profile, key, qty)
    save_economy()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Purchase Complete",
        f"Bought **{item['name']}** x`{qty}` for `${format_cash(cost)}`.\nBalance: `${format_cash(int(profile['cash']))}`")

@bot.hybrid_command(name="hinv", aliases=["inventory", "inv", "bag"])
async def hinv(ctx, member: discord.Member = None):
    target = member or ctx.author
    profile = get_user_economy(target.id)
    await send_embed(ctx, f"{target.display_name}'s Inventory", _inv_str(profile))

@bot.hybrid_command(name="hgive", aliases=["give", "pay"])
async def hgive(ctx, member: discord.Member = None, amount: str = None):
    if not member or member.bot:
        return await send_embed(ctx, "Usage", "`hgive @user <amount>`")
    if member.id == ctx.author.id:
        return await send_embed(ctx, "Invalid", "You cannot give cash to yourself.")
    if not amount:
        return await send_embed(ctx, "Usage", "`hgive @user <amount>`")
    try:
        amt = int(amount.replace(",", ""))
    except ValueError:
        return await send_embed(ctx, "Invalid", "Use a valid number.")
    if amt <= 0:
        return await send_embed(ctx, "Invalid", "Amount must be > 0.")
    sender = get_user_economy(ctx.author.id)
    receiver = get_user_economy(member.id)
    if int(sender.get("cash", 0)) < amt:
        return await send_embed(ctx, "Not Enough Cash", "You do not have enough cash.")
    sender["cash"] = int(sender.get("cash", 0)) - amt
    receiver["cash"] = int(receiver.get("cash", 0)) + amt
    save_economy()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Sent Cash",
        f"Sent `${format_cash(amt)}` to {member.mention}.\nYour balance: `${format_cash(int(sender['cash']))}`")

@bot.hybrid_command(name="hslot", aliases=["slot", "slots"])
async def hslot(ctx, amount: str = None):
    if not amount:
        return await send_embed(ctx, "Usage", "`hslot <amount>` or `hslot all`")
    profile = get_user_economy(ctx.author.id)
    cash = int(profile.get("cash", 0))
    raw = amount.lower().strip()
    bet = cash if raw == "all" else None
    if bet is None:
        try:
            bet = int(raw.replace(",", ""))
        except ValueError:
            return await send_embed(ctx, "Invalid", "Use a valid number or `all`.")
    if bet <= 0:
        return await send_embed(ctx, "Invalid", "Bet must be > 0.")
    if bet > cash:
        return await send_embed(ctx, "Not Enough Cash", f"Balance: `${format_cash(cash)}`")

    symbols = ["🍒", "🍋", "🍇", "⭐", "💎"]
    roll = [random.choice(symbols) for _ in range(3)]
    payout = 0
    if roll[0] == roll[1] == roll[2]:
        payout = bet * 4
    elif len(set(roll)) == 2:
        payout = int(bet * 1.5)

    profile["cash"] = cash - bet + payout
    save_economy()
    result_line = "JACKPOT!" if payout == bet * 4 else ("WIN!" if payout > 0 else "LOSE!")
    await send_embed(
        ctx,
        "HunterX Slot",
        f"{' '.join(roll)}\n"
        f"**Bet:** `${format_cash(bet)}`\n"
        f"**Result:** {result_line}\n"
        f"**Payout:** `${format_cash(payout)}`\n"
        f"**Balance:** `${format_cash(int(profile['cash']))}`"
    )

@bot.hybrid_command(name="hrob", aliases=["rob", "steal"])
async def hrob(ctx, target: discord.Member = None):
    if not target or target.bot:
        return await send_embed(ctx, "Usage", "`hrob @user`")
    if target.id == ctx.author.id:
        return await send_embed(ctx, "Invalid", "You cannot rob yourself.")

    robber = get_user_economy(ctx.author.id)
    victim = get_user_economy(target.id)
    victim_cash = int(victim.get("cash", 0))
    if victim_cash <= 0:
        return await send_embed(ctx, "No Cash", "The target has no cash.")

    # cooldown 10 minutes
    now = datetime.now(PH_TIME)
    last_raw = (robber.get("last_rob") or "").strip()
    if last_raw:
        try:
            last = datetime.fromisoformat(last_raw)
            if last.tzinfo is None:
                last = last.replace(tzinfo=PH_TIME)
            if now < last + timedelta(minutes=10):
                remaining = (last + timedelta(minutes=10)) - now
                mins = max(1, int(remaining.total_seconds() // 60))
                return await send_embed(ctx, "Rob Cooldown", f"Wait `{mins} minute(s)` before robbing again.")
        except Exception:
            pass

    success = random.random() < 0.45
    steal = max(1, int(victim_cash * random.uniform(0.1, 0.3)))
    if success:
        victim["cash"] = max(0, victim_cash - steal)
        robber["cash"] = int(robber.get("cash", 0)) + steal
        msg = f"Successful! You stole `${format_cash(steal)}` from {target.mention}."
    else:
        fine = max(1, int(steal * 0.6))
        robber_cash = int(robber.get("cash", 0))
        robber["cash"] = max(0, robber_cash - fine)
        msg = f"Failed! You got fined `${format_cash(fine)}`."

    robber["last_rob"] = now.isoformat()
    save_economy()
    await send_embed(ctx, "HunterX Rob", f"{msg}\nYour balance: `${format_cash(int(robber.get('cash', 0)))}`")

@bot.hybrid_command(name="htopcash", aliases=["topcash", "leaderboard", "top-cash", "cash-leaderboard"])
async def htopcash(ctx):
    # Leaderboard in current guild
    members = ctx.guild.members
    rows = []
    for m in members:
        if m.bot:
            continue
        prof = get_user_economy(m.id)
        rows.append((m, int(prof.get("cash", 0))))
    rows.sort(key=lambda x: x[1], reverse=True)
    top = rows[:10]
    if not top:
        return await send_embed(ctx, "Top Cash", "No data available.")
    lines = []
    for i, (m, cash) in enumerate(top, 1):
        lines.append(f"**{i}.** {m.mention} — `${format_cash(cash)}`")
    await send_embed(ctx, "Top Cash", "\n".join(lines))

# ==================== FUN SHIP ====================
@bot.hybrid_command(name="funship", aliases=["ship", "love-match"])
async def funship(ctx, user1: discord.Member = None, user2: discord.Member = None):
    if not user1 or not user2:
        return await send_embed(ctx, "Usage", "`funship @user1 @user2`")
    # Stable percent (same pair → same result)
    seed = int(f"{min(user1.id, user2.id)}{max(user1.id, user2.id)}") % 10_000_000
    rnd = random.Random(seed)
    percent = rnd.randint(0, 100)

    embed = discord.Embed(
        title="Fun Ship",
        description=f"{user1.mention} x {user2.mention}\n**Compatibility:** `{percent}%`",
        color=0xFFFFFF
    )
    embed.set_footer(text="HunterX")
    file = await _render_ship_image(user1, user2, percent)
    if file:
        embed.set_image(url="attachment://ship.png")
        await ctx.send(embed=embed, file=file)
    else:
        await ctx.send(embed=embed)

@bot.hybrid_command()
async def kiss(ctx, member: discord.Member):
    await fun_action(ctx, member, "kiss", "kissed")

@bot.hybrid_command()
async def hug(ctx, member: discord.Member):
    await fun_action(ctx, member, "hug", "hugged")

@bot.hybrid_command()
async def cuddle(ctx, member: discord.Member):
    await fun_action(ctx, member, "cuddle", "cuddled")

@bot.hybrid_command()
async def slap(ctx, member: discord.Member):
    await fun_action(ctx, member, "slap", "slapped")

@bot.hybrid_command()
async def punch(ctx, member: discord.Member):
    await fun_action(ctx, member, "punch", "punched")

@bot.hybrid_command()
async def throw(ctx, member: discord.Member):
    gif_url = await fetch_gif("throw") or await fetch_gif("kick")
    desc = f"{ctx.author.mention} *threw* {member.mention}"
    embed = discord.Embed(description=desc, color=0xFFFFFF)
    embed.set_footer(text="HunterX")
    if gif_url:
        embed.set_image(url=gif_url)
    await ctx.send(embed=embed)

@bot.hybrid_command()
async def boxing(ctx, member: discord.Member):
    gif_url = await fetch_gif("punch")
    desc = f"{ctx.author.mention} *is boxing* {member.mention}"
    embed = discord.Embed(description=desc, color=0xFFFFFF)
    embed.set_footer(text="HunterX")
    if gif_url:
        embed.set_image(url=gif_url)
    await ctx.send(embed=embed)

# ==================== TIKTOK ====================
@bot.hybrid_command()
async def tt(ctx, *, username: str):
    username = username.lstrip("@").strip().replace(" ", "")
    tt_link = f"https://www.tiktok.com/@{username}"
    followers = None
    avatar = None
    headers = {
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1",
        "Referer": "https://www.tiktok.com/",
        "Accept-Language": "en-US,en;q=0.9",
    }
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://www.tiktok.com/api/user/detail/?uniqueId={username}&aid=1988&app_name=tiktok_web",
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=8)
            ) as resp:
                data = await resp.json(content_type=None)
        user_info = data.get("userInfo", {})
        user = user_info.get("user", {})
        stats = user_info.get("stats", {})
        if user:
            followers = stats.get("followerCount")
            avatar = user.get("avatarMedium", "")
    except Exception:
        pass

    embed = discord.Embed(
        description=f"**<:TiktokHunterX:1512864012969316432> TikTok**\n**Account:** [@{username}]({tt_link})",
        color=0xFFFFFF
    )
    if avatar:
        embed.set_thumbnail(url=avatar)
    if followers is not None:
        embed.add_field(name="Followers", value=_fmt_num(followers), inline=True)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== INSTAGRAM ====================
@bot.hybrid_command()
async def ig(ctx, *, username: str):
    username = username.lstrip("@").strip().replace(" ", "")
    ig_link = f"https://www.instagram.com/{username}/"
    followers = None
    avatar = None
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
        "X-IG-App-ID": "936619743392459",
        "Accept": "*/*",
        "Referer": "https://www.instagram.com/",
    }
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://www.instagram.com/api/v1/users/web_profile_info/?username={username}",
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=8)
            ) as resp:
                data = await resp.json(content_type=None)
        user = data.get("data", {}).get("user", {})
        if user:
            followers = user.get("edge_followed_by", {}).get("count")
            avatar = user.get("profile_pic_url_hd") or user.get("profile_pic_url", "")
    except Exception:
        pass

    embed = discord.Embed(
        description=f"**<:HunterXig:1515267890113413230> INSTAGRAM**\n**Account:** [@{username}]({ig_link})",
        color=0xFFFFFF
    )
    if avatar:
        embed.set_thumbnail(url=avatar)
    if followers is not None:
        embed.add_field(name="Followers", value=_fmt_num(followers), inline=True)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== YOUTUBE ====================
@bot.hybrid_command()
async def yt(ctx, *, query: str):
    await ctx.typing()
    loop = asyncio.get_event_loop()

    def _get_channel_info():
        flat_opts = {"quiet": True, "no_warnings": True, "extract_flat": True}
        with yt_dlp.YoutubeDL(flat_opts) as ydl:
            result = ydl.extract_info(f"ytsearch1:{query}", download=False)
            if not result or not result.get("entries"):
                return None
            entry = result["entries"][0]
            channel_name = entry.get("channel") or entry.get("uploader", "")
            channel_id = entry.get("channel_id", "")
            channel_url = entry.get("channel_url") or entry.get("uploader_url") or (
                f"https://www.youtube.com/channel/{channel_id}" if channel_id else ""
            )
        if not channel_url:
            return None
        chan_opts = {"quiet": True, "no_warnings": True, "extract_flat": True, "playlist_items": "0"}
        try:
            with yt_dlp.YoutubeDL(chan_opts) as ydl:
                info = ydl.extract_info(channel_url, download=False)
                subs = info.get("channel_follower_count") or info.get("subscriber_count")
                videos = info.get("playlist_count") or info.get("video_count")
                avatar = info.get("thumbnails", [{}])[-1].get("url", "") if info.get("thumbnails") else ""
                bio = (info.get("description") or "").strip()
        except Exception:
            subs = None; videos = None; avatar = ""; bio = ""
        return {"channel": channel_name, "channel_url": channel_url, "subscribers": subs, "videos": videos, "avatar": avatar, "bio": bio}

    try:
        data = await loop.run_in_executor(None, _get_channel_info)
        if not data or not data["channel"]:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> Not Found", f"No YouTube channel found for `{query}`.")
        embed = discord.Embed(
            description=f"**<:hunterXyoutube:1512864016588865787> YouTube**\n> [{data['channel']}]({data['channel_url']})",
            color=0xFFFFFF
        )
        if data["avatar"]:
            embed.set_thumbnail(url=data["avatar"])
        embed.add_field(name="Subscribers", value=_fmt_num(data["subscribers"]), inline=True)
        embed.add_field(name="Videos", value=_fmt_num(data["videos"]), inline=True)
        if data.get("bio"):
            embed.add_field(name="Bio", value=data["bio"][:300], inline=False)
        embed.set_footer(text="HunterX")
        await ctx.send(embed=embed)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== AFK ====================
@bot.hybrid_command(aliases=["a"])
async def afk(ctx, *, reason="No reason"):
    afk_users[ctx.author.id] = {"reason": reason, "time": datetime.now(PH_TIME)}
    embed = make_embed("<a:kuromihelp:1512786350170963989> AFK", f"{ctx.author.mention} is now AFK\n\nReason: {reason}")
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== ROLE ====================
@bot.hybrid_command()
@commands.has_permissions(manage_roles=True)
async def role(ctx, member: discord.Member, role: discord.Role):
    if role in member.roles:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Error", "Member already has role.")
    try:
        await member.add_roles(role)
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Role Added", f"Gave {role.mention} to {member.mention}")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", f"{e}")

# ==================== REMOVE ROLE ====================
@bot.hybrid_command()
@commands.has_permissions(manage_roles=True)
async def drole(ctx, member: discord.Member, role: discord.Role):
    if role not in member.roles:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Error", "Member does not have role.")
    try:
        await member.remove_roles(role)
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> Role Removed", f"Removed {role.mention} from {member.mention}")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", f"{e}")

# ==================== BAN ====================
@bot.hybrid_command()
@commands.has_permissions(ban_members=True)
async def ban(ctx, member: discord.Member, *, reason="No reason"):
    try:
        avatar = member.display_avatar.url
        await member.ban(reason=reason)
        embed = make_embed(
            "Banned",
            f"**User:** {member.mention}\n"
            f"**UserID:** `{member.id}`\n"
            f"**Moderator:** {ctx.author.mention}\n"
            f"**Reason:** {reason}"
        )
        embed.set_thumbnail(url=avatar)
        embed.set_footer(text="HunterX")
        await ctx.send(embed=embed)
        await log_action(ctx.guild, "Banned",
            f"**User:** {member} ({member.id})\n**Moderator:** {ctx.author} (`{ctx.author.id}`)\n**Reason:** {reason}", avatar)
        await log_mod_action(ctx.guild, "Banned",
            f"**User:** {member} (`{member.id}`)\n**Moderator:** {ctx.author} (`{ctx.author.id}`)\n**Reason:** {reason}", avatar)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== UNBAN ====================
@bot.hybrid_command()
@commands.has_permissions(ban_members=True)
async def unban(ctx, user_id: str):
    # ── UNBAN ALL ─────────────────────────────────────────────────────────
    if user_id.lower() == "all":
        bans = [entry async for entry in ctx.guild.bans()]
        if not bans:
            return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> No Bans", "There are no banned users in this server.")
        progress_embed = make_embed(
            "<a:kuromihelp:1512786350170963989> Unbanning All...",
            f"Processing **{len(bans)}** banned users. Please wait..."
        )
        progress_embed.set_footer(text="HunterX")
        msg = await ctx.send(embed=progress_embed)
        success = 0
        failed = 0
        for entry in bans:
            try:
                await ctx.guild.unban(entry.user, reason=f"Mass Unban by {ctx.author}")
                success += 1
                await asyncio.sleep(0.5)
            except:
                failed += 1
        result_embed = make_embed(
            "<:HunteXCheck:1518422536558481448> Unban All Complete",
            f"**Unbanned:** `{success}` users\n**Failed:** `{failed}` users\n**Moderator:** {ctx.author.mention}"
        )
        result_embed.set_footer(text="HunterX")
        await msg.edit(embed=result_embed)
        await log_action(ctx.guild, "<:HunteXCheck:1518422536558481448> Mass Unban",
            f"**Moderator:** {ctx.author} (`{ctx.author.id}`)\n**Unbanned:** {success} users\n**Failed:** {failed}")
        await log_mod_action(ctx.guild, "<:HunteXCheck:1518422536558481448> Mass Unban",
            f"**Moderator:** {ctx.author} (`{ctx.author.id}`)\n**Unbanned:** {success} users\n**Failed:** {failed}")
        return
    # ── UNBAN SINGLE USER ─────────────────────────────────────────────────
    try:
        uid = int(user_id)
    except ValueError:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid",
            "Usage: `$unban <user_id>` or `$unban all`")
    try:
        user = await bot.fetch_user(uid)
        await ctx.guild.unban(user)
        embed = make_embed(
            "<:HunteXCheck:1518422536558481448> Unbanned",
            f"**User:** {user.mention}\n"
            f"**UserID:** `{user.id}`\n"
            f"**Moderator:** {ctx.author.mention}"
        )
        embed.set_thumbnail(url=user.display_avatar.url)
        embed.set_footer(text="HunterX")
        await ctx.send(embed=embed)
        await log_action(ctx.guild, "Unbanned",
            f"**User:** {user} (`{user.id}`)\n**Moderator:** {ctx.author} (`{ctx.author.id}`)")
        await log_mod_action(ctx.guild, "<:HunteXCheck:1518422536558481448> Unbanned",
            f"**User:** {user} (`{user.id}`)\n**Moderator:** {ctx.author} (`{ctx.author.id}`)", user.display_avatar.url)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== BAN LIST ====================
@bot.hybrid_command()
@commands.has_permissions(ban_members=True)
async def banlist(ctx):
    bans = [entry async for entry in ctx.guild.bans()]
    if not bans:
        return await send_embed(ctx, "<:249630gradientgear:1512782705522245712> Ban List", "No banned users.")
    description = ""
    for i, entry in enumerate(bans, start=1):
        description += f"**{i}.** {entry.user} (`{entry.user.id}`)\n"
    embed = make_embed("<:249630gradientgear:1512782705522245712> Ban List", description)
    embed.set_footer(text="HunterX")
    await ctx.send(embed=embed)

# ==================== KICK ====================
@bot.hybrid_command()
@commands.has_permissions(kick_members=True)
async def kick(ctx, member: discord.Member, *, reason="No reason"):
    try:
        avatar = member.display_avatar.url
        await member.kick(reason=reason)
        embed = make_embed(
            "<:HunteXCheck:1518422536558481448> Kicked",
            f"**User:** {member.mention}\n"
            f"**UserID:** `{member.id}`\n"
            f"**Moderator:** {ctx.author.mention}\n"
            f"**Reason:** {reason}"
        )
        embed.set_thumbnail(url=member.display_avatar.url)
        embed.set_footer(text="HunterX")
        await ctx.send(embed=embed)
        await log_action(ctx.guild, "__Kicked__",
            f"**User:** {member} ({member.id})\n**Moderator:** {ctx.author} (`{ctx.author.id}`)\n**Reason:** {reason}", avatar)
        await log_mod_action(ctx.guild, "<:HunteXCheck:1518422536558481448> Member Kicked",
            f"**User:** {member} (`{member.id}`)\n**Moderator:** {ctx.author} (`{ctx.author.id}`)\n**Reason:** {reason}", avatar)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== PARSE TIME ====================
def parse_time(time_string):
    time_string = time_string.lower()
    units = {"s": 1, "sec": 1, "second": 1, "seconds": 1, "m": 60, "min": 60, "minute": 60, "minutes": 60,
             "h": 3600, "hr": 3600, "hour": 3600, "hours": 3600, "d": 86400, "day": 86400, "days": 86400,
             "w": 604800, "week": 604800, "weeks": 604800}
    total_seconds = 0
    matches = re.findall(r"(\d+)([a-zA-Z]+)", time_string)
    for amount, unit in matches:
        if unit in units:
            total_seconds += int(amount) * units[unit]
    return total_seconds

# ==================== TIMEOUT ====================
@bot.hybrid_command()
@commands.has_permissions(moderate_members=True)
async def timeout(ctx, member: discord.Member, duration: str, *, reason="No reason"):
    try:
        seconds = parse_time(duration)
        if seconds <= 0:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid Time", "Example: `1h` / `30m` / `1day` / `2weeks`")
        until = discord.utils.utcnow() + timedelta(seconds=seconds)
        await member.edit(timed_out_until=until, reason=reason)
        embed = make_embed(
            "<:HunteXCheck:1518422536558481448> Timeout",
            f"**User:** {member.mention}\n"
            f"**UserID:** `{member.id}`\n"
            f"**Moderator:** {ctx.author.mention}\n"
            f"**Duration:** `{duration}`\n"
            f"**Reason:** {reason}"
        )
        embed.set_thumbnail(url=member.display_avatar.url)
        embed.set_footer(text="HunterX")
        await ctx.send(embed=embed)
        await log_action(ctx.guild, "<:HunterXTimeout:1513168503232790668> Time Out",
            f"**User:** {member} ({member.id})\n**Moderator:** {ctx.author} (`{ctx.author.id}`)\n**Duration:** {duration}\n**Reason:** {reason}",
            member.display_avatar.url)
        await log_mod_action(ctx.guild, "<:SecureXTimeout:1513168503232790668> Timed Out",
            f"**User:** {member} (`{member.id}`)\n**Moderator:** {ctx.author} (`{ctx.author.id}`)\n**Duration:** {duration}\n**Reason:** {reason}",
            member.display_avatar.url)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== REMOVE TIMEOUT ====================
@bot.hybrid_command()
@commands.has_permissions(moderate_members=True)
async def rtimeout(ctx, member: discord.Member):
    try:
        await member.edit(timed_out_until=None)
        embed = make_embed(
            "<:HunteXCheck:1518422536558481448> Timeout Removed",
            f"**User:** {member.mention}\n"
            f"**UserID:** `{member.id}`\n"
            f"**Moderator:** {ctx.author.mention}"
        )
        embed.set_thumbnail(url=member.display_avatar.url)
        embed.set_footer(text="HunterX")
        await ctx.send(embed=embed)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

# ==================== MESSAGE HANDLER ====================
@bot.event
async def on_message(message):
    if message.author.bot:
        return

    # Bot mention → show prefix info
    if message.guild and bot.user in message.mentions:
        content_clean = message.content.replace(f"<@{bot.user.id}>", "").replace(f"<@!{bot.user.id}>", "").strip()
        if not content_clean:
            gid_m = str(message.guild.id)
            prefix = config_data.get(gid_m, {}).get("prefix", "$")
            embed = discord.Embed(
                description=f"**HunterX**\n\nHi {message.author.mention}, my prefix is `{prefix}`\nType `{prefix}help` to see commands.",
                color=0xFFFFFF
            )
            embed.set_thumbnail(url=bot.user.display_avatar.url)
            embed.set_footer(text="HunterX")
            view = InviteView().build()
            await message.channel.send(embed=embed, view=view)
            return

    gid = str(message.guild.id) if message.guild else None

    if gid:
        cfg = config_data.get(gid, {})
        lower_content = message.content.lower()

        # ==================== PARTNERSHIP / AD SERVER REQUEST (.req) ====================
        try:
            guild_int_id = message.guild.id
            user_id = message.author.id
            state = partner_requests[guild_int_id].get(user_id)
            content_stripped = lower_content.strip()

            # Start request
            if content_stripped == ".req" and not state:
                post_ch_id = cfg.get("partnership_post_channel")
                post_ch = message.guild.get_channel(int(post_ch_id)) if post_ch_id else None
                if not post_ch:
                    try:
                        await message.channel.send(
                            f"{message.author.mention} No post channel is set yet. Admins can use `pspost #channel`.",
                            delete_after=10
                        )
                    except:
                        pass
                    return
                partner_requests[guild_int_id][user_id] = {"stage": "link", "link": "", "started_in": message.channel.id}
                try:
                    await message.channel.send(
                        f"{message.author.mention} Send your **ad server invite/link** now.\nType `cancel` to cancel.",
                        delete_after=60
                    )
                except:
                    pass
                return

            # Continue request flow
            if state:
                if content_stripped in {"cancel", "stop", "exit"}:
                    partner_requests[guild_int_id].pop(user_id, None)
                    try:
                        await message.channel.send(f"{message.author.mention} Cancelled.", delete_after=6)
                    except:
                        pass
                    return

                if state.get("stage") == "link":
                    url_match = URL_PATTERN.search(message.content)
                    link = url_match.group(0) if url_match else ""
                    if not link:
                        try:
                            await message.channel.send(
                                f"{message.author.mention} I could not find a link. Send it again, or type `cancel`.",
                                delete_after=10
                            )
                        except:
                            pass
                        return
                    state["link"] = link
                    state["stage"] = "confirm"
                    try:
                        await message.channel.send(
                            f"{message.author.mention} This is your ad server: `{link}`\nType `yes` if correct, or `no` if not.",
                            delete_after=60
                        )
                    except:
                        pass
                    return

                if state.get("stage") == "confirm":
                    if content_stripped in {"yes", "y"}:
                        post_ch_id = cfg.get("partnership_post_channel")
                        post_ch = message.guild.get_channel(int(post_ch_id)) if post_ch_id else None
                        if not post_ch:
                            partner_requests[guild_int_id].pop(user_id, None)
                            try:
                                await message.channel.send(
                                    f"{message.author.mention} I couldn't find the post channel. Admins can set it again with `pspost #channel`.",
                                    delete_after=10
                                )
                            except:
                                pass
                            return
                        link = state.get("link", "")
                        embed = make_embed(
                            "Partnership Request",
                            f"**Requester:** {message.author.mention}\n**Ad Server:** {link}"
                        )
                        embed.set_footer(text="HunterX")
                        try:
                            await post_ch.send(content=message.author.mention, embed=embed)
                        except:
                            pass
                        partner_requests[guild_int_id].pop(user_id, None)
                        try:
                            await message.channel.send(f"{message.author.mention} Posted in {post_ch.mention}.", delete_after=10)
                        except:
                            pass
                        return

                    if content_stripped in {"no", "n"}:
                        state["stage"] = "link"
                        state["link"] = ""
                        try:
                            await message.channel.send(
                                f"{message.author.mention} Alright. Send your ad server link again, or type `cancel`.",
                                delete_after=60
                            )
                        except:
                            pass
                        return

                    try:
                        await message.channel.send(f"{message.author.mention} Please reply with `yes` or `no` only.", delete_after=8)
                    except:
                        pass
                    return
        except Exception:
            pass

        # Anti-Mass Mention
        if cfg.get("antimassmention", False) and message.guild:
            if (not is_whitelisted(message.guild, message.author)
                    and not message.author.guild_permissions.administrator
                    and message.author.id != message.guild.owner_id):
                threshold = int(cfg.get("mass_mention_threshold", 6))
                mention_count = len(message.mentions or [])
                if message.mention_everyone or mention_count >= threshold:
                    try:
                        await message.delete()
                    except:
                        pass
                    try:
                        await log_action(
                            message.guild,
                            "Anti-Mass Mention",
                            f"**User:** {message.author} (`{message.author.id}`)\n"
                            f"**Channel:** {message.channel.mention}\n"
                            f"**Mentions:** `{mention_count}`\n"
                            f"**Everyone/Here:** `{message.mention_everyone}`"
                        )
                    except:
                        pass
                    return

        # H!p playlist filter
        if message.guild and re.match(r"^\s*h!p\s+playlist\b", lower_content):
            playlist_url = URL_PATTERN.search(message.content)
            if not playlist_url or not is_allowed_playlist_url(playlist_url.group(0)):
                try:
                    await message.delete()
                except:
                    pass
                try:
                    await message.channel.send(
                        f"{message.author.mention} Only Spotify and YouTube links are allowed for `H!p playlist`.",
                        delete_after=6
                    )
                except:
                    pass
                return
            # Valid playlist link → play/queue it
            await _handle_hp_playlist(message, playlist_url.group(0))
            return

        # Keyword filter
        if message.guild:
            keyword_list = cfg.get("keywords", [])
            if keyword_list and not is_whitelisted(message.guild, message.author):
                matched_keyword = find_keyword_match(message.content, keyword_list)
                if matched_keyword:
                    # Need Manage Messages to delete other users' messages
                    try:
                        bot_member = message.guild.me or message.guild.get_member(bot.user.id)
                        perms = message.channel.permissions_for(bot_member) if bot_member else None
                        can_delete = bool(perms and (perms.manage_messages or perms.administrator))
                    except Exception:
                        can_delete = False

                    if not can_delete:
                        # Avoid spamming the channel with permission warnings
                        try:
                            now_ts = discord.utils.utcnow().timestamp()
                            last_ts = float(delete_permission_warn_ts.get(message.guild.id, 0))
                            if now_ts - last_ts >= 300:
                                delete_permission_warn_ts[message.guild.id] = now_ts
                                await message.channel.send(
                                    "I cannot auto-delete keyword or badword messages because I am missing the `Manage Messages` permission.",
                                    delete_after=10
                                )
                        except Exception:
                            pass
                        return
                    try:
                        await message.delete()
                        await log_action(
                            message.guild,
                            "Keyword Deleted",
                            f"**User:** {message.author} ({message.author.id})\n"
                            f"**Channel:** {message.channel.mention}\n"
                            f"**Keyword:** `{matched_keyword}`"
                        )
                    except:
                        pass
                    return

        # Anti badwords (custom list)
        if cfg.get("antibadwords", False) and message.guild:
            if not is_whitelisted(message.guild, message.author):
                matched = find_badword_match(message.content, cfg.get("badwords", []))
                if matched:
                    # Need Manage Messages to delete other users' messages
                    try:
                        bot_member = message.guild.me or message.guild.get_member(bot.user.id)
                        perms = message.channel.permissions_for(bot_member) if bot_member else None
                        can_delete = bool(perms and (perms.manage_messages or perms.administrator))
                    except Exception:
                        can_delete = False

                    if not can_delete:
                        try:
                            now_ts = discord.utils.utcnow().timestamp()
                            last_ts = float(delete_permission_warn_ts.get(message.guild.id, 0))
                            if now_ts - last_ts >= 300:
                                delete_permission_warn_ts[message.guild.id] = now_ts
                                await message.channel.send(
                                    "I cannot auto-delete keyword or badword messages because I am missing the `Manage Messages` permission.",
                                    delete_after=10
                                )
                        except Exception:
                            pass
                        return
                    try:
                        await message.delete()
                    except:
                        pass
                    try:
                        await log_action(
                            message.guild,
                            "Badword Deleted",
                            f"**User:** {message.author} (`{message.author.id}`)\n"
                            f"**Channel:** {message.channel.mention}\n"
                            f"**Badword:** `{matched}`"
                        )
                    except:
                        pass
                    return

        # Anti link
        if cfg.get("antilink", False) and message.guild:
            if re.search(r"(discord\.gg|discord\.com/invite)/\S+", message.content):
                allowed_users = cfg.get("allowed_link_users", [])
                allowed_roles = cfg.get("allowed_link_roles", [])
                legacy_role = cfg.get("allowed_link_role")
                if legacy_role:
                    allowed_roles = allowed_roles + [legacy_role]
                user_role_ids = [r.id for r in message.author.roles]
                has_allowed = (
                    message.author.id in allowed_users
                    or any(rid in user_role_ids for rid in allowed_roles)
                )
                if (not is_whitelisted(message.guild, message.author)
                        and not message.author.guild_permissions.administrator
                        and not has_allowed
                        and message.author.id != message.guild.owner_id):
                    try:
                        await message.delete()
                        await log_action(message.guild, "<:HunteXCheck:1518422536558481448> Link Deleted",
                            f"**User:** {message.author} ({message.author.id})\n**Channel:** {message.channel.mention}")
                    except:
                        pass
                    return

        # Anti Spam
        if cfg.get("antispam", False) and message.guild:
            if (not is_whitelisted(message.guild, message.author)
                    and not message.author.guild_permissions.administrator
                    and message.author.id != message.guild.owner_id):
                key = (message.guild.id, message.author.id)
                now = discord.utils.utcnow().timestamp()
                spam_tracker[key] = [t for t in spam_tracker[key] if now - t < 5]
                spam_tracker[key].append(now)
                if len(spam_tracker[key]) >= 10:
                    spam_tracker[key] = []
                    try:
                        until = discord.utils.utcnow() + timedelta(minutes=5)
                        await message.author.edit(timed_out_until=until, reason="HunterX Anti Spam")
                        await log_action(message.guild, "<:HunteXCheck:1518422536558481448> Anti Spam Timeout",
                            f"**User:** {message.author} ({message.author.id}) timed out 5min for spam.")
                    except:
                        pass
                    return

        # AFK check
        if message.guild:
            if message.author.id in afk_users:
                del afk_users[message.author.id]
                try:
                    await message.channel.send(
                        embed=discord.Embed(
                            description=f"Welcome back {message.author.mention}! AFK removed.",
                            color=0xFFFFFF
                        ).set_footer(text="HunterX"),
                        delete_after=5
                    )
                except:
                    pass
            for mentioned in message.mentions:
                if mentioned.id in afk_users:
                    data = afk_users[mentioned.id]
                    reason = data.get("reason", "No reason")
                    try:
                        await message.channel.send(
                            embed=discord.Embed(
                                description=f"{mentioned.mention} is currently AFK\nReason: {reason}",
                                color=0xFFFFFF
                            ).set_footer(text="HunterX"),
                            delete_after=8
                        )
                    except:
                        pass

        # Auto respond
        if gid and message.guild:
            triggers = autorespond_data.get(gid, {})
            for trigger, reply in triggers.items():
                if trigger in lower_content:
                    try:
                        await message.channel.send(reply)
                    except:
                        pass
                    break

        # Leveling XP (optional)
        if cfg.get("leveling_enabled", False) and message.guild:
            # Avoid giving XP to obvious commands
            content_strip = message.content.strip()
            looks_like_cmd = (
                content_strip.startswith(tuple(FIXED_PREFIXES))
                or content_strip.lower().startswith("h!")
                or content_strip.startswith(".")
            )
            if not looks_like_cmd:
                try:
                    user_level = get_user_level(message.guild.id, message.author.id)
                    cooldown = int(cfg.get("leveling_cooldown", 60))
                    now_ts = time.time()
                    last_ts = float(user_level.get("last_xp", 0) or 0)
                    if now_ts - last_ts >= cooldown:
                        gained = random.randint(5, 15)
                        user_level["xp"] = int(user_level.get("xp", 0)) + gained
                        user_level["last_xp"] = now_ts
                        new_level = get_level_from_xp(int(user_level["xp"]))
                        if new_level > int(user_level.get("level", 0)):
                            user_level["level"] = new_level
                            announce_id = cfg.get("leveling_channel")
                            announce_ch = message.guild.get_channel(int(announce_id)) if announce_id else message.channel
                            try:
                                await announce_ch.send(f"{message.author.mention} leveled up to **Level {new_level}**!")
                            except:
                                pass
                        save_levels()
                except Exception:
                    pass

        # Sticky message resend
        if message.guild:
            channel_id = message.channel.id
            if channel_id in sticky_messages:
                old = sticky_messages[channel_id]
                try:
                    old_msg = await message.channel.fetch_message(old["msg_id"])
                    await old_msg.delete()
                except:
                    pass
                sent = await message.channel.send(f"{old['text']}")
                sticky_messages[channel_id] = {"text": old["text"], "msg_id": sent.id}

    await bot.process_commands(message)

# ==================== MUSIC REACTION CONTROLS ====================
@bot.event
async def on_reaction_add(reaction: discord.Reaction, user: discord.User):
    if user.bot:
        return
    msg = reaction.message
    if not msg or not msg.guild:
        return
    control = music_control_messages.get(msg.guild.id)
    if not control or int(control.get("message_id", 0)) != msg.id:
        return
    emoji = str(reaction.emoji)
    if emoji not in MUSIC_CONTROL_EMOJIS.values():
        return

    vc = msg.guild.voice_client
    member = msg.guild.get_member(user.id)
    if not vc or not vc.is_connected() or not member or not _can_control_music(member, vc):
        try:
            await reaction.remove(user)
        except:
            pass
        return

    # Remove reaction to keep it reusable
    try:
        await reaction.remove(user)
    except:
        pass

    try:
        if emoji == MUSIC_CONTROL_EMOJIS["pause"]:
            if vc.is_playing():
                vc.pause()
        elif emoji == MUSIC_CONTROL_EMOJIS["resume"]:
            if vc.is_paused():
                vc.resume()
        elif emoji == MUSIC_CONTROL_EMOJIS["skip"]:
            vc.stop()
        elif emoji == MUSIC_CONTROL_EMOJIS["stop"]:
            music_queues[msg.guild.id].clear()
            vc.stop()
    except Exception:
        pass

# ==================== MESSAGE EDIT LOG ====================
@bot.event
async def on_message_edit(before, after):
    if before.author.bot:
        return
    if before.content == after.content:
        return
    if not before.guild:
        return
    gid = str(before.guild.id)
    ch_id = config_data.get(gid, {}).get("editmsg_channel")
    if not ch_id:
        return
    log_ch = before.guild.get_channel(ch_id)
    if not log_ch:
        return
    try:
        embed = make_log_embed("Message Edited")
        embed.set_author(name=f"{before.author} | {before.author.id}", icon_url=before.author.display_avatar.url)
        embed.add_field(name="Channel", value=before.channel.mention, inline=True)
        embed.add_field(name="Before", value=before.content[:1024] if before.content else "[No text]", inline=False)
        embed.add_field(name="After", value=after.content[:1024] if after.content else "[No text]", inline=False)
        embed.set_footer(text="HunterX")
        embed.timestamp = datetime.now(PH_TIME)
        await log_ch.send(embed=embed)
        all_id = config_data.get(gid, {}).get("all_logs_channel")
        if all_id:
            all_ch = before.guild.get_channel(int(all_id))
            if all_ch and all_ch.id != log_ch.id:
                await all_ch.send(embed=embed)
    except:
        pass

# ==================== MESSAGE DELETE (Snipe + Delete Log) ====================
@bot.event
async def on_message_delete(message):
    if message.author.bot:
        return

    # Store for $snipe
    if message.content:
        snipe_data[message.channel.id] = {
            "content": message.content,
            "author": str(message.author),
            "author_icon": str(message.author.display_avatar.url),
            "time": datetime.now(PH_TIME),
        }

    # Delete message log
    if not message.guild:
        return
    gid = str(message.guild.id)
    ch_id = config_data.get(gid, {}).get("deletemsg_channel")
    if not ch_id:
        return
    log_ch = message.guild.get_channel(ch_id)
    if not log_ch:
        return
    try:
        embed = make_log_embed("Message Deleted")
        embed.set_author(name=f"{message.author} | {message.author.id}", icon_url=message.author.display_avatar.url)
        embed.add_field(name="Channel", value=message.channel.mention, inline=True)
        embed.add_field(name="Content", value=message.content[:1024] if message.content else "[No text content]", inline=False)
        embed.set_footer(text="HunterX")
        embed.timestamp = datetime.now(PH_TIME)
        await log_ch.send(embed=embed)
        all_id = config_data.get(gid, {}).get("all_logs_channel")
        if all_id:
            all_ch = message.guild.get_channel(int(all_id))
            if all_ch and all_ch.id != log_ch.id:
                await all_ch.send(embed=embed)
    except:
        pass

# ==================== WELCOME EVENT ====================
@bot.event
async def on_member_join(member):
    if member.bot:
        await _check_bot_add(member)
        return
    guild = member.guild
    gid = str(guild.id)
    cfg = config_data.get(gid, {})

    # Anti-Mass Raid: if more than 15 joins happen within 5 seconds, kick the latest 5 immediately.
    if cfg.get("antimassraid", False):
        try:
            dq = join_tracker[guild.id]
            now_ts = time.time()
            dq.append((now_ts, member.id))
            while dq and (now_ts - dq[0][0] > 5):
                dq.popleft()

            if len(dq) > 15:
                to_kick_ids = [mid for _, mid in list(dq)[-5:]]
                kicked = 0
                for mid in to_kick_ids:
                    m = guild.get_member(mid)
                    if not m:
                        continue
                    if m.guild_permissions.administrator or m.id == guild.owner_id:
                        continue
                    if is_whitelisted(guild, m):
                        continue
                    try:
                        await m.kick(reason="HunterX Anti-Mass Raid")
                        kicked += 1
                    except:
                        pass
                await log_action(
                    guild,
                    "Anti-Mass Raid Triggered",
                    f"**Join burst:** `{len(dq)}` joins / 5s\n**Kicked:** `{kicked}` member(s)"
                )
        except:
            pass

    autorole_id = cfg.get("autorole")
    if autorole_id:
        autorole = guild.get_role(autorole_id)
        if autorole:
            try:
                await member.add_roles(autorole, reason="HunterX Auto Role")
            except:
                pass

    welcome_ch_id = cfg.get("welcome_channel")
    if welcome_ch_id:
        channel = guild.get_channel(welcome_ch_id)
        if channel:
            await send_welcome_embed(guild, member, channel)

# ==================== VC LOGS EVENT ====================
@bot.event
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    if member.bot or not member.guild:
        return
    guild = member.guild
    gid = str(guild.id)
    cfg = config_data.get(gid, {})

    vclogs_id = cfg.get("vclogs_channel")
    all_logs_id = cfg.get("all_logs_channel")
    if not vclogs_id and not all_logs_id:
        return

    # Enforce 10-user limit for channels named like "fun ship"
    try:
        ch = after.channel
        if ch and isinstance(ch, discord.VoiceChannel):
            name = (ch.name or "").lower()
            if "funship" in name or "fun ship" in name:
                if not ch.user_limit or ch.user_limit != 10:
                    await ch.edit(user_limit=10, reason="HunterX Fun Ship limit (10)")
    except:
        pass

    # Build log message
    action = None
    if before.channel is None and after.channel is not None:
        action = f"**Joined:** {after.channel.mention}"
    elif before.channel is not None and after.channel is None:
        action = f"**Left:** {before.channel.mention}"
    elif before.channel is not None and after.channel is not None and before.channel.id != after.channel.id:
        action = f"**Moved:** {before.channel.mention} ➜ {after.channel.mention}"

    if not action:
        return

    embed = make_log_embed("Voice Logs", f"**User:** {member} (`{member.id}`)\n{action}")
    embed.set_footer(text="HunterX")
    embed.timestamp = datetime.now(PH_TIME)

    try:
        if vclogs_id:
            vc_ch = guild.get_channel(int(vclogs_id))
            if vc_ch:
                await vc_ch.send(embed=embed)
        if all_logs_id:
            all_ch = guild.get_channel(int(all_logs_id))
            if all_ch and (not vclogs_id or all_ch.id != int(vclogs_id)):
                await all_ch.send(embed=embed)
    except:
        pass

# ==================== BOOST EVENT ====================
@bot.event
async def on_member_update(before, after):
    guild = after.guild
    gid = str(guild.id)
    cfg = config_data.get(gid, {})

    if before.premium_since is None and after.premium_since is not None:
        boost_role_id = cfg.get("boost_role")
        if boost_role_id:
            boost_role = guild.get_role(boost_role_id)
            if boost_role and boost_role not in after.roles:
                try:
                    await after.add_roles(boost_role, reason="HunterX Boost Auto Role")
                except:
                    pass

        boost_ch_id = cfg.get("boost_channel")
        if boost_ch_id:
            boost_ch = guild.get_channel(boost_ch_id)
            if boost_ch:
                boost_msg = cfg.get("boost_message", "")
                boost_role_id = cfg.get("boost_role")
                boost_role_mention = f"<@&{boost_role_id}>" if boost_role_id else "@Booster"
                embed = discord.Embed(color=0xFF69B4)
                embed.add_field(name="Booster", value=after.name, inline=True)
                embed.add_field(name="Role", value=boost_role_mention, inline=True)
                if boost_msg:
                    formatted = boost_msg.replace("{user}", after.mention)
                    embed.description = f"-# {formatted}"
                embed.set_thumbnail(url=after.display_avatar.url)
                embed.set_footer(text="HunterX")
                try:
                    await boost_ch.send(embed=embed)
                except:
                    pass

    if before.premium_since is not None and after.premium_since is None:
        boost_role_id = cfg.get("boost_role")
        if boost_role_id:
            boost_role = guild.get_role(boost_role_id)
            if boost_role and boost_role in after.roles:
                try:
                    await after.remove_roles(boost_role, reason="HunterX Boost Ended")
                except:
                    pass

    # Anti Nickname (reset bad nicknames)
    if cfg.get("antinickname", False):
        try:
            if before.display_name != after.display_name:
                if (not is_whitelisted(guild, after)
                        and not after.guild_permissions.administrator
                        and after.id != guild.owner_id):
                    if is_bad_nickname(after.display_name):
                        try:
                            await after.edit(nick=None, reason="HunterX Anti Nickname")
                        except:
                            # fallback: set to username
                            try:
                                await after.edit(nick=after.name[:32], reason="HunterX Anti Nickname")
                            except:
                                pass
                        await log_action(
                            guild,
                            "Anti Nickname",
                            f"**User:** {after} (`{after.id}`)\n**Nickname reset:** bad nickname detected."
                        )
        except:
            pass

# ==================== ANTI BOT ADD ====================
async def _check_bot_add(member):
    guild = member.guild
    gid = str(guild.id)
    if not config_data.get(gid, {}).get("antibotadd", False):
        return
    try:
        async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
            user = entry.user
            if is_guild_owner_or_bot(guild, user):
                return
            if is_higher_role_than_bot(guild, user):
                return
            if is_whitelisted(guild, user):
                return
            try:
                await member.ban(reason="HunterX Anti Bot Add")
            except:
                pass
            try:
                await guild.ban(user, reason="HunterX Anti Bot Add")
            except:
                pass
            await log_action(guild, "<:staff1:1512771763199545394> Unauthorized Bot Add",
                f"**Banned Adder:** {user} (`{user.id}`)\n**Banned Bot:** {member}",
                user.display_avatar.url)
            break
    except:
        pass

# ==================== ANTI ROLE CREATE ====================
@bot.event
async def on_guild_role_create(role):
    guild = role.guild
    gid = str(guild.id)
    cfg = config_data.get(gid, {})

    try:
        async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.role_create):
            user = entry.user

            if is_guild_owner_or_bot(guild, user):
                return
            if is_higher_role_than_bot(guild, user):
                return
            if is_whitelisted(guild, user):
                return

            if cfg.get("antinuke", False):
                try:
                    await role.delete()
                except:
                    pass
                try:
                    await guild.ban(user, reason="HunterX Anti Role Create")
                except:
                    pass
                await log_action(guild, "Unauthorized Role Create",
                    f"**Banned:** {user} (`{user.id}`)\n**Role:** {role.name}",
                    user.display_avatar.url)
                return

            if cfg.get("anticreateadminrole", False) and role.permissions.administrator:
                try:
                    await role.delete()
                except:
                    pass
                try:
                    await guild.ban(user, reason="HunterX Anti Create Admin Role")
                except:
                    pass
                await log_action(guild, "Unauthorized Admin Role Created",
                    f"**Banned:** {user} (`{user.id}`)\n**Role:** {role.name} (had Administrator perms)",
                    user.display_avatar.url)
            break
    except:
        pass

# ==================== ANTI ROLE DELETE ====================
@bot.event
async def on_guild_role_delete(role):
    guild = role.guild
    gid = str(guild.id)
    cfg = config_data.get(gid, {})
    if not is_antinuke_enabled(guild.id) and not cfg.get("antideleterole", False):
        return
    try:
        async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.role_delete):
            user = entry.user
            if is_guild_owner_or_bot(guild, user):
                return
            if is_higher_role_than_bot(guild, user):
                return
            if is_whitelisted(guild, user):
                return
            try:
                await guild.ban(user, reason="HunterX Anti Role Delete")
            except:
                pass
            await log_action(guild, "Unauthorized Role Delete",
                f"**Banned:** {user} (`{user.id}`)\n**Role:** {role.name}",
                user.display_avatar.url)
            break
    except:
        pass

# ==================== ANTI CHANNEL CREATE ====================
@bot.event
async def on_guild_channel_create(channel):
    guild = channel.guild
    gid = str(guild.id)
    cfg = config_data.get(gid, {})

    hunterx_channels = ("hunterx-logs", "hunterx-verify", "hunterx-support", "hunterx-ticketlogs",
                        "deletemessage-logs", "rich", "hunterx-dltmsglogs", "hunterx-editmsglogs", "hunterx-modlogs")
    if channel.name in hunterx_channels:
        return
    if channel.category and channel.category.name == "helpdesk":
        return

    if not cfg.get("antinuke", False) and not cfg.get("anticreatechannel", False):
        return

    try:
        async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_create):
            user = entry.user
            if is_guild_owner_or_bot(guild, user):
                return
            if is_higher_role_than_bot(guild, user):
                return
            if is_whitelisted(guild, user):
                return
            try:
                await channel.delete()
            except:
                pass
            try:
                await guild.ban(user, reason="HunterX Anti Channel Create")
            except:
                pass
            await log_action(guild, "Unauthorized Channel Create",
                f"**Banned:** {user} (`{user.id}`)\n**Channel:** {channel.name}",
                user.display_avatar.url)
            break
    except:
        pass

# ==================== ANTI CHANNEL DELETE ====================
@bot.event
async def on_guild_channel_delete(channel):
    guild = channel.guild
    gid = str(guild.id)
    cfg = config_data.get(gid, {})

    if not cfg.get("antinuke", False) and not cfg.get("antideletechannel", False):
        return

    try:
        async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_delete):
            user = entry.user
            if is_guild_owner_or_bot(guild, user):
                return
            if is_higher_role_than_bot(guild, user):
                return
            if is_whitelisted(guild, user):
                return
            try:
                await guild.ban(user, reason="HunterX Anti Channel Delete")
            except:
                pass
            await log_action(guild, "Unauthorized Channel Delete",
                f"**Banned:** {user} (`{user.id}`)\n**Channel:** {channel.name}",
                user.display_avatar.url)
            break
    except:
        pass

# ==================== ANTI MEMBER KICK ====================
@bot.event
async def on_member_remove(member):
    guild = member.guild
    gid = str(guild.id)
    cfg = config_data.get(gid, {})

    # Anti-kick enforcement (optional)
    if cfg.get("antikick", False):
        try:
            async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.kick):
                if not entry.target or entry.target.id != member.id:
                    continue
                user = entry.user
                if is_guild_owner_or_bot(guild, user):
                    break
                if is_higher_role_than_bot(guild, user):
                    break
                if is_whitelisted(guild, user):
                    break
                try:
                    await guild.ban(user, reason="HunterX Anti Member Kick")
                except:
                    pass
                await log_action(guild, "Unauthorized Member Kick",
                    f"**Banned:** {user} (`{user.id}`)\n**Victim:** {member}",
                    user.display_avatar.url if user else None)
                break
        except:
            pass

    # Goodbye / Leave logs (optional)
    goodbye_ch_id = cfg.get("goodbye_channel")
    if goodbye_ch_id:
        ch = guild.get_channel(int(goodbye_ch_id)) if str(goodbye_ch_id).isdigit() else None
        if ch:
            try:
                now = datetime.now(PH_TIME)
                joined = member.joined_at
                if joined and joined.tzinfo is None:
                    joined = joined.replace(tzinfo=PH_TIME)
                duration = _format_duration(now - joined) if joined else "Unknown"
                roles = [r.name for r in member.roles if r.name != "@everyone"]
                roles_str = ", ".join(roles) if roles else "None"

                embed = discord.Embed(title="Member Leave", color=0xFFFFFF)
                embed.add_field(name="Name", value=str(member), inline=False)
                embed.add_field(name="Roles", value=roles_str[:1024], inline=False)
                embed.add_field(name="Account", value=duration, inline=False)

                leave_msg = (cfg.get("goodbye_message") or "").strip()
                if leave_msg:
                    formatted = (leave_msg
                                .replace("{user}", member.mention)
                                .replace("{name}", member.display_name)
                                .replace("{server}", guild.name))
                    embed.add_field(name="Message", value=formatted[:1024], inline=False)

                embed.set_thumbnail(url=member.display_avatar.url)
                embed.set_footer(text="HunterX")
                embed.timestamp = now

                # Background image (custom or black default)
                bg_url = (cfg.get("leave_bg") or "").strip() or None
                file = await _render_leave_image(member, bg_url)
                if file:
                    embed.set_image(url="attachment://leave.png")
                    await ch.send(embed=embed, file=file)
                else:
                    await ch.send(embed=embed)
            except:
                pass

# ==================== ANTI BAN EVENT ====================
@bot.event
async def on_member_ban(guild, user):
    gid = str(guild.id)
    if not config_data.get(gid, {}).get("antiban", False):
        return
    try:
        async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
            if entry.target.id != user.id:
                continue
            moderator = entry.user
            if is_guild_owner_or_bot(guild, moderator):
                return
            if is_higher_role_than_bot(guild, moderator):
                return
            if is_whitelisted(guild, moderator):
                return
            try:
                await guild.ban(moderator, reason="HunterX Anti Ban")
            except:
                pass
            await log_action(guild, "Unauthorized Member Ban",
                f"**Banned:** {moderator} ({moderator.id})\n**Victim:** {user}",
                moderator.display_avatar.url)
            break
    except:
        pass

# ==================== ANTI WEBHOOK CREATE ====================
@bot.event
async def on_webhooks_update(channel):
    guild = channel.guild
    gid = str(guild.id)
    if not config_data.get(gid, {}).get("antiwebhookcreate", False):
        return
    try:
        async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.webhook_create):
            user = entry.user
            if is_guild_owner_or_bot(guild, user):
                return
            if is_higher_role_than_bot(guild, user):
                return
            if is_whitelisted(guild, user):
                return
            try:
                webhook = entry.target
                if hasattr(webhook, "delete"):
                    await webhook.delete()
            except:
                pass
            try:
                await guild.ban(user, reason="HunterX Anti Webhook Create")
            except:
                pass
            await log_action(guild, "Unauthorized Webhook Created",
                f"**Banned:** {user} (`{user.id}`)\n**Channel:** {channel.mention}",
                user.display_avatar.url)
            break
    except:
        pass


# ==================== WEB SERVER (keep-alive) ====================
from aiohttp import web as aio_web

async def start_web_server():
    async def handle(request):
        return aio_web.Response(text="HunterX is online!")
    app = aio_web.Application()
    app.router.add_get("/", handle)
    runner = aio_web.AppRunner(app)
    await runner.setup()
    port = int(os.environ.get("PORT", 8080))
    site = aio_web.TCPSite(runner, "0.0.0.0", port)
    await site.start()

# ==================== RUN ====================
token = os.environ.get("DISCORD_TOKEN")
if not token:
    raise RuntimeError("DISCORD_TOKEN is not set. Please add it to your Render Environment Variables.")

async def main():
    await start_web_server()
    await bot.start(token)

asyncio.run(main())
