...
async def keepalive_loop():
    """Ping own web server every 4 minutes to keep Replit container alive."""
...
async def setup(ctx):
    guild = ctx.guild
    try:
        existing = discord.utils.get(guild.text_channels, name="hunterx-logs")
        if existing:
            log_channels[str(guild.id)] = existing.id
        ...
        save_data(LOGS_FILE, log_channels)
        setup_guilds.add(guild.id)
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Setup Successful", f"{channel.mention} created successfully.")
    except Exception as e:
        await ctx.send(f"```{e}```")
...
async def antilink(ctx, action: str):
    ...
    if action.lower() == "on":
        ...
        save_config()
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> AntiLink ON", "Discord invite links will be auto deleted.")
    elif action.lower() == "off":
        config_data[gid]["antilink"] = False
        save_config()
        await send_embed(ctx, "<:HunterXsadkitty:1514606490063868074> AntiLink OFF", "Discord invite links will NOT be deleted.")
    else:
        await send_embed(ctx, "Invalid", "Usage: `$antilink on/off`")
...
async def config_cmd(ctx):
    ...
    def status(val, on="Enabled", off="Disabled"):
        return f"<:HunteXCheck:1518422536558481448> {on}" if val else f"<:HunterX:1512787796635422934> {off}"
    ...
    not_set = "Not Set"
    embed = make_embed(
        "<:249630gradientgear:1512782705522245712> Server Configuration",
        f"**Anti-Nuke Settings**\n"
        f"AntiNuke: {status(cfg.get('antinuke', False))}  "
        f"AntiKick: {status(cfg.get('antikick', False))}  "
        f"AntiBan: {status(cfg.get('antiban', False))}\n"
        f"AntiBotAdd: {status(cfg.get('antibotadd', False))}  "
        f"AntiAdminRole: {status(cfg.get('anticreateadminrole', False))}  "
        f"AntiWebhook: {status(cfg.get('antiwebhookcreate', False))}\n"
        f"AntiLink: {status(cfg.get('antilink', False), 'ON', 'OFF')}  "
        f"AntiSpam: {status(cfg.get('antispam', False))}  "
        f"AntiCrtChannel: {status(cfg.get('anticreatechannel', False))}\n"
        f"AntiDelChannel: {status(cfg.get('antideletechannel', False))}  "
        f"AntiDelRole: {status(cfg.get('antideleterole', False))}\n\n"
        f"**Channels**\n"
...
if not token:
    raise RuntimeError("DISCORD_TOKEN is not set. Please add it to your Replit Secrets.")
