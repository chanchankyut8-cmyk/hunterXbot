...
async def jvc(ctx):
    if not ctx.author.voice or not ctx.author.voice.channel:
        return await send_embed(ctx, "No Voice Channel", "Kailangan nasa voice channel ka muna bago gamitin ang `jvc`.")
    ...
    try:
        if voice_client and voice_client.is_connected():
            if voice_client.channel.id == target_channel.id:
                return await send_embed(ctx, "Already Connected", f"Nasa {target_channel.mention} na ako.")
            await voice_client.move_to(target_channel)
            return await send_embed(ctx, "Moved Voice Channel", f"Lumipat ako sa {target_channel.mention}.")

        await target_channel.connect()
        await send_embed(ctx, "Joined Voice Channel", f"Pumasok ako sa {target_channel.mention}.")
    except Exception as e:
...
async def dvc(ctx):
    ...
    if not voice_client or not voice_client.is_connected():
        return await send_embed(ctx, "Not Connected", "Wala ako sa kahit anong voice channel ngayon.")

    try:
        channel_name = voice_client.channel.mention if getattr(voice_client, "channel", None) else "voice channel"
        await voice_client.disconnect(force=True)
        await send_embed(ctx, "Disconnected", f"Umalis ako sa {channel_name}.")
    except Exception as e:
        await send_embed(ctx, "Error", str(e))

@bot.hybrid_command(name="hcf", aliases=["coinflip"])
@discord.app_commands.describe(side="Pumili ng h/head o t/tails")
...
async def hcf(ctx, side: str):
    ...
    if not normalized:
        return await send_embed(ctx, "Invalid", "Gamitin mo `h`, `t`, `head`, o `tails`.")
    ...
    description = (
        ...
        f"**Status:** {'Panalo ka.' if won else 'Talo ka.'}"
    )
        ...
        try:
            ...
            if now < next_claim:
                ...
                return await send_embed(
                    ctx,
                    "Daily Cooldown",
                    f"Nakuha mo na ang daily mo.\nBalik ka after `{hours}h {minutes}m {seconds}s`."
                )
        except Exception:
            pass
