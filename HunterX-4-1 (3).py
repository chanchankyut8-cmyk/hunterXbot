...
def patch_discord_embed_text():
    ...
    def clean_init(self, *args, **kwargs):
        if kwargs.get("title"):
            kwargs["title"] = strip_emojis(kwargs["title"])
        if kwargs.get("description"):
            kwargs["description"] = strip_emojis(kwargs["description"])
        return original_init(self, *args, **kwargs)

    def clean_add_field(self, *, name, value, inline=True):
        return original_add_field(
            self,
            name=strip_emojis(str(name)),
            value=strip_emojis(str(value)),
            inline=inline
        )

    def clean_set_author(self, *, name, url=None, icon_url=None):
        return original_set_author(self, name=strip_emojis(str(name)), url=url, icon_url=icon_url)

    def clean_set_footer(self, *, text=None, icon_url=None):
        cleaned_text = strip_emojis(str(text)) if text is not None else None
...
# ==================== ANTINUKE ====================
ON_OFF_CHOICES = [
    discord.app_commands.Choice(name="ON", value="enable"),
    discord.app_commands.Choice(name="OFF", value="disable"),
]
...
@discord.app_commands.describe(side="Choose h/head or t/tails")
@discord.app_commands.choices(side=COINFLIP_CHOICES)
async def hcf(ctx, side: str):
    ...
    description = (
        ...
        f"**Status:** {'You won.' if won else 'You lost.'}"
    )
