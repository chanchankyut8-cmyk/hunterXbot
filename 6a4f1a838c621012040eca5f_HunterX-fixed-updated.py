...
async def _play_next(guild: discord.Guild, text_channel: discord.TextChannel = None):
    ...
    if not info:
        if text_channel:
            try:
                await text_channel.send(f"Di ko ma-play yung track. Skipping...")
            except:
                pass
        return await _play_next(guild, text_channel=text_channel)
...
async def _handle_hp_playlist(message: discord.Message, playlist_url: str):
    ...
    if not message.author.voice or not message.author.voice.channel:
        try:
            await message.channel.send(f"{message.author.mention} Join muna sa voice channel bago mag `h!p playlist`.", delete_after=6)
        except:
            pass
        return
    ...
    async with music_locks[guild.id]:
        ...
        try:
            ...
            try:
                await message.channel.send("Hindi ako maka-connect sa voice. Check permissions.", delete_after=6)
            except:
                pass
            return
        ...
        if added <= 0:
            try:
                await message.channel.send("Walang nakuha na playlist tracks. Baka private/blocked yung link.", delete_after=8)
            except:
                pass
            return
...
# ==================== ECONOMY SHOP ====================
# You said “ikaw na bahala” sa items; you can edit these any time.
SHOP_ITEMS = {
    "vip": {"name": "VIP Pass", "price": 15000, "desc": "Flex lang. Walang perks (for now)."},
...
class HelpPaginatorView(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message("Ikaw lang ang puwedeng gumamit ng help buttons na ito.", ephemeral=True)
            return False
        return True
...
# ==================== SET SERVER ICON ====================
@bot.hybrid_command(name="setsvpfp", aliases=["setsvicon", "spfp"])
...
# ==================== SET SERVER BANNER ====================
@bot.hybrid_command(aliases=["sbnr"])
@commands.has_permissions(manage_guild=True)
...
@bot.hybrid_command(name="setpfpbot", with_app_command=False)
async def setpfpbot(ctx, mode: str = None):
    ...
    if not ctx.message.attachments:
        return await send_embed(ctx, "Usage", "`setpfpbot [image]` or `setpfpbot serveronly` with an attached image.")
    attachment = ctx.message.attachments[0]
    ...
    try:
        ...
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Bot PFP Updated", f"Bot profile picture updated successfully. Mode: `{scope}`")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

@bot.hybrid_command(name="setbnrbot", with_app_command=False)
async def setbnrbot(ctx, mode: str = None):
    ...
    if not ctx.message.attachments:
        return await send_embed(ctx, "Usage", "`setbnrbot [image]` or `setbnrbot serveronly` with an attached image.")
    attachment = ctx.message.attachments[0]
    if not _valid_image_attachment(attachment):
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid", "Please attach a valid image (PNG, JPG, GIF, WEBP).")
    try:
        ...
        scope = "serveronly" if mode_value == "serveronly" else "global"
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Bot Banner Updated", f"Bot banner updated successfully. Mode: `{scope}`")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))
...
async def verification(ctx, action: str = "setup"):
    ...
    try:
        ...
        # These channels should STAY hidden kahit verified na (admin/log channels).
        protected_names = {
            "hunterx-logs",
            "hunterx-modlogs",
            "hunterx-dltmsglogs",
...
@bot.hybrid_command(name="pspost", with_app_command=False)
@commands.has_permissions(administrator=True)
...
@bot.hybrid_command(name="autoreact", aliases=["musicreact"])
@commands.has_permissions(administrator=True)
...
@bot.hybrid_command(name="leveltop", aliases=["levelboard", "toplevels"])
...
# ==================== ALL LOGS ====================
@bot.hybrid_command(name="all-logs", aliases=["alllogs"])
@commands.has_permissions(administrator=True)
...
# ==================== GOODBYE SETUP ====================
@bot.hybrid_command(name="setup-goodbye", aliases=["setupgoodbye"])
@commands.has_permissions(administrator=True)
async def setup_goodbye(ctx):
    ...
    try:
        ...
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Goodbye Setup",
            f"{ch.mention} created. Dito maglologs yung mga nagle-leave.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

@bot.hybrid_command(with_app_command=False, name="goodbye")
@commands.has_permissions(administrator=True)
async def goodbye_msg(ctx, *, message: str = None):
    """Set goodbye message. Use `off` para walang message sa embed."""
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
    if message is None:
        current = (cfg.get("goodbye_message") or "").strip()
        if not current:
            return await send_embed(ctx, "Goodbye Message", "Walang naka-set na goodbye message.")
        return await send_embed(ctx, "Goodbye Message", f"Current:\n{current}")
    msg = message.strip()
    if msg.lower() in {"off", "disable", "none", "remove"}:
        cfg["goodbye_message"] = ""
        save_config()
        return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Goodbye Message", "Tinanggal na yung goodbye message.")
    cfg["goodbye_message"] = msg
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Goodbye Message", "Goodbye message updated.")

@bot.hybrid_command(with_app_command=False, name="setleavebg")
@commands.has_permissions(administrator=True)
async def setleavebg(ctx, url: str = None):
    """Set leave background image. Attach an image OR pass URL. Use `reset` para black default."""
    gid = str(ctx.guild.id)
    cfg = config_data.setdefault(gid, {})
...
class SetupSVMainView(View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.author_id:
            await interaction.response.send_message("Ikaw lang ang puwedeng gumamit nito.", ephemeral=True)
            return False
        return True
...
class SetupSVTextView(View):
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("Text channels created (skip kung existing na).", ephemeral=True)
        except Exception as e:
...
class SetupSVVoiceView(View):
    ...
    async def _on_select(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("Voice channels created (skip kung existing na).", ephemeral=True)
        except Exception as e:
            await interaction.followup.send(f"Error: {e}", ephemeral=True)
...
@bot.hybrid_command(name="setupsv", with_app_command=False)
@commands.has_permissions(administrator=True)
...
async def h_stop(ctx):
    ...
    if not vc or not vc.is_connected():
        return await send_embed(ctx, "Music", "Wala akong voice connection ngayon.")
    if not _can_control_music(ctx.author, vc):
        return await send_embed(ctx, "Music", "Join ka muna sa same voice channel para mag-control.")
    try:
...
@bot.hybrid_command(name="hcash", aliases=["balance"])
...
@bot.hybrid_command(name="hdaily", aliases=["daily"])
...
# ==================== ECONOMY (EXTRA) ====================
@bot.hybrid_command(name="hshop", aliases=["shop"])
async def hshop(ctx):
    lines = []
...
@bot.hybrid_command(name="hbuy", aliases=["buy"])
...
async def hgive(ctx, member: discord.Member = None, amount: str = None):
    ...
    if member.id == ctx.author.id:
        return await send_embed(ctx, "Invalid", "Hindi mo puwedeng bigyan yung sarili mo.")
    if not amount:
        return await send_embed(ctx, "Usage", "`hgive @user <amount>`")
    try:
    ...
    if int(sender.get("cash", 0)) < amt:
        return await send_embed(ctx, "Not Enough Cash", "Wala kang sapat na cash.")
...
@bot.hybrid_command(name="hslot", aliases=["slot"])
    ...
    if last_raw:
        try:
            ...
            if now < last + timedelta(minutes=10):
                ...
                return await send_embed(ctx, "Rob Cooldown", f"Wait `{mins} minute(s)` bago ulit mag-rob.")
        except Exception:
            pass
...
@bot.hybrid_command(name="htopcash", aliases=["topcash", "leaderboard"])
async def htopcash(ctx):
    ...
    top = rows[:10]
    if not top:
        return await send_embed(ctx, "Top Cash", "Walang data.")
    lines = []
...
# ==================== FUN SHIP ====================
@bot.hybrid_command(name="funship", aliases=["ship"])
    ...
    if gid:
        ...
        try:
            ...
            if content_stripped == ".req" and not state:
                ...
                if not post_ch:
                    try:
                        await message.channel.send(
                            f"{message.author.mention} Wala pang naka-set na post channel. Admin: gamitin `pspost #channel`.",
                            delete_after=10
                        )
                    except:
                        pass
                ...
                try:
                    await message.channel.send(
                        f"{message.author.mention} Send mo yung **ad server invite/link** mo ngayon.\nType `cancel` para i-cancel.",
                        delete_after=60
                    )
                except:
                    pass
            ...
            if state:
                ...
                if state.get("stage") == "link":
                    ...
                    if not link:
                        try:
                            await message.channel.send(
                                f"{message.author.mention} Wala akong makita na link. Send ulit (or `cancel`).",
                                delete_after=10
                            )
                        except:
                            pass
                ...
                if state.get("stage") == "confirm":
                    if content_stripped in {"yes", "y"}:
                        ...
                        if not post_ch:
                            ...
                            try:
                                await message.channel.send(
                                    f"{message.author.mention} Di ko mahanap yung post channel. Admin: set ulit via `pspost #channel`.",
                                    delete_after=10
                                )
                            except:
                                pass
                        ...
                        partner_requests[guild_int_id].pop(user_id, None)
                        try:
                            await message.channel.send(f"{message.author.mention} Posted na sa {post_ch.mention}.", delete_after=10)
                        except:
                            pass
                        return
                    ...
                    if content_stripped in {"no", "n"}:
                        ...
                        try:
                            await message.channel.send(
                                f"{message.author.mention} Sige. Send ulit yung ad server link mo (or `cancel`).",
                                delete_after=60
                            )
                        except:
                            pass
                    ...
                    try:
                        await message.channel.send(f"{message.author.mention} Type `yes` or `no` lang.", delete_after=8)
                    except:
                        pass
                    return
        except Exception:
        ...
        if message.guild and re.match(r"^\s*h!p\s+playlist\b", lower_content):
            ...
            if not playlist_url or not is_allowed_playlist_url(playlist_url.group(0)):
                ...
                try:
                    await message.channel.send(
                        f"{message.author.mention} Spotify at YouTube links lang ang puwede sa `H!p playlist`.",
                        delete_after=6
                    )
                except:
                    pass
