...
def parse_ticket_topic(channel):
    owner_id = None
    claimer_id = None
    if channel.topic:
        if "Ticket owner:" in channel.topic:
            try:
                owner_id = int(channel.topic.split("Ticket owner:")[1].split("|")[0].strip())
            except:
                pass
        if "Claimed by:" in channel.topic:
            try:
                claimer_id = int(channel.topic.split("Claimed by:")[1].strip())
            except:
                pass
    return owner_id, claimer_id
...
class TicketActionView(View):
    ...
    def _parse_topic(self, channel):
        return parse_ticket_topic(channel)
    ...
    async def close_button(self, interaction: discord.Interaction, button: Button):
        ...
        owner_id, claimer_id = self._parse_topic(channel)
    ...
    @discord.ui.button(label="Claim", style=discord.ButtonStyle.success, custom_id="hunterx_ticket_claim")
    async def claim_button(self, interaction: discord.Interaction, button: Button):
        channel = interaction.channel
        guild = interaction.guild
        claimer = interaction.user

        owner_id, existing_claimer = self._parse_topic(channel)

        if claimer.id == owner_id:
            return await interaction.response.send_message("You created this ticket, you can't claim it yourself.", ephemeral=True)

        if not (claimer.guild_permissions.administrator or claimer.id == guild.owner_id or claimer.id == BOT_OWNER_ID):
        ...
        try:
            await channel.edit(topic=f"Ticket owner: {owner_id} | Claimed by: {claimer.id}")
        except:
            pass
        ...
        try:
            ...
            overwrites[claimer] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True, manage_channels=True)
            await channel.edit(overwrites=overwrites)
        except:
            pass

        for item in self.children:
            if item.custom_id == "hunterx_ticket_claim":
                item.label = f"Claimed by {claimer.display_name}"
                item.disabled = True
                item.style = discord.ButtonStyle.gray

        await interaction.response.edit_message(view=self)
        await log_ticket(guild, "Ticket Claimed", f"**Ticket:** #{channel.name}\n**Claimed by:** {claimer.mention} ({claimer.id})")
...
class TicketView(View):
    ...
    async def create_ticket(self, interaction: discord.Interaction, button: Button):
        ...
        await ticket_ch.send(f"Hi {member.mention}, what you need?")

        close_embed = discord.Embed(
            description="If you want to close it click **Close** button below.",
            color=0xFFFFFF
        )
        close_embed.set_footer(text="HunterX")
        await ticket_ch.send(embed=close_embed, view=TicketActionView())
...
def build_help_embed(category: str, prefix: str, bot_thumb: str) -> discord.Embed:
    ...
    if category == "hunterx":
        ...
        desc = '\n'.join([
            ...
            f'`ticketreset`',
            f'`ticketlogs`',
            f'`joinvc` - Join ticket voice channel.',
...
# ==================== TICKET VOICE COMMANDS ====================
@bot.hybrid_command(name="joinvc", aliases=["jvc"])
async def joinvc(ctx):
    ...
    if not channel.topic or "Ticket owner:" not in channel.topic:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Invalid Channel", "This command can only be used inside a ticket channel.")

    owner_id, claimer_id = parse_ticket_topic(channel)
    if member.id != owner_id and member.id != claimer_id and not member.guild_permissions.administrator:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the ticket owner or the staff who claimed this ticket can use this.")

    vc_name = f"{channel.name}-vc"
    ...
    if not vc:
        ...
        owner_member = guild.get_member(owner_id) if owner_id else None
        if owner_member:
            overwrites[owner_member] = discord.PermissionOverwrite(view_channel=True, connect=True)
        overwrites[member] = discord.PermissionOverwrite(view_channel=True, connect=True)
        try:
            vc = await guild.create_voice_channel(name=vc_name, category=channel.category, overwrites=overwrites, reason="Ticket voice channel")
        except Exception as e:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> Error", f"Failed to create voice channel: {e}")
    else:
        try:
    ...
    if member.voice and member.voice.channel:
        try:
            ...
            return await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Joined VC", f"You've been moved to {vc.mention}.")
        except Exception as e:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> Error", f"{vc.mention} is ready, but I couldn't move you automatically: {e}")
    else:
        return await send_embed(ctx, "VC Ready",
            f"{vc.mention} is ready. Join any voice channel first then run `$joinvc` again to be moved automatically, or connect to {vc.mention} manually.")

@bot.hybrid_command(name="dvc", aliases=["disconnectvc", "leavevc"])
async def dvc(ctx):
    member = ctx.author
    if not member.voice or not member.voice.channel:
        return await send_embed(ctx, "<:HunterX:1512787796635422934> Not Connected", "You're not connected to a voice channel.")
    try:
        await member.move_to(None)
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Disconnected", "You have been disconnected from the voice channel.")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", f"Failed to disconnect: {e}")
