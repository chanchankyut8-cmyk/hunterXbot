...
async def _enqueue_playlist(guild: discord.Guild, playlist_url: str) -> int:
    ...
    if isinstance(info, dict) and info.get("entries"):
        for entry in info.get("entries", [])[:25]:
            ...
            music_queues[guild.id].append({"url": url, "title": title})
            added += 1
        return added

    # Single
    url = info.get("webpage_url") or playlist_url
    music_queues[guild.id].append({"url": url, "title": info.get("title")})
    return 1
...
async def _play_next(guild: discord.Guild, text_channel: discord.TextChannel = None):
    ...
    info = await _ytdl_extract(YTDL_STREAM_OPTS, url)
    if not info:
        if text_channel:
            try:
...
def find_badword_match(text: str, badwords: List[str]) -> Optional[str]:
    ...
    for w in badwords:
        ...
        if ww in tokens or ww in lower_text:
            return ww
    return None
...
def build_help_embed(category: str, prefix: str, bot_thumb: str) -> discord.Embed:
    ...
    if category == "hunterx":
        ...
        desc = '\n'.join([
            ...
            f'`set-bot-pfp [image]` - Set bot pfp globally.',
            f'`set-bot-banner [image]` - Set bot banner globally.',
            f'`server-bot-pfp` - Set bot pfp in server-only mode.',
            f'`server-bot-banner` - Set bot banner in server-only mode.',
...
async def _set_bot_avatar(ctx, mode: str = None, image: discord.Attachment = None, *, forced_serveronly: bool = False, command_name: str = "setpfpbot"):
    """
    Usage:
    - setpfpbot [attach image]              -> bot owner only
    - setpfpbot serveronly [attach image]   -> server owner or bot owner
    - svbotpfp [attach image]               -> server owner or bot owner
    """
    if forced_serveronly and not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")

    mode_value = "serveronly" if forced_serveronly else (mode or "").strip().lower()
    if mode_value == "serveronly":
        if not ctx.guild:
            return await send_embed(ctx, "Server Only", "The `serveronly` mode can only be used inside a server.")
        if not can_use_serveronly_bot_media(ctx):
            return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the server owner or bot owner can use `serveronly`.")
    else:
        if ctx.author.id != BOT_OWNER_ID:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the bot owner can use `setpfpbot` without `serveronly`.")

    usage_text = (
        f"`{command_name}` with an attached image."
        if forced_serveronly
        else "`setpfpbot [image]` or `setpfpbot serveronly` with an attached image."
    )
    attachment = image or _ctx_attachment(ctx, "image")
    if not attachment:
    ...
    try:
        ...
        scope = "serveronly" if mode_value == "serveronly" else "global"
        success_title = "Server-Only Bot PFP Updated" if forced_serveronly else "Bot PFP Updated"
        success_message = (
            "The bot profile picture was updated successfully for this server-only mode."
            if forced_serveronly
            else f"Bot profile picture updated successfully. Mode: `{scope}`"
        )
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> " + success_title, success_message)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))

@bot.hybrid_command(name="setpfpbot", aliases=["set-bot-pfp", "bot-pfp"])
async def setpfpbot(ctx, image: discord.Attachment = None):
    # Force server-only behavior (no global mode)
    await _set_bot_avatar(ctx, image=image, forced_serveronly=True, command_name="setpfpbot")

@bot.hybrid_command(name="svbotpfp", aliases=["server-bot-pfp"])
async def svbotpfp(ctx, image: discord.Attachment = None):
    await _set_bot_avatar(ctx, image=image, forced_serveronly=True, command_name="svbotpfp")

async def _set_bot_banner(ctx, mode: str = None, image: discord.Attachment = None, *, forced_serveronly: bool = False, command_name: str = "setbnrbot"):
    """
    Usage:
    - setbnrbot [attach image]              -> bot owner only
    - setbnrbot serveronly [attach image]   -> server owner or bot owner
    - svbotbnr [attach image]               -> server owner or bot owner
    """
    if forced_serveronly and not ctx.guild:
        return await send_embed(ctx, "Server Only", "This command can only be used inside a server.")
    ...
    if mode_value == "serveronly":
        ...
        if ctx.author.id != BOT_OWNER_ID:
            return await send_embed(ctx, "<:HunterX:1512787796635422934> No Permission", "Only the bot owner can use `setbnrbot` without `serveronly`.")

    usage_text = (
        f"`{command_name}` with an attached image."
        if forced_serveronly
        else "`setbnrbot [image]` or `setbnrbot serveronly` with an attached image."
    )
    attachment = image or _ctx_attachment(ctx, "image")
    if not attachment:
        return await send_embed(ctx, "Usage", usage_text)
    ...
    try:
        ...
        scope = "serveronly" if mode_value == "serveronly" else "global"
        success_title = "Server-Only Bot Banner Updated" if forced_serveronly else "Bot Banner Updated"
        success_message = (
            "The bot banner was updated successfully for this server-only mode."
            if forced_serveronly
            else f"Bot banner updated successfully. Mode: `{scope}`"
        )
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> " + success_title, success_message)
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))
...
async def setbnrbot(ctx, image: discord.Attachment = None):
    # Force server-only behavior (no global mode)
    await _set_bot_banner(ctx, image=image, forced_serveronly=True, command_name="setbnrbot")

@bot.hybrid_command(name="svbotbnr", aliases=["server-bot-banner"])
async def svbotbnr(ctx, image: discord.Attachment = None):
    await _set_bot_banner(ctx, image=image, forced_serveronly=True, command_name="svbotbnr")
...
async def badwords_add(ctx, *, word: str):
    ...
    words.sort()
    save_config()
    await send_embed(ctx, "<:HunteXCheck:1518422536558481448> Badword Added", f"`{w}` added.")
...
async def leveltop(ctx):
    ...
    if not users:
        return await send_embed(ctx, "Level Leaderboard", "Wala pang data.")
...
async def all_logs(ctx):
    ...
    try:
        ...
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> All Logs",
            f"{ch.mention} created. Lahat ng logs imi-mirror dito (admin-only).")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))
...
async def vclogs(ctx):
    ...
    try:
        ...
        await send_embed(ctx, "<:HunteXCheck:1518422536558481448> VC Logs",
            f"{ch.mention} created. Lahat ng voice logs maglologs dito (admin-only).")
    except Exception as e:
        await send_embed(ctx, "<:HunterX:1512787796635422934> Error", str(e))
...
def _setupsv_embed_text() -> discord.Embed:
    desc = (
        ...
        "Setup your channels\n\n"
        "Choose Category\n"
        "Tapos Select a category button sa baba."
    )
    return discord.Embed(description=desc, color=0xFFFFFF)
...
async def on_message(message):
    ...
    if gid:
        ...
        if message.guild:
            ...
            if (keyword_list
                    and not is_whitelisted(message.guild, message.author)
                    and not message.author.guild_permissions.administrator
                    and message.author.id != message.guild.owner_id):
                matched_keyword = next(
                    (word for word in keyword_list if word and word.lower() in lower_content),
                    None
                )
                if matched_keyword:
                    ...
                    if not can_delete:
                        ...
                        try:
                            ...
                            if now_ts - last_ts >= 300:
                                ...
                                await message.channel.send(
                                    "Hindi ko ma-auto delete ang keyword/badwords dahil kulang ako ng permission: `Manage Messages`.",
                                    delete_after=10
                                )
                        except Exception:
                            pass
...
async def on_member_join(member):
    ...
    # Anti-Mass Raid: kapag >15 joins within 5 seconds, i-kick agad ang latest 5.
